import streamlit as st
import json
import os

# Set page configuration
st.set_page_config(
    page_title="מערכת הכנה למבחן כניסה - שיקום הפה",
    page_icon="🦷",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Custom CSS for Hebrew RTL & Beautiful UI
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Rubik:wght@300;400;500;700&display=swap');
    
    html, body, [class*="css"] {
        font-family: 'Rubik', sans-serif;
        direction: rtl;
        text-align: right;
    }
    
    .main-header {
        background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%);
        color: white;
        padding: 1.8rem;
        border-radius: 12px;
        box-shadow: 0 4px 15px rgba(0,0,0,0.1);
        margin-bottom: 1.5rem;
        text-align: center;
    }
    
    .question-card {
        background-color: #ffffff;
        border: 1px solid #e0e6ed;
        border-radius: 10px;
        padding: 1.2rem;
        margin-bottom: 1rem;
        box-shadow: 0 2px 8px rgba(0,0,0,0.04);
    }
    
    .question-title {
        font-size: 1.15rem !important;
        font-weight: 600;
        color: #1e293b;
        margin-top: 0.5rem;
        margin-bottom: 0.8rem;
        line-height: 1.5;
        text-align: right;
    }

    .difficulty-badge {
        display: inline-block;
        padding: 4px 12px;
        border-radius: 20px;
        font-weight: 500;
        font-size: 0.85rem;
        margin-bottom: 8px;
    }
    
    .badge-easy { background-color: #e6f4ea; color: #137333; }
    .badge-medium { background-color: #fef7e0; color: #b06000; }
    .badge-hard { background-color: #feefe3; color: #c5221f; }
    .badge-expert { background-color: #f3e8fd; color: #7627bb; }
    
    .explanation-box {
        background-color: #f8f9fa;
        border-right: 4px solid #1e3c72;
        padding: 1rem 1.2rem;
        margin-top: 1rem;
        border-radius: 4px;
        font-size: 0.98rem;
        line-height: 1.6;
        text-align: right;
    }

    .bookmark-box {
        background-color: #fff9db;
        border: 1px solid #fcc419;
        border-radius: 8px;
        padding: 1rem;
        margin-top: 0.8rem;
        font-size: 0.95rem;
        text-align: right;
    }
    
    .stRadio > label {
        font-size: 1.05rem !important;
        font-weight: 500 !important;
        text-align: right !important;
        direction: rtl !important;
    }

    div[role="radiogroup"] {
        direction: rtl !important;
        text-align: right !important;
    }
</style>
""", unsafe_allow_html=True)

# 15 Topics List
TOPICS = [
    "1. אוקלוזיה, TMJ ותנועות לסת",
    "2. ארטיקולטורים ורישום בין-לסתי",
    "3. עקרונות השחזת שיניים וגאומטריה שיקומית",
    "4. חומרי מטבע וטכניקות הטבעה בשיקום קבוע",
    "5. צמנטים דנטליים, מתכות וסרמיקות",
    "6. תותבות חלקיות נשלפות (RPD): ביומכניקה ומחברים",
    "7. RPD: מאחזים, מסעדים וקומפלקס RPI",
    "8. תותבות שלמות: גבולות אנטומיים ואזורי תמיכה",
    "9. תותבות שלמות: אוקלוזיה, מימד אנכי ויחס בין-לסתי",
    "10. ספיגת עצם אלבאולרית ופתולוגיות רירית",
    "11. שתלים דנטליים: ביומכניקה, איכות עצם ותכנון",
    "12. שיקום על גבי שתלים: מבנים ואפשרויות שיקומיות",
    "13. יחסי גומלין פריודונטיה-שיקום וניידות שיניים",
    "14. תותבות על (Overdentures) ואטצ'מנטים",
    "15. אסתטיקה, בחירת צבע ורפואת שיניים דיגיטלית"
]

ALL_QUESTIONS = {
  "1": [
    {
      "id": 1,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מה נכון לגבי המבנה האנטומי של הדיסק הארטיקולרי במפרק הלסת (TMJ)?",
      "options": [
        "א. החלק האחורי (Posterior band) הוא החלק העבה ביותר בדיסק.",
        "ב. החלק המרכזי (Intermediate zone) הוא העבה ביותר ומכיל עצבוב עשיר.",
        "ג. החלק הקדמי (Anterior band) הוא העבה ביותר בדנטליזציה תקינה.",
        "ד. ה-Retrodiscal pad מורכב מסחוס היאליני קשיח ונטול כלי דם."
      ],
      "correct": "א. החלק האחורי (Posterior band) הוא החלק העבה ביותר בדיסק.",
      "explanation": "לפי שחזורי המבחנים והספרות העיונית (זארב / שילינבורג), הדיסק הארטיקולרי ב-TMJ מורכב מ-Dense connective tissue. החלק האחורי (Posterior band) הוא העבה ביותר, החלק המרכזי (Intermediate zone) הוא הדק ביותר והוא האזור נושא העומס, והחלק הקדמי (Anterior band) בעל עובי בינוני.",
      "summary": "הדיסק הארטיקולרי ב-TMJ: החלק האחורי (Posterior band) הוא העבה ביותר, החלק המרכזי (Intermediate zone) הוא הדק ביותר ונושא את העומס. הדיסק אינו עצבוב בחלקו המרכזי."
    },
    {
      "id": 2,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "איזו תכונה מתארת נכונה ארטיקולטור מסוג Arcon בהשוואה לארטיקולטור Non-Arcon?",
      "options": [
        "א. הרכיב הקונדילרי ממוקם בזרוע העליונה והפוסה המכנית ממוקמת בזרוע התחתונה.",
        "ב. הרכיב הקונדילרי ממוקם בזרוע התחתונה והפוסה המכנית ממוקמת בזרוע העליונה, בדומה לאנטומיה האנושית.",
        "ג. שינוי המימד האנכי (VDO) גורם ל-Condylar guidance להשתנות ולהפוך לרדודה יותר.",
        "ד. הוא מתאים לשימוש אך ורק בביצוע תותבות שלמות ולא בשיקומים קבועים. בהתאם להנחיות הכלליות המופיעות בספרות הלימוד הדנטלית."
      ],
      "correct": "ב. הרכיב הקונדילרי ממוקם בזרוע התחתונה והפוסה המכנית ממוקמת בזרוע העליונה, בדומה לאנטומיה האנושית.",
      "explanation": "לפי שילינבורג (פרק 3), בארטיקולטור Arcon הרכיבים הקונדילריים מחוברים לזרוע התחתונה והפוסה לזרוע העליונה (כמו בגולגולת). יתרונו הגדול הוא שבשינוי המימד האנכי (הסרת משנכי שעווה) הזווית בין הפוסה למישור הסגר נשמרת קבועה, ולכן ה-Condylar guidance אינו משתנה.",
      "summary": "ארטיקולטור Arcon: קונדיל בזרוע תחתונה, פוסה בזרוע עליונה (מדמה אנטומיה אנושית). זווית ההדרכה הקונדילרית אינה משתנה בעת שינוי המימד האנכי."
    },
    {
      "id": 3,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "באיזה מדור של מפרק הלסת (TMJ) מתרחשת התנועה הצירית (Rotational / Hinge movement)?",
      "options": [
        "א. בתוך ה-Retrodiscal tissue בלבד.",
        "ב. במדור העליון (Superior compartment) בלבד.",
        "ג. במדור התחתון (Inferior compartment) בלבד.",
        "ד. בשני המדורים בו-זמנית ובמידה שווה."
      ],
      "correct": "ג. במדור התחתון (Inferior compartment) בלבד.",
      "explanation": "התנועה הצירית (תנועת ציר / הרוטציה הראשונית עד פתיחה של כ-20-25 מ\"מ) מתרחשת במדור התחתון של המפרק (בין הקונדיל למשטח התחתון של הדיסק). התנועה המחליקה (Translational movement) מתרחשת במדור העליון (בין הדיסק ל-Articular eminence).",
      "summary": "קינמטיקת ה-TMJ: תנועת רוטציה (צירית) מתרחשת במדור התחתון (Inferior compartment), בעוד תנועת טרנסלציה (החלקה) מתרחשת במדור העליון (Superior compartment)."
    },
    {
      "id": 4,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "בעת ביצוע צילום פנורמי, התקבלה תמונה בה הלסת התחתונה נראית צרה מדי והעקומה מודגשת. מה קרה במהלך הצילום?",
      "options": [
        "א. ראש המטופל הוטה אחורנית באופן מוגזם. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. המטופל מוקם אחורה מדי (Backward to the focal trough).",
        "ג. המטופל נמוך מדי ביחס למכשיר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. המטופל מוקם קדימה מדי (Forward to the focal trough)."
      ],
      "correct": "ד. המטופל מוקם קדימה מדי (Forward to the focal trough).",
      "explanation": "לפי שחזורי המבחנים והספרות, כאשר המטופל ממוקם קדימה מדי מחוץ ל-Focal trough בצילום פנורמי, השיניים הקדמיות והלסת ייראו מוצרות וצרות (Narrow/magnified in reverse), בעוד שמיקום אחורה מדי גורם להרחבה וטשטוש.",
      "summary": "עיוותים בצילום פנורמי: מיקום מטופל קדימה מדי ביחס ל-Focal trough גורם להצרת השיניים הקדמיות והפרזת העקומה. מיקום אחורה מדי גורם להרחבה וטשטוש."
    },
    {
      "id": 5,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "איזה שריר מבין הבאים אחראי על התחלת סגירת הלסת (Elevation of the mandible)?",
      "options": [
        "א. Medial pterygoid",
        "ב. Digastric muscle",
        "ג. Lateral pterygoid",
        "ד. Geniohyoid muscle"
      ],
      "correct": "א. Medial pterygoid",
      "explanation": "לפי השחזורים והספרות העיונית, שריר ה-Medial Pterygoid (יחד עם ה-Masseter וה-Temporalis) הוא מניף הלסת, כאשר בשאלות השחזור מודגש ה-Medial Pterygoid כמי שמתחיל את פעולת ה-Elevation/סגירה.",
      "summary": "שרירי הלעיסה: Medial Pterygoid, Masseter ו-Temporalis הם שרירי סגירה (Elevators). ה-Lateral Pterygoid אחראי על פתיחה, פרוטרוזיה ותנועות צדיות."
    },
    {
      "id": 6,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מהו המרחק הממוצע המקובל עבור ה-Interocclusal distance (Freeway space) במצב מנוחה (VDR)?",
      "options": [
        "א. 0.5 - 1.0 מ\"מ",
        "ב. 2.0 - 4.0 מ\"מ",
        "ג. 10.0 - 12.0 מ\"מ",
        "ד. 6.0 - 8.0 מ\"מ"
      ],
      "correct": "ב. 2.0 - 4.0 מ\"מ",
      "explanation": "ה-Freeway space מוגדר כהפרש בין המימד האנכי במנוחה (VDR) לבין המימד האנכי בסגר (VDO). ערך פיזיולוגי תקין במרבית האוכלוסיה הוא 2 עד 4 מ\"מ.",
      "summary": "Freeway Space (Interocclusal Rest Space): ההפרש בין VDR ל-VDO נע באופן תקין בין 2 ל-4 מ\"מ (VDR = VDO + Freeway space)."
    },
    {
      "id": 7,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "איזה חלק בשריר ה-Lateral Pterygoid מופעל בעיקר בעת סגירת הפה והידוק השיניים?",
      "options": [
        "א. שני החלקים פועלים בסינרגיה מלאה בעת סגירה.",
        "ב. ה-Inferior belly של ה-Lateral Pterygoid.",
        "ג. ה-Superior belly של ה-Lateral Pterygoid.",
        "ד. שום חלק אינו פועל בעת סגירה."
      ],
      "correct": "ג. ה-Superior belly של ה-Lateral Pterygoid.",
      "explanation": "ה-Superior belly פועל בעת סגירה והידוק (Stabilizing the disc-condyle complex), ואילו ה-Inferior belly פועל בעת פתיחה, פרוטרוזיה ותנועה לטרלית.",
      "summary": "Lateral Pterygoid: הראשי העליון (Superior head) פועל בעת סגירה והידוק לייצוב קומפלקס דיסק-קונדיל, בעוד הראש התחתון (Inferior head) פועל בפתיחה וגלישה."
    },
    {
      "id": 8,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מה תפקידה העיקרי של קשת הפנים (Facebow) בהעברת הגבסים לארטיקולטור?",
      "options": [
        "א. העברת היחס של הלסת התחתונה לציר הסיבוב המפרקי ב-CR. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. בדיקת זווית Bennett באופן דינמי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. קביעת ה-Condylar guidance angle בצורה אוטומטית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. העברת היחס התלת-ממדי של הלסת העליונה לבסיס הגולגולת ולציר הסיבוב הקונדילרי."
      ],
      "correct": "ד. העברת היחס התלת-ממדי של הלסת העליונה לבסיס הגולגולת ולציר הסיבוב הקונדילרי.",
      "explanation": "קשת פנים (Facebow) מעבירה את מיקום הלסת העליונה ביחס לבסיס הגולגולת וציר הסיבוב הקונדילרי (Terminal Hinge Axis) אל הזרוע העליונה של הארטיקולטור.",
      "summary": "קשת פנים (Facebow): מעבירה את היחס התלת-ממדי של המקסילה (הלסת העליונה) ביחס לציר הסיבוב הקונדילרי ולבסיס הגולגולת אל הארטיקולטור."
    },
    {
      "id": 9,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "כיצד מוגדרת עמדת Terminal Hinge Axis (ציר הסיבוב הסופי)?",
      "options": [
        "א. ציר דמיוני המחבר את שני הקונדילים כאשר הם ממוקמים בעמדת Centric Relation ומתרחשת רוטציה טהורה.",
        "ב. ציר הסיבוב של החותכות התחתונות בעת לקיחת מטבע. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הציר העובר דרך מרכז המזואיד בעת פתיחה מקסימלית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. ציר קיומי שסביבו מתרחשת תנועת טרנסלציה בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "א. ציר דמיוני המחבר את שני הקונדילים כאשר הם ממוקמים בעמדת Centric Relation ומתרחשת רוטציה טהורה.",
      "explanation": "Terminal Hinge Axis הוא הציר הדמיוני שעובר דרך שני הקונדילים בעודם ב-CR, שסביבו הלסת התחתונה יכולה לבצע תנועת סיבוב (רוטציה) טהורה עד פתיחה של כ-20-25 מ\"מ.",
      "summary": "Terminal Hinge Axis: ציר הסיבוב הרוטציוני הטהור של המנדיבולה כאשר הקונדילים נמצאים בעמדת Centric Relation (ללא טרנסלציה)."
    },
    {
      "id": 10,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "איזו נקודה במעטפת התנועה של Posselt מייצגת את הסגירה במקסימום מגעים בין השיניים?",
      "options": [
        "א. Edge-to-Edge Position. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. ICP / MI (Intercuspal Position / Maximum Intercuspation).",
        "ג. Maximum Protrusion. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. RCP (Retruded Contact Position). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. ICP / MI (Intercuspal Position / Maximum Intercuspation).",
      "explanation": "ICP / MI היא עמדת הסגירה שבה השיניים העליונות והתחתונות מגיעות למקסימום מגעים אוקלוזליים, ללא תלות בעמדת המפרק.",
      "summary": "Posselt's Envelope of Motion: ICP/MI מייצגת את הסגירה הדנטלית במקסימום מגעים. RCP מייצגת את המגע הראשוני בעמדת Centric Relation."
    },
    {
      "id": 11,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "בסגר מסוג Group Function, אילו שיניים עומדות במגע בעת תנועה צדית בצד העובד (Working side)?",
      "options": [
        "א. החותכות המרכזיות והצדדיות בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. השיניים בצד הלא-עובד בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. הניב והשיניים האחוריות (מלתעות וטחנות) בצד העובד.",
        "ד. הניב בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. הניב והשיניים האחוריות (מלתעות וטחנות) בצד העובד.",
      "explanation": "ב-Group Function, בעת תנועה צדית, המגעים מתחלקים בין הניב לבין התלוליות הבוקאליות של השיניים האחוריות בצד העובד, תוך הפרדה (Disclusion) בצד הלא-עובד.",
      "summary": "Group Function: חלוקת מגעים בתנועה לטרלית בין הניב והשיניים האחוריות בצד העובד (Working side), תוך ניתוק מגע בצד הלא-עובד."
    },
    {
      "id": 12,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מה מאפיין את הנוזל הסינוביאלי (Synovial fluid) במפרק ה-TMJ?",
      "options": [
        "א. הוא מיוצר ע\"י הסחוס ההיאליני ומכיל אחוז גבוה של תאי דם אדומים.",
        "ב. זרימתו נערכת אך ורק בזמן שינה ואין לו תפקיד בסיכה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הוא מצוי אך ורק במדור התחתון של המפרק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הוא מספק הזנה וחמצן לרקמות האווסקולריות של הדיסק והסחוס המפרקי, ומשמש כחומר סיכה."
      ],
      "correct": "ד. הוא מספק הזנה וחמצן לרקמות האווסקולריות של הדיסק והסחוס המפרקי, ומשמש כחומר סיכה.",
      "explanation": "הנוזל הסינוביאלי מיוצר ע\"י הממברנה הסינוביאלית וממלא שני תפקידים קריטיים: הזנת המשטחים האווסקולריים (הדיסק והסחוס) והפחתת חיכוך (סיכה).",
      "summary": "נוזל סינוביאלי ב-TMJ: אחראי על הזנת הרקמות נטולות כלי הדם (דיסק ומשטחי המפרק) ומשמש כחומר סיכה להפחתת חיכוך בתנועה."
    },
    {
      "id": 13,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "איזה ligament במפרק ה-TMJ אחראי על הגבלת פתיחת הפה המוגזמת והגנה על הרקמות האחוריות?",
      "options": [
        "א. Temporomandibular (Lateral) ligament",
        "ב. Pterygomandibular raphe כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Stylomandibular ligament כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Sphenomandibular ligament כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. Temporomandibular (Lateral) ligament",
      "explanation": "ה-Temporomandibular ligament (חלקו האלכסוני והאופקי) מחזק את הקפסולה הצידית ומגביל את תנועת הרוטציה האחורית והסטת הקונדיל אחורנית.",
      "summary": "Temporomandibular Ligament: הרצועה המרכזית המייצבת את המפרק מצידו הלטרלי ומגבילה תנועה אחורנית מופרזת של הקונדיל."
    },
    {
      "id": 14,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מהי ההגדרה של Centric Occlusion (CO) בספרות הקלאסית?",
      "options": [
        "א. עמדה פרוטרוזיבית של 3 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מגע השיניים בעת שהלסת התחתונה נמצאת בעמדת Centric Relation.",
        "ג. עמדת מנוחה שבה השרירים רפויים לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. המגע המקסימלי בין החותכות העליונות והתחתונות."
      ],
      "correct": "ב. מגע השיניים בעת שהלסת התחתונה נמצאת בעמדת Centric Relation.",
      "explanation": "Centric Occlusion (CO) מוגדרת בספרות הקלאסית כסגירת השיניים (מגעים אוקלוזליים) כאשר הלסת התחתונה ממוקמת בעמדת Centric Relation (RCP).",
      "summary": "Centric Occlusion (CO): מגע השיניים שמתרחש כאשר הקונדילים נמצאים בעמדת Centric Relation (נקרא גם RCP)."
    },
    {
      "id": 15,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "באיזה מביניהם ממוקמת ה-Articular Eminence ביחס ל-Glenoid Fossa?",
      "options": [
        "א. פוסטריורית (אחורית) ל-Glenoid fossa.",
        "ב. מדיאלית בלבד ל-External auditory meatus.",
        "ג. אנטריורית (קדמית) ל-Glenoid fossa.",
        "ד. בתוך ה-Infratemporal fossa."
      ],
      "correct": "ג. אנטריורית (קדמית) ל-Glenoid fossa.",
      "explanation": "ה-Articular eminence היא הבליטה הגרמית הממוקמת קדימה (אנטריורית) ל-Glenoid fossa, ועליה מחליק קומפלקס הקונדיל והדיסק בעת פתיחה ופרוטרוזיה.",
      "summary": "Articular Eminence: ממוקמת קדימה (אנטריורית) ל-Glenoid fossa בעצם הטמפורלית ומהווה את שביל הגלישה של הקונדיל."
    },
    {
      "id": 16,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מהי עקומת Spee (Curve of Spee)?",
      "options": [
        "א. עקומת שחיקה הנגרמת עקב ברוקסיזם בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. עקומה אופקית במישור הפרונטלי המחברת את התלוליות הבוקאליות והלינגואליות. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר.",
        "ג. עקומה הנוצרת רק בתותבות שלמות מאוזנות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. עקומה אנטרו-פוסטריורית במישור הסגיטלי המתחילה מלהב הניב התחתון ועוברת דרך התלוליות הבוקאליות של השיניים האחוריות."
      ],
      "correct": "ד. עקומה אנטרו-פוסטריורית במישור הסגיטלי המתחילה מלהב הניב התחתון ועוברת דרך התלוליות הבוקאליות של השיניים האחוריות.",
      "explanation": "Curve of Spee היא עקומה אנטרו-פוסטריורית אנטומית במישור הסגיטלי. Curve of Wilson היא העקומה הרוחבית במישור הפרונטלי.",
      "summary": "Curve of Spee: עקומה סגיטלית (אנטרו-פוסטריורית). Curve of Wilson: עקומה פרונטלית (רוחבית/טרנסברסלית)."
    },
    {
      "id": 17,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "כיצד נקראת העקומה האוקלוזלית הרוחבית במישור הפרונטלי המחברת את התלוליות הימניות והשמאליות?",
      "options": [
        "א. Curve of Wilson",
        "ב. Christensen curve",
        "ג. Monson sphere",
        "ד. Curve of Spee"
      ],
      "correct": "א. Curve of Wilson",
      "explanation": "Curve of Wilson היא העקומה האוקלוזלית הרוחבית במישור הפרונטלי (מבט חזיתי) המחברת את התלוליות הבוקאליות והלינגואליות של המשנן התחתון והעליון.",
      "summary": "Curve of Wilson: העקומה האוקלוזלית הרוחבית במישור הפרונטלי."
    },
    {
      "id": 18,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מה קורה לדיסק הארטיקולרי במצב של Internal Derangement עם הקלקה (Disc Displacement with Reduction)?",
      "options": [
        "א. הדיסק נשאר תקוע אחורנית ואינו זז כלל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הדיסק מוזז קדימה במנוחה, ובפתיחת הפה הקונדיל עולה עליו בחזרה תוך צליל הקלקה (Click).",
        "ג. הדיסק נעלם לחלוטין עקב ספיגה גרמית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הדיסק עורק מדיאלית ללא אפשרות לחזור (Without reduction). עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "ב. הדיסק מוזז קדימה במנוחה, ובפתיחת הפה הקונדיל עולה עליו בחזרה תוך צליל הקלקה (Click).",
      "explanation": "ב-Disc displacement WITH reduction, הדיסק מועתק אנטריורית במנוחה, ובמהלך הפתיחה הקונדיל מחליק ועולה בחזרה על הדיסק (Reduction) תוך הקלקה.",
      "summary": "Disc Displacement with Reduction: הדיסק מועתק אנטריורית בסגירה, אך חוזר למקומו התקין מעל הקונדיל בעת פתיחה תוך צליל הקלקה."
    },
    {
      "id": 19,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "איזה עצב מבין הבאים מספק את העצבוב התחושתי המרכזי למפרק ה-TMJ?",
      "options": [
        "א. Hypoglossal nerve (CN XII)",
        "ב. Greater palatine nerve",
        "ג. Auriculotemporal nerve",
        "ד. Facial nerve (CN VII)"
      ],
      "correct": "ג. Auriculotemporal nerve",
      "explanation": "העיצבוב התחושתי העיקרי ל-TMJ מגיע מה-Auriculotemporal nerve (סעיף של V3), יחד עם ענפים מ-Deep temporal nerve ו-Masseteric nerve.",
      "summary": "עצבוב ה-TMJ: מסופק בעיקרו ע\"י ה-Auriculotemporal nerve (ענף מ-V3), בתוספת ענפים מ-Masseteric ו-Deep temporal nerves."
    },
    {
      "id": 20,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קל",
      "question": "מה מאפיין את המושג Christensen's Phenomenon בתותבות שלמות ובאוקלוזיה?",
      "options": [
        "א. מגע חזק ויתר בטוחנות בעת פתיחת פה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. התכווצות של שריר ה-Masseter בעת בליעה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. ספיגה מהירה של העצם האלבאולרית בחלק הקדמי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הופעת רווח אוקלוזלי (Disclusion) בין השיניים האחוריות בעת הגלשת הלסת קדימה (פרוטרוזיה)."
      ],
      "correct": "ד. הופעת רווח אוקלוזלי (Disclusion) בין השיניים האחוריות בעת הגלשת הלסת קדימה (פרוטרוזיה).",
      "explanation": "תופעת כריסטנסן (Christensen's phenomenon) היא הופעת הרווח האופקי/אנכי בין השיניים האחוריות בעת תנועה פרוטרוזיבית, שנגרמת עקב הדרכת הזווית הקונדילרית.",
      "summary": "Christensen's Phenomenon: היפרדות (Disclusion) ורווח בין המשננים האחוריים בעת תנועה פרוטרוזיבית עקב זווית השיפוע הקונדילרי."
    },
    {
      "id": 21,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מהי ההגדרה המדויקת של תנועת Bennett (Bennett Movement / Side Shift)?",
      "options": [
        "א. התנועה הצדית הגופנית (Bodily lateral movement) של הקונדיל בצד העובד (Working side) בעת תנועה לטרלית.",
        "ב. הגלישה הקדמית-מדיאלית של הקונדיל בצד הלא-עובד בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. תנועה אנכית טהורה של המנדיבולה בעת סגירה ב-Centric Relation. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר.",
        "ד. מעבר המנדיבולה מעמדת Rest Position ל-Maximum Intercuspation. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. התנועה הצדית הגופנית (Bodily lateral movement) של הקונדיל בצד העובד (Working side) בעת תנועה לטרלית.",
      "explanation": "תנועת Bennett מוגדרת כתנועה הצידית הגופנית של המנדיבולה בצד העובד (Working side condyle). בצד הלא-עובד (Non-working side), תנועה זו משתקפת כזווית Bennett (Bennett angle) והחלקה קדימה, למטה ומדיאלית.",
      "summary": "תנועת Bennett: התנועה הגופנית הצידית של הקונדיל בצד העובד בעת תנועה לטרלית. משפיעה על מורפולוגיית התלוליות והחריצים בשיניים האחוריות."
    },
    {
      "id": 22,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מה מאפשר לדיסק הארטיקולרי ב-TMJ לנוע קדימה מול ה-Articular Eminence בעת פתיחת פה?",
      "options": [
        "א. חיבור גרמי קשיח ל-Tympanic plate. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. היעדר חיבור של הדיסק לעצם הטמפורלית (Lack of direct osseous attachment to the temporal bone).",
        "ג. הידוק הסיבים האלסטיים התחתונים אל צוואר הקונדיל בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. מתיחה קבועה של ה-Temporomandibular ligament בצד המדיאלי. בהתאם להנחיות הכלליות המופיעות בספרות הלימוד הדנטלית."
      ],
      "correct": "ב. היעדר חיבור של הדיסק לעצם הטמפורלית (Lack of direct osseous attachment to the temporal bone).",
      "explanation": "לפי ספרי הלימוד (Mohl / Zarb) ושחזורי המבחנים (2016, 2017, 2018), מה שמאפשר לדיסק הארטיקולרי את החופש החלקתי לנוע קדימה יחד עם הקונדיל מול ה-Articular Eminence הוא העובדה שאינו מחובר מורפולוגית ישירות לעצם הטמפורלית.",
      "summary": "תנועתיות הדיסק הארטיקולרי: היעדר חיבור גרמי ישיר של הדיסק לעצם הטמפורלית מאפשר לו להחליק חופשי קדימה יחד עם הקונדיל מעבר ל-Articular Eminence."
    },
    {
      "id": 23,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "לאחר ביצוע רישום בין-לסתי בעובי 4 מ\"מ בארטיקולטור מסוג Non-Arcon והסרת המשנך, הגבסים הובאו למגע אוקלוזלי. מה יתרחש מבחינת ה-Condylar inclination?",
      "options": [
        "א. ה-Condylar guidance תתאפס לחלוטין לקו האופק.",
        "ב. ה-Condylar inclination תשתנה ותהפוך לתלולה יותר (Steeper).",
        "ג. ה-Condylar inclination תשתנה ותהפוך לרדודה יותר (Less steep).",
        "ד. ה-Condylar inclination תישאר ללא שינוי. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "ג. ה-Condylar inclination תשתנה ותהפוך לרדודה יותר (Less steep).",
      "explanation": "לפי שילינבורג (עמ' 28-30), בארטיקולטור Non-Arcon הרכיב הקונדילרי ממוקם בזרוע העליונה. כשסוגרים את המנשך לאחר הסרת משנך בעובי 3-5 מ\"מ, הזווית הגיאומטרית משתנה וההדרכה הקונדילרית הופכת לרדודה יותר (Less steep). בארטיקולטור Arcon היא נשארת ללא שינוי.",
      "summary": "שינוי VDO ב-Non-Arcon: סגירת הארטיקולטור לאחר הסרת משנך עבה גורמת להדרכה הקונדילרית להפוך לרדודה יותר (Less steep). בארטיקולטור Arcon הזווית נשארת קבועה."
    },
    {
      "id": 24,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מדוע נקבעת עמדת Centric Relation (CR) כעמדת הייחוס המועדפת בשיקום פה נרחב ובתותבות שלמות?",
      "options": [
        "א. מכיוון שבעמדה זו מבוטלת לחלוטין תנועת ה-Bennett side shift. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מכיוון שבעמדה זו שרירי הלעיסה מגיעים לשיא כוח ההתכווצות שלהם. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מכיוון שזו עמדה שמוכתבת ע\"י המגעים האוקלוזליים של השיניים הקיימות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מכיוון שזו עמדה מפרקית אנטומית שניתנת לשחזור (Repeatable/Reproducible) ואינה תלויה במגע שיניים."
      ],
      "correct": "ד. מכיוון שזו עמדה מפרקית אנטומית שניתנת לשחזור (Repeatable/Reproducible) ואינה תלויה במגע שיניים.",
      "explanation": "עמדת Centric Relation (CR) מוגדרת כעמדה המפרקית של הקונדילים בחלק הכי אנטרו-סופריורי של הפוסה מול המשטח האוולרי של הדיסק. היא אינה תלויה בשיניים, והיתרון הקליני המרכזי שלה הוא היותה ניתנת לשחזור מדויק ומהווה עמדת מוצא בטוחה לשיקום נרחב.",
      "summary": "חשיבות עמדת CR: עמדה מפרקית ניתנת לשחזור (Reproducible) שאינה תלויה בשיניים, ולכן משרתת כעמדת ייחוס בטוחה לשיקום נרחב ומשוחזר."
    },
    {
      "id": 25,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "למה מתייחס המונח האסתטי \"Gull-Wing Appearance\" במראה המשנן הקדמי העליון?",
      "options": [
        "א. מצב שבו הקצה האינסיזלי של החותכת הלטרלית העליונה ממוקם אפיקלית לקצוות האינסיזליים של הצנטרלית והניב.",
        "ב. מראה שנוצר עקב שחיקה זוויתית של החותכות המרכזיות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מתאר אנטומי קעור של קו החניכיים מעל הניב בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. מראה שנוצר כאשר קו האמצע הדנטלי מוסט ב-2 מ\"מ מאישוני העיניים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. מצב שבו הקצה האינסיזלי של החותכת הלטרלית העליונה ממוקם אפיקלית לקצוות האינסיזליים של הצנטרלית והניב.",
      "explanation": "לפי שחזורי המבחנים (2024, 2012), \"Gull-Wing Appearance\" מתאר מתאר אסתטי שבו הקצה האינסיזלי של הלטראלית העליונה קצר יותר (ממוקם אפיקלית) ביחס לקו המחבר את הקצוות האינסיזליים של הצנטרלית והניב, מה שיוצר מראה דמוי כנפי שחף.",
      "summary": "Gull-Wing Appearance: מתאר קו חיתוך אסתטי קדמי שבו החותכת הלטרלית העליונה קצרה יותר (אפיקלית) מלהבי הצנטרלית והניב."
    },
    {
      "id": 26,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "כיצד מחושבת זווית Bennett (Bennett Angle) בארטיקולטור חצי-מתכוונן לפי נוסחת Hanau?",
      "options": [
        "א. L = (H + 10) / 4",
        "ב. L = (H / 8) + 12",
        "ג. L = H * 1.5 - 5",
        "ד. L = (H / 2) + 20"
      ],
      "correct": "ב. L = (H / 8) + 12",
      "explanation": "נוסחת הנאו (Hanau's formula) לחישוב זווית Bennett (L) על סמך זווית ההדרכה הקונדילרית האופקית (H) היא: L = (H / 8) + 12.",
      "summary": "נוסחת Hanau לזווית Bennett: L = (H / 8) + 12 (כאשר L היא זווית ה-Bennett ו-H היא ה-Horizontal condylar inclination)."
    },
    {
      "id": 27,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מהי ההגדרה המדויקת של Primary Occlusal Trauma (טראומה אוקלוזלית ראשונית)?",
      "options": [
        "א. ניידות שן שנוצרת אך ורק עקב ספיגת עצם דלקתית חריפה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. טראומה שמתרחשת עקב אובדן מגע ב-Centric Relation בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. כוחות אוקלוזליים מופרזים המופעלים על שן בעלת תמיכה פריודונטלית תקינה ובריאה.",
        "ד. כוחות אוקלוזליים תקינים ופיזיולוגיים המופעלים על שן בעלת תמיכה פריודונטלית מופחתת ושמורה."
      ],
      "correct": "ג. כוחות אוקלוזליים מופרזים המופעלים על שן בעלת תמיכה פריודונטלית תקינה ובריאה.",
      "explanation": "Primary occlusal trauma מוגדרת כנזק הרקמתי הנגרם מכוחות אוקלוזליים חריגים המופעלים על מנגנון תמיכה פריודונטלי תקין. Secondary occlusal trauma מתרחשת כאשר כוחות תקינים/חריגים מופעלים על מנגנון מופחת.",
      "summary": "טראומה אוקלוזלית: ראשונית (Primary) = כוחות חריגים על תמיכה בריאה. שניונית (Secondary) = כוחות (תקינים/חריגים) על תמיכה פריודונטלית מופחתת."
    },
    {
      "id": 28,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מה משפיע יותר מכל על גובה התלוליות (Cusp height) של השיניים האחוריות בעת שחזור?",
      "options": [
        "א. היקף השחזת קו הסיום בשיניים העליונות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. עובי הרירית בחרך החך בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. סוג הצמנט המשמש להדבקה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. זווית ההדרכה הקונדילרית (Condylar guidance) וההדרכה האינסיזלית (Incisal guidance)."
      ],
      "correct": "ד. זווית ההדרכה הקונדילרית (Condylar guidance) וההדרכה האינסיזלית (Incisal guidance).",
      "explanation": "גובה התלוליות ועומק החריצים בשיניים אחוריות מוכתבים ישירות ע\"י תלוליות ההדרכה הקונדילרית (חלק אחורי) וההדרכה האינסיזלית (חלק קדמי). ככל שההדרכה תלולה יותר, התלוליות יכולות להיות גבוהות יותר.",
      "summary": "גורמי גובה התלוליות: הדרכה קונדילרית והדרכה אינסיזלית תלולות מאפשרות תלוליות גבוהות יותר בשיניים האחוריות ללא הפרעה אקסצנטרית."
    },
    {
      "id": 29,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מה מתרחש בקונדיל בצד הלא-עובד (Non-working side / Mediotrusive) בעת תנועה לטרלית?",
      "options": [
        "א. הקונדיל נע קדימה, למטה ומדיאלית (פנימה).",
        "ב. הקונדיל נע אחורה, למעלה ולטרלית.",
        "ג. הקונדיל יוצא לחלוטין מגבולות הקפסולה המפרקית.",
        "ד. הקונדיל נשאר בעמדת רוטציה טהורה במרכז הפוסה."
      ],
      "correct": "א. הקונדיל נע קדימה, למטה ומדיאלית (פנימה).",
      "explanation": "הקונדיל בצד הלא-עובד (Orbiting / Mediotrusive condyle) נע קדימה, למטה ומדיאלית (פנימה) לאורך ה-Articular eminence והקיר המדיאלי של הפוסה.",
      "summary": "תנועת הקונדיל הלא-עובד (Non-working / Orbiting condyle): נע קדימה, למטה ומדיאלית (Downward, Forward, Medially)."
    },
    {
      "id": 30,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "כיצד משפיע הגדלת ה-Overjet של השיניים הקדמיות על מורפולוגיית התלוליות האחוריות?",
      "options": [
        "א. מחייבת ביטול מוחלט של עקומת Spee. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. דורשת תלוליות אחוריות נמוכות/רדודות יותר (Shorter cusps).",
        "ג. אינה משפיעה כלל על גובה התלוליות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מאפשרת תלוליות אחוריות גבוהות ותלולות יותר."
      ],
      "correct": "ב. דורשת תלוליות אחוריות נמוכות/רדודות יותר (Shorter cusps).",
      "explanation": "ככל שה-Overjet הקדמי גדול יותר, ההדרכה האינסיזלית פחות יעילה בניתוק מיידי של האחוריות, ולכן נדרשות תלוליות אחוריות נמוכות יותר כדי למנוע מגעים מטרידים אקסצנטריים.",
      "summary": "השפעת Overjet קדמי: Overjet גדול מקטין את זווית ההדרכה האינסיזלית ומחייב תלוליות אחוריות נמוכות יותר (Shorter cusps)."
    },
    {
      "id": 31,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מה נכון לגבי ה-Immediate Side Shift (ISS) בתנועת Bennett?",
      "options": [
        "א. זוהי תנועה המבוטלת לחלוטין בארטיקולטור Arcon. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. זוהי תנועה המתרחשת רק בסוף הפרוטרוזיה בפתולוגיות TMJ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. זוהי התנועה הצידית המיידית של המנדיבולה המתרחשת בתחילת התנועה הלטרלית לפני תנועת ההחלקה הקדמית.",
        "ד. תנועה המתרחשת רק במטופלי Class III. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. זוהי התנועה הצידית המיידית של המנדיבולה המתרחשת בתחילת התנועה הלטרלית לפני תנועת ההחלקה הקדמית.",
      "explanation": "Immediate Side Shift (ISS) מוגדרת כהעתקה הצידית המיידית של הלסת התחתונה מדיאלית בצד הלא-עובד בתחילת התנועה הלטרלית. ככל ש-ISS גדול יותר, נדרשות תלוליות נמוכות וחריצים רחבים יותר.",
      "summary": "Immediate Side Shift (ISS): תנועה צידית מיידית בתחילת הגלישה הלטרלית. דורשת תלוליות נמוכות וחריצים אוקלוזליים רחבים יותר."
    },
    {
      "id": 32,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "במצב של Long Centric (Freedom in Centric), מהו טווח התנועה האופקית המופעל בין CR ל-ICP?",
      "options": [
        "א. 2.0 - 4.0 מ\"מ במישור הפרונטלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. 0.0 מ\"מ (מגע נקודתי קשיח בלבד). תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. 3.0 - 5.0 מ\"מ במישור האנכי. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר.",
        "ד. 0.5 - 1.0 מ\"מ במישור האופקי ללא שינוי במימד האנכי."
      ],
      "correct": "ד. 0.5 - 1.0 מ\"מ במישור האופקי ללא שינוי במימד האנכי.",
      "explanation": "Freedom in Centric (Long Centric) מאפשרת למנדיבולה חופש תנועה קדמי-אחורי שטוח של כ-0.5 עד 1.0 מ\"מ בין CR ל-ICP באותו מימד אנכי סגורי.",
      "summary": "Freedom in Centric (Long Centric): חופש תנועה אופקי שטוח של כ-0.5-1.0 מ\"מ בין CR ל-MI באותו מימד אנכי סגורי."
    },
    {
      "id": 33,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מהי התשובה הנכונה לגבי כיוון שבילי הגלישה (Escape grooves) של התלוליות בצד העובד למקסילה ולמנדיבולה?",
      "options": [
        "א. במקסילה השביל מופנה בוקאלית, ובמנדיבולה השביל מופנה לינגואלית.",
        "ב. בשתי הלסתות השביל מופנה מזיאלית בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. בשתי הלסתות השביל מופנה דיסטלית בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. במקסילה השביל מופנה לינגואלית, ובמנדיבולה השביל מופנה בוקאלית."
      ],
      "correct": "א. במקסילה השביל מופנה בוקאלית, ובמנדיבולה השביל מופנה לינגואלית.",
      "explanation": "בתנועה לטרלית בצד העובד, תלולית התמיכה הבלתי-עובדת או העובדת נעה בשביל שחורץ במקסילה לכיוון בוקאלי ובמנדיבולה לכיוון לינגואלי.",
      "summary": "שבילי גלישה בלטרליות (Working ridge/groove): במקסילה מופנה בוקאלית (Transverse lateral path), ובמנדיבולה מופנה לינגואלית."
    },
    {
      "id": 34,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "איזה סוג מגע אוקלוזלי נחשב ליציב ביותר בשיקום קבוע עם Cusp-Fossa Occlusion?",
      "options": [
        "א. מגע בשיפוע התלולית בלבד.",
        "ב. מגע תלת-נקודתי (Tripodization).",
        "ג. מגע נקודתי בלהב האינסיזלי.",
        "ד. מגע שטוח בודד במרכז הפוסה בלבד."
      ],
      "correct": "ב. מגע תלת-נקודתי (Tripodization).",
      "explanation": "מגע תלת-נקודתי (Tripodization) סביב הקודקוד של תלולית התמיכה בתוך הפוסה מעניק יציבות מקסימלית ומכוון את הכוחות לאורך הציר האורך של השן.",
      "summary": "Cusp-Fossa Occlusion: מגע תלת-נקודתי (Tripodization) מעניק יציבות ביומכנית מקסימלית ומכוון כוחות לאורך הציר האורכי של השן."
    },
    {
      "id": 35,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "כיצד משפיע מיקום ציר הסיבוב (Hinge Axis) הקדמי-אחורי בעת שימוש בקשת פנים לא מדויקת על הארטיקולטור?",
      "options": [
        "א. לא תהיה שום השפעה כל עוד הארטיקולטור מסוג Arcon. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. השגיאה משפיעה אך ורק על צילום פנורמי ולא על הארטיקולטור. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. שגיאה במיקום ציר הסיבוב תגרום לשגיאה במיקום המגעים האוקלוזליים בעת שינוי המימד האנכי (VDO).",
        "ד. השגיאה תתבטל מעצמה בעת שימוש בצמנט שרפי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. שגיאה במיקום ציר הסיבוב תגרום לשגיאה במיקום המגעים האוקלוזליים בעת שינוי המימד האנכי (VDO).",
      "explanation": "אם ציר הסיבוב בארטיקולטור אינו חופף לציר הסיבוב האנטומי של המטופל (בשל קשת פנים לא מדויקת), כל שינוי במימד האנכי בארטיקולטור יגרור הסטה אוקלוזלית קדמית/אחורית במציאות.",
      "summary": "דיוק Hinge Axis: אי-חפיפה בין ציר הסיבוב בארטיקולטור לציר האנטומי תגרום לשגיאה במגעים האוקלוזליים אם מבוצע שינוי VDO."
    },
    {
      "id": 36,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מה נכון לגבי ה-Working Side Condyle (הקונדיל בצד העובד)?",
      "options": [
        "א. נקרא Orbiting Condyle והוא נע קדימה ולמטה ב-45 מעלות.",
        "ב. הוא נע תמיד 5 מ\"מ מדיאלית בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. אינו נע כלל ונשאר קפוא לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. נקרא גם ה-Rotating Condyle והוא מבצע בעיקר תנועת סיבוב/העתקה קלה במרחב."
      ],
      "correct": "ד. נקרא גם ה-Rotating Condyle והוא מבצע בעיקר תנועת סיבוב/העתקה קלה במרחב.",
      "explanation": "הקונדיל בצד העובד מוגדר כ-Rotating / Working condyle. הוא מסתובב סביב הציר האנכי ויכול לבצע העתקה קלה (Laterotrusion).",
      "summary": "Working Side Condyle: נקרא Rotating Condyle. מבצע בעיקר רוטציה עם תנועה גופנית קלה (Laterotrusion)."
    },
    {
      "id": 37,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מהי האוריינטציה הראויה של מישור הסגר (Plane of Occlusion) ביחס למישור קמפר (Camper's Line)?",
      "options": [
        "א. מישור הסגר אמור להיות מקביל בקירוב למישור קמפר (Ala-Tragus Line).",
        "ב. אין שום קשר אנטומי ביניהם. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מישור הסגר מקביל למישור ה-Frankfort בלבד ונטוי 45 מעלות לקמפר.",
        "ד. מישור הסגר ניצב ב-90 מעלות למישור קמפר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. מישור הסגר אמור להיות מקביל בקירוב למישור קמפר (Ala-Tragus Line).",
      "explanation": "במישור הסגיטלי, מישור הסגר האנטומי (בעיקר בתותבות שלמות ובשיקום) מתוכנן להיות מקביל למישור קמפר (Camper's Line / Ala-Tragus Line).",
      "summary": "Camper's Line (Ala-Tragus Line): קו ייחוס אנטומי במישור הסגיטלי שאליו משווים ומקבילים את מישור הסגר."
    },
    {
      "id": 38,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "בבדיקת מגעים אוקלוזליים עם נייר אוקלוזלי, מה מצביעה סימניה בצורת טבעת חלולה (Halo / Ring effect)?",
      "options": [
        "א. מגע מרפרף וקל מאוד. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מגע גבוה ומופרז (Heavy / High contact point) שבו החומר נדחק מהמרכז להיקף.",
        "ג. היעדר מגע מוחלט. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מגע שנוצר ע\"י נוזל הרוק בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. מגע גבוה ומופרז (Heavy / High contact point) שבו החומר נדחק מהמרכז להיקף.",
      "explanation": "סימן של טבעת חלולה (Halo effect) ברישום אוקלוזלי נוצר כאשר הכוח האוקלוזלי כה חזק עד שהוא סוחט את הצבע מהמרכז החוצה, ומהווה אינדיקציה למגע גבוה.",
      "summary": "Halo Effect בנייר סגר: טבעת צבע חלולה במרכז מעידה על נקודת מגע גבוהה/מופרזת שסחטה את הדיו מהמרכז."
    },
    {
      "id": 39,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מהי השפעת זווית ההדרכה הקונדילרית (Condylar guidance) תלולה על ה-Overbite הקדמי הנדרש?",
      "options": [
        "א. מחייבת ביטול ה-Overjet הקדמי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. גורמת לספיגה מהירה של הקונדיל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הדרכה קונדילרית תלולה מאפשרת עומק Overbite קדמי גדול יותר ללא חשש ממגעים מטרידים אחוריים.",
        "ד. מחייבת שטוח לחלוטין של ה-Overbite ל-0 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. הדרכה קונדילרית תלולה מאפשרת עומק Overbite קדמי גדול יותר ללא חשש ממגעים מטרידים אחוריים.",
      "explanation": "ככל שההדרכה הקונדילרית תלולה יותר, הנתוק המיידי של השיניים האחוריות בפרוטרוזיה חזק יותר, מה שמאפשר Overbite קדמי עמוק יותר ותלוליות אחוריות גבוהות יותר.",
      "summary": "הדרכה קונדילרית תלולה: מעניקה Disclusion אחורי מהיר, ומאפשרת Overbite קדמי עמוק ותלוליות אחוריות גבוהות."
    },
    {
      "id": 40,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "בינוני",
      "question": "מה מוגדר כ-Protrusive Interocclusal Record (Check bite)?",
      "options": [
        "א. רישום שנלקח בתנועה לטרלית מקסימלית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. רישום לקביעת גודל ה-Free way space. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. רישום שנלקח ב-CR בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. רישום נלקח כאשר הלסת התחתונה מוסטת קדימה 3-5 מ\"מ, ומשמש לכיול זווית ה-Condylar Guidance בארטיקולטור."
      ],
      "correct": "ד. רישום נלקח כאשר הלסת התחתונה מוסטת קדימה 3-5 מ\"מ, ומשמש לכיול זווית ה-Condylar Guidance בארטיקולטור.",
      "explanation": "רישום פרוטרוזיבי נלקח בגלישה קדמית של 3-5 מ\"מ כדי לכייל את זווית ההדרכה הקונדילרית האופקית (Horizontal Condylar Guidance) בארטיקולטור.",
      "summary": "Protrusive Check-Bite: רישום בין-לסתי בגלישה קדמית של 3-5 מ\"מ המשמש לכיול זווית ה-Condylar Guidance בארטיקולטור."
    },
    {
      "id": 41,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מה נכון לגבי תופעת ה-Mandibular Flexure בעת הפעלת כוחות סגירה ולעיסה חזקים?",
      "options": [
        "א. התופעה מופעלת בעיקר ע\"י התכווצות שרירי ה-Medial Pterygoid, וגורמת להתקרבות ענפי המנדיבולה במישור האופקי.",
        "ב. אין לתופעה זו כל השפעה ביומכנית על תכנון שרשראות שתלים או FPD ארוכים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. התופעה גורמת להתרחקות המולרים התחתונים זה מזה בעת פתיחה מקסימלית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. התופעה אינהתרחשת במבוגרים עקב הסתיידות של הסימפיזיס. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. התופעה מופעלת בעיקר ע\"י התכווצות שרירי ה-Medial Pterygoid, וגורמת להתקרבות ענפי המנדיבולה במישור האופקי.",
      "explanation": "לפי מיש ושילינבורג ושחזורי המבחנים, בעת פתיחה וסגירה מאומצת שרירי ה-Medial Pterygoid (וה-Lateral Pterygoid) מפעילים כוח הגורם לכפיפה (Mandibular Flexure) של גוף המנדיבולה פנימה (צמצום המרחק הבין-מולרי עד עשרות/מאות מיקרונים). לתופעה זו משמעות קריטית בתכנון גשרים קשיחים וארוכים על שתלים.",
      "summary": "Mandibular Flexure: בעת פתיחה/סגירה מאומצת שרירי ה-Pterygoid מביאים לכפיפת המנדיבולה פנימה במעט. קריטי בתכנון שיקומים קשיחים ארוכי טווח."
    },
    {
      "id": 42,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "למה מיועד רישום ה-Lateral Interocclusal Record (Check bite) בעבודה עם ארטיקולטור חצי-מתכוונן?",
      "options": [
        "א. לקביעת המימד האנכי של הסגר (VDO) בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. למציאת עמדת הקונדילים בפוסה בעת תנועה צדית לקביעת ה-Condylar guidance והגבולות האנטומיים בארטיקולטור.",
        "ג. להעברת יחס הלסת העליונה לבסיס הגולגולת ביחס למישור אורביטלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. לבדיקת מגעים מטרידים בעמדת Maximum Intercuspation בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. למציאת עמדת הקונדילים בפוסה בעת תנועה צדית לקביעת ה-Condylar guidance והגבולות האנטומיים בארטיקולטור.",
      "explanation": "לפי הספרות העיונית והשחזורים, רישום Lateral check bite נלקח כדי למקם את הקונדילים בתנועה צדית בפוסה, וכך לכייל את זווית ההדרכה הקונדילרית (Condylar guidance angle) וה-Bennett angle בארטיקולטור חצי-מתכוונן.",
      "summary": "Lateral Check-Bite: מיועד לכיול מדויק של ה-Condylar Guidance וה-Bennett Angle בארטיקולטור חצי-מתכוונן בתנועות אקסצנטריות."
    },
    {
      "id": 43,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מה מבין הבאים אינו נחשב לגורם אטיולוגי להיווצרות סגר כפול (Dual Bite)?",
      "options": [
        "א. מחלה דגנרטיבית של מפרק הלסת (כגון Rheumatoid Arthritis).",
        "ב. טיפול אורתודונטי שאינו מלא או בלתי מספק.",
        "ג. סגר אנטומי מסוג Class III קלאסי תורשתי.",
        "ד. תנועה הרגלית פרוטרוזיבית כרונית (כגון Sunday Bite במטופלי Class II div 1)."
      ],
      "correct": "ג. סגר אנטומי מסוג Class III קלאסי תורשתי.",
      "explanation": "Dual Bite מוגדר כהפרש של מעבר מ-ICP ל-RCP הגדול מ-2 מ\"מ (קיים ב-1-3% באוכלוסיה). הגורמים המוכרים בספרות (Egermack-Eriksson / שחזורי 2018-2020): תנועה הרגלית פרוטרוזיבית (Sunday bite), טיפול אורתו לא מלא, שבר דו-צדדי במפרק, ומחלות דגנרטיביות. Class III אנטומי אינו גורם ל-Dual bite.",
      "summary": "Dual Bite (סגר כפול): הפרש מורחב (>2 מ\"מ) בין RCP ל-ICP. נגרם מהרגל (Sunday bite), טראומה/שבר, אורתודונטיה לקויה או מחלת מפרק, אך לא מ-Class III תורשתי."
    },
    {
      "id": 44,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "במטופל עם סגר Class 2 שבו בוצעה הרמת מנשך (הגדלת VDO), המשנן התחתון זז דיסטלית ביחס לעליון. מה עלינו לעשות כדי לשחזר מגעים קדמיים ב-MI?",
      "options": [
        "א. להוריד לחלוטין את התלוליות הבוקאליות במלוא המשנן התחתון.",
        "ב. להאריך את החותכות התחתונות בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. להטות את החותכות התחתונות לינגואלית ב-15 מעלות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. להוסיף למתאר הפלטינלי של הקדמיות העליונות ולייצר פלטפורמה (Palatal Platform)."
      ],
      "correct": "ד. להוסיף למתאר הפלטינלי של הקדמיות העליונות ולייצר פלטפורמה (Palatal Platform).",
      "explanation": "לפי שחזורי המבחנים (2024, 2012), כאשר מגדילים מימד אנכי במטופל Class 2, הקונדילים מסתובבים והמנדיבולה נעה אחורנית (דיסטלית) ביחס למקסילה. כדי ליצור מגעים קדמיים ב-MI יש להוסיף נפח למתאר הפלטינלי של הקדמיות העליונות (Building a palatal ledge/platform).",
      "summary": "הרמת מנשך ב-Class II: הסטת המנדיבולה דיסטלית בעת הגדלת VDO מחייבת עיצוב פלטפורמה פלטינלית (Palatal Platform) בקדמיות העליונות לשחזור מגעים."
    },
    {
      "id": 45,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מה נכון לגבי המרחק הבין-קונדילרי (Intercondylar distance) בארטיקולטורים סמי-אדג'סטבל סטנדרטיים?",
      "options": [
        "א. בארטיקולטורים ממוצעים המרחק נקבע לכ-110 מ\"מ (בעוד שבגולגולת האדם הוא משתנה בין אינדיבידואלים).",
        "ב. הוא קובע באופן ישיר את עקומת Spee במקביל לקשת הפנים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הוא נקבע ל-90 מ\"מ קבוע מראש בכל הדגמים. בהתאם להנחיות הכלליות המופיעות בספרות הלימוד הדנטלית.",
        "ד. ככל שהמרחק הבין-קונדילרי קטן יותר, הזווית בין שבילי הגלישה הבוקאליים והלינגואליים גדלה."
      ],
      "correct": "א. בארטיקולטורים ממוצעים המרחק נקבע לכ-110 מ\"מ (בעוד שבגולגולת האדם הוא משתנה בין אינדיבידואלים).",
      "explanation": "לפי הספרות (שילינבורג / מול) ושחזורי המבחנים (2016–2024), המרחק הבין-קונדילרי הממוצע בארטיקולטורים חצי-מתכווננים הוא 110 מ\"מ. באדם המרחק הוא אינדיבידואלי ומשתנה.",
      "summary": "Intercondylar Distance: בארטיקולטורים חצי-מתכווננים נקבע לממוצע של 110 מ\"מ. משפיע על כיוון החריצים והתלוליות בתנועות אקסצנטריות."
    },
    {
      "id": 46,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מהי ההשפעה של הגדלת ה-Intercondylar distance בארטיקולטור על הזווית בין שבילי הגלישה (Protrusive/Lateral grooves) בשיניים האחוריות?",
      "options": [
        "א. החריצים הופכים למקבילים לחלוטין למישור הסגר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הזווית בין השביל הבוקאלי והלינגואלי (Groove direction) הופכת לקטנה יותר (More acute/narrower).",
        "ג. אין שום השפעה על כיוון החריצים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הזווית הופכת לרחבה יותר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. הזווית בין השביל הבוקאלי והלינגואלי (Groove direction) הופכת לקטנה יותר (More acute/narrower).",
      "explanation": "ככל שהמרחק הבין-קונדילרי גדול יותר, הזווית שנוצרת בין שבילי הגלישה הבוקאליים והלינגואליים בשיניים האחוריות הופכת לקטנה/חדה יותר.",
      "summary": "מרחק בין-קונדילרי וזווית חריצים: מרחק בין-קונדילרי גדול יותר מקטין את הזווית בין שבילי הגלישה האקסצנטריים בשיניים האחוריות."
    },
    {
      "id": 47,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מה קורה ב-Gothic Arch Tracer (Pantomographic recording) בשיא הזווית (Apex of the Gothic Arch)?",
      "options": [
        "א. שיא הזווית מייצג את עמדת הפתיחה המקסימלית.",
        "ב. שיא הזווית מייצג את ה-Rest position.",
        "ג. שיא הזווית מייצג את עמדת ה-Centric Relation (CR).",
        "ד. שיא הזווית מייצג את עמדת ה-Maximum Protrusion."
      ],
      "correct": "ג. שיא הזווית מייצג את עמדת ה-Centric Relation (CR).",
      "explanation": "בגות'יק ארץ' טרייסר (Gothic Arch / Arrowhead tracing), קודקוד החץ (Apex) מייצג במודל הדינמי את עמדת ה-Centric Relation האנטומית.",
      "summary": "Gothic Arch Tracing: קודקוד החץ (Apex) בנתיב הרישום האופקי מייצג את עמדת Centric Relation (CR) המדויקת."
    },
    {
      "id": 48,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "בניתוח מגעים אוקלוזליים, מהו ה-Non-Working Side Interference (הפרעה בצד הלא-עובד)?",
      "options": [
        "א. מגע המתרחש בעמדת מנוחה במשור הפרונטלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. ניתוק מלא של כל המשננים בתנועה פרוטרוזיבית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מגע המתרחש בין הניבים בצד העובד בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. מגע המתרחש בין התלוליות התומכות (Palatal עליונה / Buccal תחתונה) בצד הלא-עובד בעת תנועה לטרלית, המונע מגע בצד העובד."
      ],
      "correct": "ד. מגע המתרחש בין התלוליות התומכות (Palatal עליונה / Buccal תחתונה) בצד הלא-עובד בעת תנועה לטרלית, המונע מגע בצד העובד.",
      "explanation": "הפרעה בצד הלא-עובד נוצרת בין התלוליות המרכזיות התומכות (Palatal עליונה ו-Buccal תחתונה) בצד המדיוטרוסיבי. נחשבת להפרעה המזיקה ביותר למערכת הלעיסה וה-TMJ.",
      "summary": "הפרעה בצד הלא-עובד (Non-working interference): מגע הרסני בין תלוליות תמיכה בצד המדיוטרוסיבי המונע מגע תקין בצד העובד."
    },
    {
      "id": 49,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "איזו רקמה מבין הבאים ב-TMJ מציגה התנהגות ויסקואלסטית (Viscoelastic behavior) בולטת בעת ספיגת עומסים?",
      "options": [
        "א. הדיסק הארטיקולרי והרקמה הרכה של ה-Retrodiscal pad.",
        "ב. הסחוס ההיאליני של ה-Tympanic plate בלבד.",
        "ג. השריר הדיגסטרי בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. העצם הקורטיקלית של ה-Condylar head בלבד."
      ],
      "correct": "א. הדיסק הארטיקולרי והרקמה הרכה של ה-Retrodiscal pad.",
      "explanation": "הדיסק הארטיקולרי וה-Retrodiscal tissue מורכבים מסיבי קולגן, פרוטאוגליקנים ומים, ומציגים התנהגות ויסקואלסטית שתלויה בזמן ובקצב הפעלת העומס.",
      "summary": "התנהגות ויסקואלסטית ב-TMJ: הדיסק הארטיקולרי והרקמה הרכה בולמים עומסים ומגיבים בדפורמציה תלוית זמן וקצב העמסה."
    },
    {
      "id": 50,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "בעת הגדלת המימד האנכי (VDO) במטופל Class III, כיצד נעה המנדיבולה ביחס למקסילה בסגירה הראשונית?",
      "options": [
        "א. המנדיבולה זזה ימינה בלבד ב-45 מעלות. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. המנדיבולה נעה אחורנית (דיסטלית) ולמטה, מה שמקל על השגת יחס קדמי תקין.",
        "ג. אין שום שינוי במיקום המנדיבולה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. המנדיבולה בולטת קדימה (פרוטרוזיבית) עוד יותר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. המנדיבולה נעה אחורנית (דיסטלית) ולמטה, מה שמקל על השגת יחס קדמי תקין.",
      "explanation": "בעת סיבוב המנדיבולה סביב ה-Hinge axis להגדלת VDO, גוף המנדיבולה זז אחורנית ולמטה ביחס למקסילה, מה שמסייע בשיקום מטופלי Class III לקבלת יחס קדמי תקין.",
      "summary": "שינוי VDO ב-Class III: פתיחת המימד האנכי גורמת להסטה דיסטלית של המנדיבולה, מה שמקל על שיפור היחס הקדמי והסגר."
    },
    {
      "id": 51,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מהי המשמעות של A-B-C Contacts בסגר אוקלוזלי יציב לפי שילינבורג?",
      "options": [
        "א. מגעים אופקיים בלבד בחלק הקדמי. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. מגעים שנוצרים אך ורק בחומרי מטבע אלסטומריים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מגע A בין תלולית גזירה עליונה לתמיכה תחתונה, B בין תלוליות תמיכה, C בין תמיכה עליונה לגזירה תחתונה.",
        "ד. מגעים המופיעים בעמדת Rest בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. מגע A בין תלולית גזירה עליונה לתמיכה תחתונה, B בין תלוליות תמיכה, C בין תמיכה עליונה לגזירה תחתונה.",
      "explanation": "לפי שילינבורג, מגעים A, B, C מבטיחים יציבות רוחבית. מגע B (בין תלוליות התמיכה הפלטינלית העליונה והבוקאלית התחתונה) הוא הקריטי ביותר למניעת כוחות הטיות.",
      "summary": "מגעי A-B-C (Shillingburg): מגע B (בין תלוליות התמיכה העליונה והתחתונה) הוא הקריטי ביותר למניעת כוחות הטיות רוחביים."
    },
    {
      "id": 52,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מהו ה-Envelope of Function להבדיל מ-Envelope of Motion של Posselt?",
      "options": [
        "א. נתיב התנועה של הקונדיל בתוך ה-Submandibular space. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מעטפת התנועה הנמדדת אך ורק בארטיקולטור Non-Arcon. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מעטפת הגבולות הגרמיים המקסימליים של המפרק ללא שרירים. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. מעטפת התנועה הפונקציונלית הדינמית (בזמן לעיסה ודיבור) המתרחשת כולה בתוך הגבולות האנטומיים הקיצוניים של Posselt."
      ],
      "correct": "ד. מעטפת התנועה הפונקציונלית הדינמית (בזמן לעיסה ודיבור) המתרחשת כולה בתוך הגבולות האנטומיים הקיצוניים של Posselt.",
      "explanation": "Envelope of Function מייצגת את התנועה הפונקציונלית בפועל (לעיסה, דיבור) והיא פנימית לגבולות האנטומיים הקיצוניים (Envelope of Motion) שמוגדרים ע\"י מפרק ורצועות.",
      "summary": "Envelope of Function: מעטפת התנועה התפקודית (לעיסה/דיבור), המוכלת במלואה בתוך מעטפת הגבולות האנטומיים (Posselt's Envelope of Motion)."
    },
    {
      "id": 53,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "איזה חלק בדיסק הארטיקולרי מעוצב בצורה הקעורה ביותר כדי להתאים לראש הקונדיל?",
      "options": [
        "א. המשטח התחתון (Inferior surface) של הדיסק.",
        "ב. ה-Anterior band. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. ה-Retrodiscal pad האחורי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. המשטח העליון (Superior surface) של הדיסק."
      ],
      "correct": "א. המשטח התחתון (Inferior surface) של הדיסק.",
      "explanation": "המשטח התחתון של הדיסק קעור (Concave) כדי להתאים באופן מדויק למשטח הקמור של ראש הקונדיל. המשטח העליון קעור-קמור כדי להתאים לפוסה ול-Eminence.",
      "summary": "מורפולוגיית הדיסק הארטיקולרי: משטח תחתון קעור להתאמה לקונדיל; משטח עליון קעור-קמור להתאמה ל-Glenoid fossa ו-Articular eminence."
    },
    {
      "id": 54,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מה מתרחש ב-TMJ בעת פתיחת פה במצב של Disc Displacement WITHOUT Reduction?",
      "options": [
        "א. הדיסק חוזר למקומו עם הקלקה עזה בפתח 40 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הדיסק נשאר מועתק קדימה, הקונדיל אינו מצליח לעלות עליו, ונוצרת הגבלה חמורה בפתיחה (Closed Lock) ללא הקלקה.",
        "ג. הפתיחה גדלה ל-60 מ\"מ ללא שום הגבלה. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. המפרק ענטק ממקומו בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. הדיסק נשאר מועתק קדימה, הקונדיל אינו מצליח לעלות עליו, ונוצרת הגבלה חמורה בפתיחה (Closed Lock) ללא הקלקה.",
      "explanation": "ב-Disc Displacement WITHOUT Reduction, הדיסק אינו חוזר למקומו, מונע את הטרנסלציה הקדמית של הקונדיל וגורם לנעילה סגורה (Closed Lock) ולסטיית הלסת לצד הפגוע.",
      "summary": "Disc Displacement WITHOUT Reduction: הדיסק תקוע אנטריורית, הקונדיל אינו יכול להחליק קדימה, ונוצרת נעילה סגורה (Closed Lock) עם סטייה לצד הפגוע."
    },
    {
      "id": 55,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מהו ה-Shift Angle לפי Dawson בהגדרת התנועות האוקלוזליות?",
      "options": [
        "א. הסטת קו האמצע בלסת העליונה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. זווית השחזת השורש בטיפול פריודונטלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הזווית הנסגרת בין מסלול ה-CR ל-ICP בעת מגע מסיטי (Deflective contact).",
        "ד. הסטת הניב העליון בתנועה פרוטרוזיבית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. הזווית הנסגרת בין מסלול ה-CR ל-ICP בעת מגע מסיטי (Deflective contact).",
      "explanation": "לפי דוסון, מגע מסיטי ב-CR גורם להחלקה (Slide from CR to ICP) בזווית מסוימת שיכולה להיות אנטרו-סופריורית או לטרלית.",
      "summary": "Slide from CR to ICP: החלקה הנגרמת ע\"י מגע מסיטי ראשוני (Deflective contact) המסיטה את הלסת מ-CR ל-ICP."
    },
    {
      "id": 56,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "איזה מרכיב במערכת העצבים מעורב בבקרת רפלקס הפתיחה המהיר בעת נשיכה פתאומית על גוף קשה (Nociceptive reflex)?",
      "options": [
        "א. המערכת המרכזית במוח הקטן ללא רפלקס היקפי. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ב. קולטנים ברירית הבלע בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. עצב ה-Facial בגרעין המוטורי בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. קולטנים פריודונטליים (Periodontal mechanoreceptors) המעבירים אות לגרעין המזאנצפלי של עצב V."
      ],
      "correct": "ד. קולטנים פריודונטליים (Periodontal mechanoreceptors) המעבירים אות לגרעין המזאנצפלי של עצב V.",
      "explanation": "מכנורצפטורים ב-PDL מזהים עומס חריג ומפעילים רפלקס נורו-מוסקורלי מהיר הגורם להרפיית שרירי הסגירה והפעלת שרירי הפתיחה להגנה על השיניים.",
      "summary": "רפלקס ההגנה האוקלוזלי (Nociceptive reflex): מכנורצפטורים ב-PDL מפעילים הרפיה מיידית של שרירי הסגירה בעת עומס פתאומי קשה להגנה על המשנן."
    },
    {
      "id": 57,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מהי זווית Fischer (Fischer's Angle) בארטיקולציה דינמית?",
      "options": [
        "א. הזווית שבין מסלול הקונדיל בתנועה פרוטרוזיבית לבין מסלולו בצד הלא-עובד (Mediotrusive path) במישור הסגיטלי.",
        "ב. זווית השחזת קו הסיום בכתר זירקוניה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הזווית בין שני הניבים העליונים במישור הפרונטלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הזווית הנוצרת בין קשת הפנים למישור אורביטלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. הזווית שבין מסלול הקונדיל בתנועה פרוטרוזיבית לבין מסלולו בצד הלא-עובד (Mediotrusive path) במישור הסגיטלי.",
      "explanation": "זווית פישר מוגדרת כזווית במישור הסגיטלי בין מסלול ה-Protrusive path לבין ה-Mediotrusive path של הקונדיל בצד הלא-עובד.",
      "summary": "Fischer's Angle: הזווית במישור הסגיטלי בין נתיב הגלישה הפרוטרוזיבי לנתיב המדיוטרוסיבי (Non-working) של הקונדיל."
    },
    {
      "id": 58,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "בניתוח ביומכני, מדוע כוחות אופקיים (Lateral forces) הרסניים יותר למנגנון התמיכה הפריודונטי מאשר כוחות אנכיים (Axial forces)?",
      "options": [
        "א. מכיוון שאינם מפעילים את ה-Masseter. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מכיוון שסיבי ה-PDL (בעיקר ה-Oblique fibers) מיועדים לספוג כוחות מתיחה מאונכים ולא כוחות גזירה והטיה.",
        "ג. מכיוון שהעצם הקורטיקלית דקה יותר בחלק האפיקלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מכיוון שכוחות אופקיים מבטלים את אספקת הדם לשן השכנה. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר."
      ],
      "correct": "ב. מכיוון שסיבי ה-PDL (בעיקר ה-Oblique fibers) מיועדים לספוג כוחות מתיחה מאונכים ולא כוחות גזירה והטיה.",
      "explanation": "סיבי ה-PDL האלכסוניים (Oblique fibers) ממירים כוח דחיסה אנכי למתיחה על העצם, בעוד כוחות אופקיים מייצרים מומנט פיתול וכוחות לחיצה מרוכזים באזור הפלקרום והאפקס.",
      "summary": "ספיגת כוחות ב-PDL: סיבי ה-PDL האלכסוניים מתוכננים לספוג כוחות צדדיים אנכיים (Axial). כוחות אופקיים מייצרים מומנט פיתול הרסני."
    },
    {
      "id": 59,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "כיצד משפיע שימוש ב-Fully Adjustable Articulator לעומת Semi-Adjustable על תכנון משנן מורכב?",
      "options": [
        "א. אינו דורש לקיחת קשת פנים כלל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מקצר את זמן העבודה במעבדה ל-5 דקות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מאפשר לשחזר במדויק את נתיבי ה-Side Shift הדינמיים, זווית Fischer וקעירות הפוסה (Custom Pantographic tracing).",
        "ד. מבטל את הצורך ברישום Centric Relation. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. מאפשר לשחזר במדויק את נתיבי ה-Side Shift הדינמיים, זווית Fischer וקעירות הפוסה (Custom Pantographic tracing).",
      "explanation": "ארטיקולטור מתכוונן לחלוטין (Fully adjustable) מבוסס על רישום פנטוגרפי ומאפשר התאמה מלאה של קימורי הפוסה ונתיבי הגלישה הדינמיים של המטופל.",
      "summary": "Fully Adjustable Articulator: מאפשר כיול תלת-ממדי מותאם אישית של קימורי הפוסה ונתיבי הגלישה הדינמיים (מבוסס רישום פנטוגרפי)."
    },
    {
      "id": 60,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "קשה",
      "question": "מהי ההגדרה המדויקת של Functional Outer Aspect (FOA) של התלוליות?",
      "options": [
        "א. השטח האינסיזלי של הניבים התחתונים בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. קו החניכיים הזוויתי בשן טחנת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. השיפוע הפנימי של התלוליות הבלתי-תומכות בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. השיפוע החיצוני של תלוליות התמיכה (Buccal בתחתונות / Palatal בעליונות) המשתתף במגעים אוקלוזליים בסגירה וגלישה."
      ],
      "correct": "ד. השיפוע החיצוני של תלוליות התמיכה (Buccal בתחתונות / Palatal בעליונות) המשתתף במגעים אוקלוזליים בסגירה וגלישה.",
      "explanation": "FOA (Functional Outer Aspect) הוא השיפוע החיצוני ברוחב כ-1 מ\"מ של תלולית התמיכה (Stamp cusp) הנכנס במגע בסגירה פונקציונלית.",
      "summary": "FOA (Functional Outer Aspect): השיפוע החיצוני של תלולית התמיכה ברוחב כ-1 מ\"מ המעורב במגעים פונקציונליים."
    },
    {
      "id": 61,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "על פי הספרות העיונית (שילינבורג / מול) והשחזורים, ממה נובעת תחילת התנועה הצדית בצד העובד (Working Side) לעומת הצד הלא-עובד (Non-Working Side)?",
      "options": [
        "א. התחלת התנועה בצד העובד נובעת מהשפעת ה-Temporomandibular Ligament, ובצד הלא-עובד מהקיר המדיאלי של ה-Glenoid Fossa.",
        "ב. הדיסק הארטיקולרי ננעל לחלוטין בצד הלא-עובד ומונע גלישה מדיאלית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. התחלת התנועה בצד העובד מוכתבת אך ורק ע\"י התכווצות שריר ה-Masseter בצד הנגדי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הקיר הלטרלי של ה-Glenoid fossa בצד העובד הוא הגורם היחיד המכתיב את ה-Progressive side shift."
      ],
      "correct": "א. התחלת התנועה בצד העובד נובעת מהשפעת ה-Temporomandibular Ligament, ובצד הלא-עובד מהקיר המדיאלי של ה-Glenoid Fossa.",
      "explanation": "לפי שחזורי המבחנים (2018, 2019) וציטוטי הלימוד מהספרות, תחילת תנועת ה-Side shift בצד העובד מושפעת ומרוסנת ע\"י ה-Temporomandibular ligament בצד העובד, ובצד הלא-עובד היא מותנית במבנה הקיר המדיאלי של ה-Glenoid fossa.",
      "summary": "ריסון תנועה לטרלית: בצד העובד מרוסנת ע\"י ה-Temporomandibular ligament, ובצד הלא-עובד מוכתבת ע\"י הקיר המדיאלי של ה-Glenoid fossa."
    },
    {
      "id": 62,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהו ההבדל המדויק בין המושגים Condylar Angulation לבין Condylar Inclination בספרות הדנטלית ושחזורי המבחנים?",
      "options": [
        "א. הראשון מוגדר רק בארטיקולטור Non-Arcon והשני מוגדר רק בארטיקולטור Arcon. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הראשון (Condylar Angulation) נמדד בצד הלא-עובד במישור הוריזונטלי (מקורו בתנועת Bennett), והשני (Condylar Inclination) נמדד בפרוטרוזיה במישור פרה-סגיטלי.",
        "ג. אין שום הבדל ביניהם – מדובר בשמות נרדפים לאותה זווית הדרכה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הראשון נמדד בתנועה פונקציונלית של סגירה בלבד, והשני נמדד בעת מנוחה (VDR). תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. הראשון (Condylar Angulation) נמדד בצד הלא-עובד במישור הוריזונטלי (מקורו בתנועת Bennett), והשני (Condylar Inclination) נמדד בפרוטרוזיה במישור פרה-סגיטלי.",
      "explanation": "לפי שחזורי המבחנים (2016-2019): Condylar Angulation מקורו בתנועת Bennett ונמדד בצד הלא עובד במישור הוריזונטלי (או זווית Bennett), בעוד Condylar Inclination נמדד בעת גלישה קדימה (פרוטרוזיה) במישור פרה-סגיטלי (זווית ההדרכה הקונדילרית).",
      "summary": "Condylar Angulation vs Inclination: Angulation היא זווית Bennett במישור האופקי בצד הלא-עובד. Inclination היא זווית ההדרכה הקונדילרית במישור הסגיטלי בפרוטרוזיה."
    },
    {
      "id": 63,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהי המשמעות הביומכנית המלאה של סגר מוגן הדדית (Mutually Protected Occlusion / Organic Occlusion)?",
      "options": [
        "א. השיניים הקדמיות והאחוריות עומדות במגע הדוק והרמוני בכל תנועות הגלישה הלטרליות והפרוטרוזיביות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הניבים בלבד נושאים בכל עומס הסגירה ב-MI ללא מגע בטוחנות. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. השיניים האחוריות מגינות על הקדמיות בעמדת MI (סופגות את הכוחות המאונכים), והשיניים הקדמיות מפרידות (Disclude) את האחוריות בתנועות אקסצנטריות.",
        "ד. מגע רציף של כל התלוליות בצד העובד והלא-עובד בעת ובעונה אחת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. השיניים האחוריות מגינות על הקדמיות בעמדת MI (סופגות את הכוחות המאונכים), והשיניים הקדמיות מפרידות (Disclude) את האחוריות בתנועות אקסצנטריות.",
      "explanation": "סגר מוגן הדדית (Mutually Protected Occlusion) מתבסס על כך שבסגירה מרכזית (MI) השיניים האחוריות סופגות את העומס האנכי ומגינות על הקדמיות (מגע קל/מרפרף בקדמיות), ואילו בגלישה אקסצנטרית (לטרלית/פרוטרוזיה) השיניים הקדמיות/ניבים מנחות ומפרידות (Disclude) את האחוריות.",
      "summary": "Mutually Protected Occlusion: ב-MI האחוריות סופגות עומס אנכי ומגינות על הקדמיות; בגלישה אקסצנטרית הקדמיות מגינות על האחוריות ע\"י Disclusion."
    },
    {
      "id": 64,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "משוואת Hanau (Hanau's Quint) מגדירה 5 פרמטרים ליצירת Balanced Occlusion. איזה מהגורמים הבאים נמצא בשליטתו הישירה של רופא השיניים בתכנון המשנן?",
      "options": [
        "א. Condylar Guidance (זווית ההדרכה הקונדילרית במפרק).",
        "ב. האנטומיה הגרמית של ה-Glenoid Fossa. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. קצב ספיגת העצם האלבאולרית ברכס השארי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Incisal Guidance, Cusp Height, ו-Plane of Occlusion / Curve of Spee."
      ],
      "correct": "ד. Incisal Guidance, Cusp Height, ו-Plane of Occlusion / Curve of Spee.",
      "explanation": "במשוואת Hanau (המחברת בין Condylar Guidance, Incisal Guidance, Cusp Height, Plane of Occlusion, ו-Curve of Spee/Compensation), ה-Condylar Guidance נקבעת ע\"י אנטומיית המטופל ואינה נשלטת ע\"י הרופא. לעומת זאת, גובה התלוליות, ההדרכה האינסיזלית, ומישור הסגר/עקומות הפיצוי נשלטים ע\"י הקלינאי.",
      "summary": "Hanau's Quint: הדרכה קונדילרית נקבעת ע\"י אנטומיית המטופל (קבועה). הדרכה אינסיזלית, גובה תלוליות, מישור סגר ועקומת פיצוי נשלטים ע\"י הרופא."
    },
    {
      "id": 65,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מה יתרחש ברירית החך/הרכס (Mucosa) תחת עומס לחיצה מודרני מתמיד של 10 דקות במטופל מבוגר (Viscoelastic behavior)?",
      "options": [
        "א. בהתחלה דחיסה אלסטית מהירה ולאחר מכן דפורמציה אלסטית איטית, כאשר החזרה למימדים המקוריים עשויה להימשך עד 4 שעות.",
        "ב. דחיסה פלסטית מהירה בשתי הדקות הראשונות בלבד ולאחריה עצירה מוחלטת. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. דפורמציה פלסטית בלתי הפיכה באופן מיידי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הרחבה אלסטית מיידית של הרקמה התת-רירית ללא השפעת זמן. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "א. בהתחלה דחיסה אלסטית מהירה ולאחר מכן דפורמציה אלסטית איטית, כאשר החזרה למימדים המקוריים עשויה להימשך עד 4 שעות.",
      "explanation": "לפי Boucher ושחזורי המבחנים (2016), ההתנהגות הוויסקואלסטית של הרקמה הרכה תחת עומס של 10 דקות כוללת התכווצות אלסטית מהירה בהתחלה ודפורמציה איטית בהמשך. החזרה המלאה למימדים המקוריים במטופלים מבוגרים עשויה להימשך עד 4 שעות.",
      "summary": "ויסקואלסטיות הרירית תחת עומס: דחיסה מהירה ראשונית ודפורמציה איטית נמשכת. החזרה למצב המוצא במבוגרים עשויה להימשך עד 4 שעות."
    },
    {
      "id": 66,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מה קורה ל-Incisal Guidance Table המתכוונן בארטיקולטור בעת שחזור קדמי מורכב?",
      "options": [
        "א. משמש אך ורק בעבודה עם סורק סורק תוך אוראלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מותאם אישית ע\"י אקריל כדי לשקף ולשמר את ההדרכה הקדמית הפיזיולוגית והאסתטית של המטופל (Custom Incisal Guide Table).",
        "ג. נקבע תמיד ל-0 מעלות לביטול כוחות גזירה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. אינו משפיע כלל על תנועות הלסת האקסצנטריות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. מותאם אישית ע\"י אקריל כדי לשקף ולשמר את ההדרכה הקדמית הפיזיולוגית והאסתטית של המטופל (Custom Incisal Guide Table).",
      "explanation": "Custom Incisal Guide Table עשוי משרף אקרילי משמש להעתקת ההדרכה הקדמית הפיזיולוגית הקיימת או המתוכננת של המטופל למעבדה כדי לשמר אותה בשחזור הסופי.",
      "summary": "Custom Incisal Guide Table: שולחן אקרילי המעתיק במדויק את ההדרכה הקדמית של המטופל לשימור התנועה הפונקציונלית והאסתטית בשחזורים קדמיים."
    },
    {
      "id": 67,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "איזה סוג קולגן מהווה את המרכיב העיקרי של הדיסק הארטיקולרי במפרק ה-TMJ?",
      "options": [
        "א. Type II Collagen בלבד המאפיין סחוס היאליני. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. Type X Collagen בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. Type I Collagen (סיבי קולגן צפופים המאורגנים בהתאם לכיווני העומס).",
        "ד. Type IV Collagen המאפיין ממברנת בסיס בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. Type I Collagen (סיבי קולגן צפופים המאורגנים בהתאם לכיווני העומס).",
      "explanation": "הדיסק הארטיקולרי מורכב בעיקרו מ-Type I Collagen (ולא Type II סחוקי היאליני), מה שמעניק לו עמידות גבוהה לכוחות מתיחה וגזירה.",
      "summary": "הרכב הדיסק ב-TMJ: עשוי מ-Dense Fibrous Connective Tissue המורכב בעיקר מ-Type I Collagen מעורב עם כמות קטנה של כונדרוציטים."
    },
    {
      "id": 68,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהי ההשפעה של Laterotrusion (תנועת הקונדיל בצד העובד) המלווה ברכיב סופריורי (Upward movement) על גובה התלוליות האחוריות?",
      "options": [
        "א. אינה משפיעה כלל עקב הגנת הניב. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מאפשרת תלוליות גבוהות ותלולות ב-20%. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מבטלת לחלוטין את ה-Bennett angle. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מחייבת תלוליות אחוריות נמוכות/רדודות עוד יותר עקב הסיכון המוגבר למגע מסיטי."
      ],
      "correct": "ד. מחייבת תלוליות אחוריות נמוכות/רדודות עוד יותר עקב הסיכון המוגבר למגע מסיטי.",
      "explanation": "אם הקונדיל בצד העובד עולה למעלה בעת תנועה לטרלית (Laterosuperotrusive), המרווח האוקלוזלי קטן, ולכן נדרשות תלוליות אחוריות נמוכות יותר למניעת התנגשות.",
      "summary": "Laterosuperotrusive Movement: תנועת קונדיל עובד כלפי מעלה מקטינה את המרווח הבין-אוקלוזלי ומחייבת תלוליות אחוריות נמוכות."
    },
    {
      "id": 69,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "בניתוח אלקטרומיוגרפי (EMG) של שרירי הלעיסה, מה מתרחש בפעילות שרירי ה-Elevators בעת ניתוק מגעים אחוריים (Disclusion) ע\"י הניבים?",
      "options": [
        "א. ירידה דרמטית ואינהיביציה (Inhibition) של הפעילות החשמלית בשרירי ה-Masseter וה-Temporalis.",
        "ב. עלייה חדה ועוררות של 200% בפעילות שריר ה-Masseter. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הפעלה מועצמת של ה-Medial Pterygoid בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. היעדר כל שינוי בפעילות החשמלית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. ירידה דרמטית ואינהיביציה (Inhibition) של הפעילות החשמלית בשרירי ה-Masseter וה-Temporalis.",
      "explanation": "ניתוק מגע אחורי (Disclusion) ע\"י הניבים מפעיל מנגנון נוירולוגי המדכא (Inhibits) את הפעילות השרירית של ה-Elevators (Masseter & Temporalis) ומפחית עומסים מפרקיים.",
      "summary": "אינהיביציה שרירית ע\"י Disclusion: ניתוק השיניים האחוריות בגלישה מוריד רפלקסיבית את הפעילות החשמלית (EMG) של שרירי הסגירה."
    },
    {
      "id": 70,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהו ה-Centric Relation Record המדויק ביותר במטופל מחוסר שיניים או בעל הפרעות מוסקולריות לפי הספרות?",
      "options": [
        "א. רישום הנלקח בעמידה בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. רישום מונחה באמצעות Lucia Jig / Deprogrammer או Bilateral Manipulation לפי Dawson.",
        "ג. דחיפת הלסת לאחור באופן כוחני ע\"י המטופל בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. רישום תוך כדי לכיסת חומר מטבע אלגינצי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. רישום מונחה באמצעות Lucia Jig / Deprogrammer או Bilateral Manipulation לפי Dawson.",
      "explanation": "שימוש ב-Lucia Jig או Deprogrammer מנטרל את הזיכרון השרירי (Proprioceptive memory) ומאפשר הובלה מדויקת ל-CR באמצעות Dawson Bilateral Manipulation.",
      "summary": "Deprogramming & CR Record: שימוש ב-Lucia Jig מנטרל זיכרון תחושתי-שרירי ומאפשר רישום CR מפרקי מדויק ללא הטיות דנטליות."
    },
    {
      "id": 71,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מה נכון לגבי ה-Superior Retrodiscal Lamina (השכבה העליונה של ה-Retrodiscal pad)?",
      "options": [
        "א. אחראית על דחיפת הקונדיל מדיאלית בעת סגירה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מחוברת לראש הניב בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. עשירה בסיבים אלסטיים (Elastic fibers) ומחוברת ל-Tympanic plate, ואחראית למשוך את הדיסק אחורנית בסוף פתיחת הפה.",
        "ד. עשויה מסחוס היאליני קשיח חסר תנועה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. עשירה בסיבים אלסטיים (Elastic fibers) ומחוברת ל-Tympanic plate, ואחראית למשוך את הדיסק אחורנית בסוף פתיחת הפה.",
      "explanation": "ה-Superior Retrodiscal Lamina מכילה סיבים אלסטיים המותחים את הדיסק אחורנית בעת סגירת הפה ומייצבים אותו מעל הקונדיל.",
      "summary": "Superior Retrodiscal Lamina: עשירה בסיבים אלסטיים; מושכת ומייצבת את הדיסק הארטיקולרי אחורנית בעת סגירת הפה."
    },
    {
      "id": 72,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "כיצד מוגדרת זווית ה-Condylar Guidance Angle ביחס ל-Frankfort Horizontal Plane?",
      "options": [
        "א. הזווית הניצבת למישור המדיאלי ב-90 מעלות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הזווית שבין הניב העליון לחותכת התחתונה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. זווית השווה תמיד ל-0 מעלות בכל האוכלוסיה. במקביל להתפלגות הכוחות הווקטוריים במישור הסגיטלי והפרונטלי.",
        "ד. הזווית במישור הסגיטלי שבין נתיב תנועת הקונדיל בפרוטרוזיה לבין מישור פרנקפורט (נעה לרוב בין 30 ל-45 מעלות)."
      ],
      "correct": "ד. הזווית במישור הסגיטלי שבין נתיב תנועת הקונדיל בפרוטרוזיה לבין מישור פרנקפורט (נעה לרוב בין 30 ל-45 מעלות).",
      "explanation": "Condylar Guidance Angle היא הזווית במישור הסגיטלי שנוצרת בין שביל החלקת הקונדיל על ה-Articular Eminence לבין מישור הייחוס האופקי (Frankfort Horizontal Plane).",
      "summary": "Condylar Guidance Angle: הזווית הסגיטלית בין מסלול גלישת הקונדיל על ה-Eminence לבין מישור Frankfort Horizontal (בממוצע 30-45°)."
    },
    {
      "id": 73,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהו ה-Cuspal Interferences בצד העובד (Working side interference)?",
      "options": [
        "א. מגע לא רצוי בין התלוליות הלא-תומכות (Buccal עליונה / Lingual תחתונה) בצד העובד המונע מגע תקין בניב או בתלוליות התמיכה.",
        "ב. מגע תקין ב-MI. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מגע בין החותכות המרכזיות ב-CR. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מגע המופיע אך ורק בעת בליעה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. מגע לא רצוי בין התלוליות הלא-תומכות (Buccal עליונה / Lingual תחתונה) בצד העובד המונע מגע תקין בניב או בתלוליות התמיכה.",
      "explanation": "Working side interference מתרחשת בצד העובד בין תלוליות הגזירה/האוטמות (Non-supporting cusps), ומפריעה לתנועה ההרמונית של הניבים ותלוליות התמיכה.",
      "summary": "הפרעה בצד העובד (Working interference): מגע לא רצוי בצד העובד בין תלוליות גזירה (Non-centric cusps) המפריע לתנועה לטרלית תקינה."
    },
    {
      "id": 74,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "בניתוח ביומכני של כוחות סגירה, היכן ממוקם ה-Fulcrum (נקודת המשען) של הלסת התחתונה במבנה המנוף (Lever system)?",
      "options": [
        "א. במרכז שריר ה-Masseter – מנוף מסוג Class II.",
        "ב. במפרקי הלסת (TMJ) – מנוף מסוג Class III.",
        "ג. בלהב החותכות הקדמיות – מנוף מסוג Class I.",
        "ד. בשן הטחנת הראשונה בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. במפרקי הלסת (TMJ) – מנוף מסוג Class III.",
      "explanation": "המנדיבולה פועלת כמנוף מסוג Class III שבו ה-Fulcrum ממוקם ב-TMJ, הכוח (Effort) מופעל ע\"י שרירי הלעיסה, והעומס (Load) מופעל על השיניים.",
      "summary": "ביומכניקת הלסת: המנדיבולה פועלת כמנוף מסוג Class III, כאשר ה-TMJ מהווה את הפלקרום (Fulcrum) ושרירי הסגירה מפעילים את הכוח."
    },
    {
      "id": 75,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהי התופעה הידועה כ-Pankey-Mann-Schuyler (PMS) Philosophy בשיקום פה מלא?",
      "options": [
        "א. עבודה עם כתרים זמניים מאקריל בלבד ללא שחזור קבוע. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. טכניקה המבוססת אך ורק על עבודה ללא קשת פנים. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ג. טכניקת שיקום המתחילה בארגון המשנן התחתון הקדמי, הגדרת ההדרכה הקדמית, ולאחר מכן בניית המשנן האחורי בעזרת Monson Spherical Theory.",
        "ד. ביטול מוחלט של מגעים בניבים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. טכניקת שיקום המתחילה בארגון המשנן התחתון הקדמי, הגדרת ההדרכה הקדמית, ולאחר מכן בניית המשנן האחורי בעזרת Monson Spherical Theory.",
      "explanation": "פילוסופיית PMS מציעה גישה שלבית לשיקום פה נרחב: 1. שיקום המשנן הקדמי התחתון. 2. ביסוס הדרכה קדמית. 3. בניית המשטחים האוקלוזליים האחוריים.",
      "summary": "פילוסופיית PMS (Pankey-Mann-Schuyler): גישה שלבית לשיקום מלא – ייצוב קדמיות תחתונות, ביסוס הדרכה קדמית, ובניית המשנן האחורי בהרמוניה."
    },
    {
      "id": 76,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מה מתארת עקומת Monson (Monson's Spherical Theory)?",
      "options": [
        "א. עקומה המגדירה את גובה ה-Retrodiscal pad. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. עקומה ישרה שטוחה לחלוטין ללא קימורים. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ג. תיאוריית שחיקה דנטלית במבוגרים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. תיאוריה לפיה המשטחים האוקלוזליים של המשנן משיקים לכדור בעל רדיוס של 4 אינץ' (כ-10.4 ס\"מ) שמרכזו ב-Crista Galli."
      ],
      "correct": "ד. תיאוריה לפיה המשטחים האוקלוזליים של המשנן משיקים לכדור בעל רדיוס של 4 אינץ' (כ-10.4 ס\"מ) שמרכזו ב-Crista Galli.",
      "explanation": "תיאוריית מונסון משלבת את עקומת Spee ועקומת Wilson למשטח כדורי (Sphere) ברדיוס של 4 אינץ' (10.4 ס\"מ) שמרכזו באזור ה-Crista galli.",
      "summary": "Monson's Spherical Theory: משטחי הסגר משיקים לכדור ברדיוס 4 אינץ' (10.4 ס\"מ) שמרכזו ב-Crista Galli (משלב Spee & Wilson)."
    },
    {
      "id": 77,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהי השפעת אובדן תמיכה אחורית (Posterior Biting Collapse) על ה-TMJ והשיניים הקדמיות?",
      "options": [
        "א. העמסת יתר על ה-TMJ, הסטה אנטרו-סופריורית של הקונדילים, והטיות/מרווחים (Fanning) בחותכות העליונות עקב כוחות אופקיים.",
        "ב. ייצוב מלא של המפרק ללא שחיקה. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ג. ירידה בעומק ה-Overbite ל-0. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הגדלת ה-Freeway space ל-15 מ\"מ ללא שינוי בקדמיות. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "א. העמסת יתר על ה-TMJ, הסטה אנטרו-סופריורית של הקונדילים, והטיות/מרווחים (Fanning) בחותכות העליונות עקב כוחות אופקיים.",
      "explanation": "אובדן טחנות ומלתעות מעביר את עומס הלעיסה לקדמיות ול-TMJ, וגורם לירידה ב-VDO, דחיסת הקונדיל, והטיה לאביאלית (Fanning) של הקדמיות העליונות.",
      "summary": "Posterior Bite Collapse: אובדן תמיכה אחורית מוביל לירידה ב-VDO, העמסת יתר על ה-TMJ, והטיה לאביאלית (Fanning) של הקדמיות העליונות."
    },
    {
      "id": 78,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהו ה-Centric Stop ותפקידו הביומכני במשנן?",
      "options": [
        "א. הלהב האינסיזלי של הניב העליון במנוחה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. נקודת המגע של תלולית התמיכה בפוסה או ב-Marginal ridge הנגדי, המייצבת את המימד האנכי (VDO) ומונעת בקיעת יתר.",
        "ג. מגע המתרחש בעת תנועה פרוטרוזיבית בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. נקודת העצירה של הארטיקולטור ב-0 מעלות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. נקודת המגע של תלולית התמיכה בפוסה או ב-Marginal ridge הנגדי, המייצבת את המימד האנכי (VDO) ומונעת בקיעת יתר.",
      "explanation": "Centric Stops הם נקודות המגע שבין תלוליות התמיכה (Stamp cusps) למשטחים הנגדיים, השומרים על המימד האנכי של הסגר (VDO) ומונעים תזוזת שיניים.",
      "summary": "Centric Stops: נקודות מגע אוקלוזליות של תלוליות התמיכה המייצבות את השן, שומרות על VDO, ומונעות בקיעת יתר או הטיות."
    },
    {
      "id": 79,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "איזו שיטה נחשבת לזהב ברישום אלקטרוני דינמי של תנועות המנדיבולה להעברה לארטיקולטור ממוחשב?",
      "options": [
        "א. שימוש בנייר אוקלוזלי בעובי 200 מיקרון.",
        "ב. מדידת ה-Freeway space בעזרת סרגל עץ.",
        "ג. JAW (Zebris) / Pantographic Axiography תלת-ממדית.",
        "ד. צילום סטילס של הפנים בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. JAW (Zebris) / Pantographic Axiography תלת-ממדית.",
      "explanation": "Axiography / Pantography דיגיטלית (כגון Zebris / Cadiax) מודדת את תנועות הקונדילים בזמן אמת ומייצרת קובץ כיול מדויק לארטיקולטור וירטואלי.",
      "summary": "Electronic Axiography / Pantography: מדידה דיגיטלית בזמן אמת של ציר הסיבוב ותנועות הקונדילים לכיול ארטיקולטור ממוחשב."
    },
    {
      "id": 80,
      "topic": "1. אוקלוזיה, TMJ ותנועות לסת",
      "difficulty": "מומחה",
      "question": "מהי המשמעות הביולוגית של ה-Biological Width בשיקום שולי כתר קדמי ואחורי?",
      "options": [
        "א. המרחק בין החור האורביטלי לקצה הניב. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. עובי חומרי ההדבקה מתחת לכתר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. רוחב העצם הקורטיקלית מסביב לשתל בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. מכלול הרקמה הרכה שמעל העצם האלבאולרית (Junctional epithelium + Connective tissue attachment) ברוחב ממוצע של כ-2.04 מ\"מ."
      ],
      "correct": "ד. מכלול הרקמה הרכה שמעל העצם האלבאולרית (Junctional epithelium + Connective tissue attachment) ברוחב ממוצע של כ-2.04 מ\"מ.",
      "explanation": "Biological Width כולל את ה-Junctional Epithelium (~0.97mm) וה-Connective Tissue Attachment (~1.07mm) ובסה\"כ כ-2.04 מ\"מ. פגיעה ברוחב הביולוגי ע\"י קו סיום עמוק מדי גורמת לדלקת כרונית וספיגת עצם.",
      "summary": "Biological Width: מרחק פיזיו-אנטומי של כ-2.04 מ\"מ מעל עצם הרכס (אפיתל מחבר + חיבור חיבורי). פגיעה בשולי הכתר מובילה לדלקת כרונית וספיגת עצם."
    }
  ],
  "2": [
    {
      "id": 201,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מהי ההגדרה המבנית הכללית הבסיסית של ארטיקולטור מסוג Arcon?",
      "options": [
        "א. הרכיב הקונדילרי ממוקם בזרוע התחתונה והפוסה המכנית בזרוע העליונה, באופן המדמה את האנטומיה האנושית.",
        "ב. הארטיקולטור אינו מאפשר ביצוע תנועות גלישה צדיות כלל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הפוסה והקונדיל מחוברים בקפיץ קבוע ללא אפשרות פירוק הזרועות. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. הרכיב הקונדילרי (Condylar element) ממוקם בזרוע העליונה והפוסה בזרוע התחתונה."
      ],
      "correct": "א. הרכיב הקונדילרי ממוקם בזרוע התחתונה והפוסה המכנית בזרוע העליונה, באופן המדמה את האנטומיה האנושית.",
      "explanation": "לפי שילינבורג ושחזורי המבחנים (2016-2024), בארטיקולטור מסוג Arcon הרכיבים הקונדילריים מחוברים לזרוע התחתונה והפוסה המכנית מחוברת לזרוע העליונה, בדיוק כפי שהקונדיל והפוסה ממוקמים בגולגולת האדם [11, 13, 228]."
    },
    {
      "id": 202,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מה תפקידה המרכזי של קשת הפנים (Facebow) ברפואת שיניים שיקומית?",
      "options": [
        "א. למדוד את עובי הדיסק הארטיקולרי במפרק הלסת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. להעביר את יחס הלסת העליונה לציר הסיבוב המפרקי (Hinge axis) ולמישור הייחוס בפנים אל הארטיקולטור.",
        "ג. לקבוע את המימד האנכי של הסגר (VDO) במטופלים מחוסרי שיניים. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר.",
        "ד. לקבוע את גובה התלוליות של השיניים האחוריות בתותבות שלמות. בהתאם להנחיות הכלליות המופיעות בספרות הלימוד הדנטלית."
      ],
      "correct": "ב. להעביר את יחס הלסת העליונה לציר הסיבוב המפרקי (Hinge axis) ולמישור הייחוס בפנים אל הארטיקולטור.",
      "explanation": "לפי הספרות העיונית (שילינבורג / זארב / מול), קשת הפנים משמשת להעברת היחס התלת-ממדי של הלסת העליונה ביחס לציר הסיבוב הבין-קונדילרי (Terminal Hinge Axis) ולמישור הייחוס בפנים אל הארטיקולטור [5, 16, 222]."
    },
    {
      "id": 203,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "איזה חומר מבין הבאים נחשב בעל הקשיחות הגבוהה ביותר והתנגדות מינימלית לסגירה בעת רישום בין-לסתי (Interocclusal record)?",
      "options": [
        "א. שעווה רכה (Baseplate wax). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אלגינאט (Irreversible hydrocolloid).",
        "ג. משחת Zinc Oxide Eugenol (ZOE) / אקריל קשיח.",
        "ד. חומר הטבעה אגאר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. משחת Zinc Oxide Eugenol (ZOE) / אקריל קשיח.",
      "explanation": "משחת ZOE ואקריל מציעים קשיחות גבוהה לאחר התקשות עם מינימום עיוות אלסטי (Rebound), מה שהופך אותם לבעלי נאמנות גבוהה ברישום הבין-לסתי בהשוואה לשעוות [16, 17]."
    },
    {
      "id": 204,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "באיזה מרחק נקודת הייחוס האחורית של קשת פנים ארביטררית (Arbitrary facebow) ממוקמת בקירוב מקו ה-Tragus-Cantus?",
      "options": [
        "א. 25 מ\"מ מתחת לאורביטה.",
        "ב. 5 מ\"מ אחורית ל-Tragus.",
        "ג. 2 מ\"מ מעל ה-External auditory meatus.",
        "ד. 13 מ\"מ קדמית ל-Tragus."
      ],
      "correct": "ד. 13 מ\"מ קדמית ל-Tragus.",
      "explanation": "לפי הספרות העיונית (מול / שילינבורג ושחזורי המבחנים), בקשת פנים ארביטררית נקודת הייחוס לציר הסיבוב ממוקמת כ-13 מ\"מ קדמית ל-Tragus על הקו המחבר את ה-Tragus לזווית העין (Tragus-cantus line) [52, 160, 225]."
    },
    {
      "id": 205,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מה מתרחש ל-Condylar guidance בארטיקולטור מסוג Arcon בעת שינוי המימד האנכי (הסרת משנך השעווה)?",
      "options": [
        "א. ה-Condylar guidance אינו משתנה ונשאר קבוע.",
        "ב. ה-Condylar guidance מתאפס באופן אוטומטי.",
        "ג. ה-Condylar guidance הופך לתלול יותר (Steeper).",
        "ד. ה-Condylar guidance הופך לרדוד יותר (Less steep)."
      ],
      "correct": "א. ה-Condylar guidance אינו משתנה ונשאר קבוע.",
      "explanation": "בארטיקולטור Arcon הזווית בין הפוסה המכנית (בזרוע העליונה) לזרוע העליונה נשארת קבועה גם בעת פתיחה או סגירה של המנשך, ולכן ה-Condylar guidance אינו משתנה [11, 13, 228]."
    },
    {
      "id": 206,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "איזו תנועה מפרקית מתרחשת במחצית הראשונית של פתיחת הפה (עד 20-25 מ\"מ)?",
      "options": [
        "א. תנועת טרנסלציה (החלקה) במדור העליון.",
        "ב. תנועת רוטציה (צירית) במדור התחתון.",
        "ג. תנועה לטרלית גופנית של הדיסק בלבד.",
        "ד. פריקה מלאה של הקונדיל מחוץ לפוסה."
      ],
      "correct": "ב. תנועת רוטציה (צירית) במדור התחתון.",
      "explanation": "פתיחת פה ראשונית מתרחשת סביב ה-Terminal Hinge Axis וכוללת רוטציה טהורה במדור התחתון של ה-TMJ [3, 36]."
    },
    {
      "id": 207,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "באיזה מישור ייחוס אופקי בפנים נעזרים בלוחית Fox (Fox plane) לקביעת מישור הסגר בתותבות שלמות?",
      "options": [
        "א. Frankfurt horizontal plane בלבד.",
        "ב. קו ה-Nasion-Pogonion. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Camper's line (קו ה-Ala-Tragus).",
        "ד. Mandibular plane בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. Camper's line (קו ה-Ala-Tragus).",
      "explanation": "קביעת מישור הסגר באזור האחורי בתותבות שלמות מתבצעת ע\"י הקבלה ל-Camper's line (הקו המחבר את תחתית נחיר האף ל-Tragus) בעזרת Fox plane [5, 288]."
    },
    {
      "id": 208,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מהו המרחק הבין-קונדילרי (Intercondylar distance) הסטנדרטי הממוצע בארטיקולטורים חצי-מתכווננים?",
      "options": [
        "א. 80 מ\"מ.",
        "ב. 150 מ\"מ.",
        "ג. 60 מ\"מ.",
        "ד. 110 מ\"מ."
      ],
      "correct": "ד. 110 מ\"מ.",
      "explanation": "המרחק הבין-קונדילרי הממוצע שקבוע בארטיקולטורים חצי-מתכווננים רבים הוא 110 מ\"מ [49, 116, 229]."
    },
    {
      "id": 209,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "כיצד משפיע שימוש במשנך שעווה בעובי 4 מ\"מ על ארטיקולטור מסוג Non-Arcon לאחר הוצאת המשנך וסגירת הגבסים?",
      "options": [
        "א. הדרכה קונדילרית (Condylar guidance) הופכת לרדודה יותר (Less steep).",
        "ב. הזווית הופכת לשלילית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הדרכה קונדילרית אינה משתנה כלל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הדרכה קונדילרית הופכת לתלולה יותר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. הדרכה קונדילרית (Condylar guidance) הופכת לרדודה יותר (Less steep).",
      "explanation": "בארטיקולטור Non-Arcon, עקב מיקום הרכיב הקונדילרי בזרוע העליונה, סגירת המנשך לאחר הסרת משנך בעובי 3-5 מ\"מ משנה את היחס הגיאומטרי והופכת את ההדרכה הקונדילרית לרדודה יותר [13, 55, 228]."
    },
    {
      "id": 210,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מהי הטכניקה המכונה Triple Tray בעת ביצוע רישום סגר ושחזור בודד?",
      "options": [
        "א. כף אישית העשויה משלוש שכבות אקריל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מטבע המטפל בו-זמנית בשן המושחזת, השיניים הנגדיות ורישום הסגר ב-MI.",
        "ג. רישום של עמדת CR בלבד באמצעות שלושה חומרי הטבעה שונים.",
        "ד. מטבע של שלוש לסתות שונות בעת ובעונה אחת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. מטבע המטפל בו-זמנית בשן המושחזת, השיניים הנגדיות ורישום הסגר ב-MI.",
      "explanation": "Triple tray מאפשרת ללכוד במקביל את ההטבעה של ההכנה, הקשת הנגדית ורישום הסגר במגע מלא [17, 107, 147]."
    },
    {
      "id": 211,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "בעבודה עם קשת פנים הנשענת על פתחי השמע החיצוניים (Ear-piece facebow), מהו הגורם המפצה על המיקום האחורי של התעלה?",
      "options": [
        "א. הקשת מתאימה לשימוש אך ורק בארטיקולטור Fully adjustable.",
        "ב. אין כל קיזוז – המטופל נדרש להטות את הראש קדימה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. המכשיר כולל קיזוז מובנה (Offset) המעביר את הנקודה בקירוב לציר הסיבוב האנטומי.",
        "ד. יש להוסיף 20 מ\"מ לגובה הפין האנכי בארטיקולטור. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר."
      ],
      "correct": "ג. המכשיר כולל קיזוז מובנה (Offset) המעביר את הנקודה בקירוב לציר הסיבוב האנטומי.",
      "explanation": "קשתות פנים מודרניות מסוג Ear-piece כוללות הסטה מתוכננת (Offset) שמביאה את המדידה לקירוב מצוין של ציר הסיבוב [52, 160]."
    },
    {
      "id": 212,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מה מבין הבאים נכון לגבי הרכבת גבס עליון ראשון בארטיקולטור בעזרת קשת פנים?",
      "options": [
        "א. מרכיבים את שני הגבסים בו-זמנית ללא קשת פנים. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ב. קשת הפנים משמשת להרכבת הגבס התחתון בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. הגבס התחתון חייב להיות מורכב תמיד ראשון. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הגבס העליון מורכב ראשון כשהוא נתמך ע\"י קשת הפנים או שולחן ההרכבה, ורק לאחר מכן מורכב הגבס התחתון בעזרת רישום בין-לסתי."
      ],
      "correct": "ד. הגבס העליון מורכב ראשון כשהוא נתמך ע\"י קשת הפנים או שולחן ההרכבה, ורק לאחר מכן מורכב הגבס התחתון בעזרת רישום בין-לסתי.",
      "explanation": "הפרוטוקול הקליני הסטנדרטי מורה על הרכבת הגבס העליון ראשית באמצעות קשת הפנים, ולאחר מכן הרכבת הגבס התחתון ביחס אליו [5, 16]."
    },
    {
      "id": 213,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "למה משמש שולחן הדרכה אינסיזלי (Incisal guide table) בארטיקולטור?",
      "options": [
        "א. לשמור ולחקות את ההדרכה הקדמית (Anterior guidance) המקורית או המתוכננת במהלך תנועות הגלישה.",
        "ב. למדוד את הלחץ המופעל על הדיסק ב-TMJ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. לכייל את זווית Bennett בצד העובד בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. למנוע התפשטות של חומר הגבס בעת ההתקשות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. לשמור ולחקות את ההדרכה הקדמית (Anterior guidance) המקורית או המתוכננת במהלך תנועות הגלישה.",
      "explanation": "שולחן ההדרכה האינסיזלי מגן על השיניים הקדמיות בגבס ומבטיח שההדרכה הקדמית תועתק במדויק לשחזורים [9, 28]."
    },
    {
      "id": 214,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "איזה רישום בין-לסתי מיועד לכייל את ה-Condylar guidance angle בארטיקולטור חצי-מתכוונן?",
      "options": [
        "א. Centric Relation record במנוחה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Protrusive interocclusal record (רישום פרוטרוזיבי).",
        "ג. Facebow transfer בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. רישום שוליים בפוליאתר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Protrusive interocclusal record (רישום פרוטרוזיבי).",
      "explanation": "רישום פרוטרוזיבי בשעווה/חומר קשיח תופס את הקונדילים כשהם גלשו קדימה בפוסה, וכך מאפשר לכייל את זווית ה-Condylar guidance בארטיקולטור [12, 55, 228]."
    },
    {
      "id": 215,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מה נכון לגבי שימוש בארטיקולטור פשוט (Hinge articulator / ציר בלבד)?",
      "options": [
        "א. הוא מצריך שימוש חובה בקשת פנים קינמטית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הוא מדמה באופן מושלם את תנועת Bennett וההדרכה הקונדילרית.",
        "ג. הוא מאפשר תנועת ציר פתיחה-סגירה בלבד ואינו מדמה תנועות גלישה אקסצנטריות אנטומיות.",
        "ד. הוא מומלץ לשיקום פה מלא ומורכב במיוחד. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. הוא מאפשר תנועת ציר פתיחה-סגירה בלבד ואינו מדמה תנועות גלישה אקסצנטריות אנטומיות.",
      "explanation": "ארטיקולטור ציר פשוט מתפקד כציר סגירה בלבד ללא שחזור נכון של רדיוס התנועה או שבילי הגלישה [9, 28]."
    },
    {
      "id": 216,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "איזה מישור אופקי בפנים מחבר בין ה-Nasion לבין ה-Pogonion?",
      "options": [
        "א. Occlusal plane. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Camper's plane. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Frankfurt horizontal plane.",
        "ד. מישור הפנים (Facial plane)."
      ],
      "correct": "ד. מישור הפנים (Facial plane).",
      "explanation": "לפי שחזורי המבחנים (2016), קו/מישור הפנים מוגדר בין ה-Nasion לבין ה-Pogonion [36, 95]."
    },
    {
      "id": 217,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "בעת קביעת מישור הסגר הוורטיקלי בתותבות שלמות, באיזה מדד אנטומי אחורי נעזרים לקביעת הגובה?",
      "options": [
        "א. שליש עליון עד שני שליש מגובה ה-Retromolar pad.",
        "ב. שיא גובה ה-Buccal shelf. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. ה-Incisive papilla בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. גבול ה-Genial tubercle העליון. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. שליש עליון עד שני שליש מגובה ה-Retromolar pad.",
      "explanation": "מישור הסגר האחורי בתותבת תחתונה נקבע בגובה 1/2 עד 2/3 מגובה ה-Retromolar pad [24, 50, 229]."
    },
    {
      "id": 218,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מהי המטרה העיקרית של ביצוע Split-cast במודלי גבס בארטיקולטור?",
      "options": [
        "א. למנוע ספיגת עצם ברכס השארי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. לוודא ולבדוק את דיוק הושבת המודלים ודיוק הרישום הבין-לסתי.",
        "ג. לאפשר הדבקה מהירה של השחזור עם ZPC. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. להקטין את זמן ההתקשות של גבס ההרכבה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. לוודא ולבדוק את דיוק הושבת המודלים ודיוק הרישום הבין-לסתי.",
      "explanation": "טכניקת Split-cast מאפשרת להפריד ולהחזיר את המודל לבסיסו כדי לבדוק שוב ושוב את הדירות והתאמת הרישום הבין-לסתי [16, 28]."
    },
    {
      "id": 219,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "מהו היתרון המרכזי של חומר מבוסס Polyether לרישום בין-לסתי?",
      "options": [
        "א. זמן התקשות של 45 דקות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. גמישות אלסטית רכה במיוחד המאפשרת מעוך קל.",
        "ג. יציבות ממדית גבוהה וקשיחות מעולה לאחר התקשות.",
        "ד. ספיגת מים מוגברת הגורמת להתפשטות רצויה."
      ],
      "correct": "ג. יציבות ממדית גבוהה וקשיחות מעולה לאחר התקשות.",
      "explanation": "פוליאתר מציג יציבות ממדית מצוינת וקשיחות גבוהה שמונעת דפורמציה בעת הרכבת המודלים [16, 17]."
    },
    {
      "id": 220,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קל",
      "question": "איזה מקור שגיאה נפוץ ביותר בעת שימוש בשעוות סגר רכות (Baseplate wax) לרישום CR?",
      "options": [
        "א. התקשות מהירה מדי בתוך 2 שניות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. סדיקת כפות הגבס עקב חומציות גבוהה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הדבקות כימית בלתי הפיכה לאמייל השן. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. עיוות אלסטי (Rebound) ודפורמציה בעת קירור או לחיצה בלתי אחידה."
      ],
      "correct": "ד. עיוות אלסטי (Rebound) ודפורמציה בעת קירור או לחיצה בלתי אחידה.",
      "explanation": "שעוות נוטות לעיוות תרמי ואלסטי ניכר אם מופעל עליהן לחץ לא אחיד או בעת שינויי טמפרטורה [16, 17]."
    },
    {
      "id": 221,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "על פי מחקרו של Weinberg, אם קיים הפרש/שגיאה של 5 מ\"מ בין ציר הסיבוב האמיתי של המטופל לבין ציר הסיבוב שרשמנו בקשת פנים ארביטררית (עבור משנך בעובי 3 מ\"מ), מה יהיה השינוי/השגיאה במיקום המולר השני?",
      "options": [
        "א. כ-0.2 מ\"מ (שגיאה קלינית מזערית ברת כיוונון).",
        "ב. 5.0 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. 2.5 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. 0.0 מ\"מ בדיוק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. כ-0.2 מ\"מ (שגיאה קלינית מזערית ברת כיוונון).",
      "explanation": "לפי שילינבורג (עמ' 30) ושחזורי המבחנים, שגיאה של 5 מ\"מ במיקום הציר בקשת פנים ארביטררית תתרגם לשגיאה סגרית זעירה של כ-0.2 מ\"מ בלבד באזור המולרי בעת הסרת משנך של 3 מ\"מ [16]."
    },
    {
      "id": 222,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "בנוסחת Hanau (Hanau's Quint) ליצירת Balanced Occlusion, כיצד מחושבת זווית Bennett ($L$)?",
      "options": [
        "א. L = H + Incisal guidance כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. L = H/8 + 12 (כאשר H היא זווית ה-Condylar inclination).",
        "ג. L = H * 2 + 5 כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. L = H/2 - 10 כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. L = H/8 + 12 (כאשר H היא זווית ה-Condylar inclination).",
      "explanation": "לפי הספרות והשחזורים, נוסחת הופמן/הנאו לכיול זווית Bennett בארטיקולטור הנאו היא L = H/8 + 12 [32, 104]."
    },
    {
      "id": 223,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "כיצד משפיע מרחק בין-קונדילרי קטן יותר מזה של המטופל על שבילי הגלישה (Grooves) של התלוליות בשיניים האחוריות?",
      "options": [
        "א. התלוליות מתמזגות למשטח שטוח לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אין שום השפעה על כיוון שבילי הגלישה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הזווית בין שבילי הגלישה הבוקאליים והלינגואליים (Working and Non-working paths) הופכת לחדה/קטנה יותר.",
        "ד. הזווית בין שבילי הגלישה הופכת לקהה/רחבה יותר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. הזווית בין שבילי הגלישה הבוקאליים והלינגואליים (Working and Non-working paths) הופכת לחדה/קטנה יותר.",
      "explanation": "ככל שהמרחק הבין-קונדילרי קטן יותר, הזווית הנוצרת בין שביל בצד העובד לשביל בצד הלא-עובד על פני המשטח האוקלוזלי קטנה יותר [11, 49, 116]."
    },
    {
      "id": 224,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "על פי מחקר השוואתי של חומרי רישום בין-לסתי במקרים של Free-end saddle (Pagnano et al. / שחזורים), איזה חומר הראה את הסטייה/העיוות הגדול ביותר בהשוואה להרכבה ידנית?",
      "options": [
        "א. Zinc Oxide Eugenol. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אקריל קשיח + שעווה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. גבס מטבעים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Condensation silicone (סיליקון בדחיסה)."
      ],
      "correct": "ד. Condensation silicone (סיליקון בדחיסה).",
      "explanation": "במחקרי נאמנות חומרי רישום סגר, סיליקון בדחיסה (Condensation silicone) הראה את הסטייה הגדולה ביותר עקב התכווצות תוצרי הלוואי (אלכוהול) ועיוות אלסטי [16, 17]."
    },
    {
      "id": 225,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "מהו הצידוק המרכזי של Mohl ו-Shillingburg לשימוש בקשת פנים ארביטררית (Arbitrary facebow) ברוב הטיפולים השיקומיים השגרתיים?",
      "options": [
        "א. היא מספקת דיוק קליני מספק בהחלט ברוב המקרים כאשר השינוי ב-VDO מזערי, וחוסכת זמן ומורכבות של קשת קינמטית.",
        "ב. אין צורך בגבס עליון בעת שימוש בקשת זו. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. היא המכשיר היחיד שמסוגל למדוד את תנועת ה-Bennett המיידית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. היא מונעת לחלוטין שחיקה של שיני הארטיקולטור. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. היא מספקת דיוק קליני מספק בהחלט ברוב המקרים כאשר השינוי ב-VDO מזערי, וחוסכת זמן ומורכבות של קשת קינמטית.",
      "explanation": "לפי Mohl ושילינבורג, קשת פנים ארביטררית (13 מ\"מ מה-Tragus או סמוכה לאוזן) מספקת דיוק מצוין לרוב השיקומים הקליניים [52, 160]."
    },
    {
      "id": 226,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "מה מאפשר רישום ה-Lateral Interocclusal Record (Check bite) בארטיקולטור סמי-אדג'סטבל?",
      "options": [
        "א. קביעת המימד האנכי במנוחה (VDR) בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. מציאת עמדת הקונדילים בפוסה בתנועה צדית לקביעת זווית ה-Condylar guidance וזווית Bennett בארטיקולטור.",
        "ג. מדידת הלחץ התת-דיסקי בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. ביטול תנועת ה-Hinge axis של המפרק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. מציאת עמדת הקונדילים בפוסה בתנועה צדית לקביעת זווית ה-Condylar guidance וזווית Bennett בארטיקולטור.",
      "explanation": "לפי שחזורי המבחנים (2016-2024), רישום Check bite לטרלי תופס את עמדת הקונדילים בגלישה צדית ומאפשר כיול אנטומי מתאים בארטיקולטור [12, 63, 147]."
    },
    {
      "id": 227,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "למה גורם Rebound effect בחומרי הטבעה ורישום סגר אלסטומריים בעת הפעלת לחץ סגירה חזק מדי?",
      "options": [
        "א. עליה בדרגת החומציות של הגבס. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. החומר נמס והופך לנוזל הידרופילי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. החומר נדחס פלסטית בעת הסגירה, ובעת הסרת העומס הוא חוזר אלסטיות וגורם להרכבה שגויה ולא מודבקת של המודלים (מרווח סגרי).",
        "ד. המודלים ננעלים יחד ללא יכולת הפרדה. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "ג. החומר נדחס פלסטית בעת הסגירה, ובעת הסרת העומס הוא חוזר אלסטיות וגורם להרכבה שגויה ולא מודבקת של המודלים (מרווח סגרי).",
      "explanation": "לחץ סגירה מופרז על חומר אלסטומרי גורם לדחיסה אלסטית, שכאשר הלחץ משתחרר החומר 'קופץ' בחזרה (Rebound) ומספק רישום מוטעה שמפריע להושבת המודלים [49, 104, 163]."
    },
    {
      "id": 228,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "מדוע ארטיקולטורים מסוג Non-Arcon היו פופולריים מאוד בעבר בביצוע תותבות שלמות (Complete Dentures)?",
      "options": [
        "א. מכיוון שאינם מצריכים שימוש בגבס להרכבת המודלים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מכיוון שהם מדמים את האנטומיה האנושית בדיוק רב יותר מאשר Arcon. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מכיוון שזהו הארטיקולטור היחיד המאפשר שימוש בקשת פנים קינמטית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מכיוון שהזרוע העליונה והתחתונה אינן נפרדות בקלות בעת סידור שיני התותבת, מה שמקל על שמירת עמדת הסגר המרכזי."
      ],
      "correct": "ד. מכיוון שהזרוע העליונה והתחתונה אינן נפרדות בקלות בעת סידור שיני התותבת, מה שמקל על שמירת עמדת הסגר המרכזי.",
      "explanation": "לפי שילינבורג (פרק 3), ארטיקולטור Non-Arcon אינו מתפרק בקלות בעת מניפולציה של השיניים האחוריות בתותבות שלמות, ולכן טכנאים רבים העדיפו אותו לסידור שיניים [56, 228]."
    },
    {
      "id": 229,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "מהי הדרך المומלצת ליצירת שולחן הדרכה אינסיזלי מותאם אישית (Custom Incisal Guide Table)?",
      "options": [
        "א. שימוש באקריל מתקשה מעצמו (Self-curing resin) והנעת פין ההדרכה האינסיזלי בתוך האקריל הרך בהתאם למגעי השיניים הקדמיות במודל.",
        "ב. יציקת מתכת נוזלית על שולחן הפלסטיק השטוח. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. שימוש בשעוות דביקה (Sticky wax) בלבד ללא אקריל. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. הגדלת זווית הפוסה ה-TMJ ב-45 מעלות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. שימוש באקריל מתקשה מעצמו (Self-curing resin) והנעת פין ההדרכה האינסיזלי בתוך האקריל הרך בהתאם למגעי השיניים הקדמיות במודל.",
      "explanation": "שולחן אינסיזלי מותאם נוצר ע\"י מריחת אקריל רך והנעת מודלי הגבס בתנועות הגלישה השונות, כך שהפין האינסיזלי מטביע בשולחן את מסלול ההדרכה המדויק [9, 28]."
    },
    {
      "id": 230,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "בינוני",
      "question": "מה מייצגת תופעת Christensen (Christensen's phenomenon) בעת גלישת המנדיבולה לפרוטרוזיה?",
      "options": [
        "א. החלקה מדיאלית של הקונדיל בצד העובד. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. היווצרות מרווח/הפרדה (Disclusion) בין השיניים האחוריות בעת גלישה קדימה עקב שיפוע ה-Condylar guidance וההדרכה הקדמית.",
        "ג. כפיפה גופנית של עצם המנדיבולה פנימה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. נעילה מלאה של כל התלוליות האחוריות ללא מרווח. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "ב. היווצרות מרווח/הפרדה (Disclusion) בין השיניים האחוריות בעת גלישה קדימה עקב שיפוע ה-Condylar guidance וההדרכה הקדמית.",
      "explanation": "תופעת כריסטנסן מתארת את הרווח הנוצר בין המשננים האחוריים בעת גלישה קדימה (פרוטרוזיה) כתוצאה מהשיפוע של ה-Condylar guidance [28, 104]."
    },
    {
      "id": 241,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קשה",
      "question": "מהו ההבדל המושגי והקליני המדויק בין Condylar Angulation לבין Condylar Inclination בשחזורי המבחנים והספרות?",
      "options": [
        "א. אין כל הבדל – מדובר במונחים זהים לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Condylar Angulation קיים רק בארטיקולטור מסוג Non-Arcon. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Condylar Angulation נמדד בצד הלא-עובד במישור הוריזונטלי (מקורו בתנועת Bennett / זווית Bennett), בעוד Condylar Inclination נמדד בגלישה קדימה (פרוטרוזיה) במישור פרה-סגיטלי.",
        "ד. Condylar Inclination נמדד רק במנוחה, ו-Condylar Angulation נמדד בסגירה מרכזית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. Condylar Angulation נמדד בצד הלא-עובד במישור הוריזונטלי (מקורו בתנועת Bennett / זווית Bennett), בעוד Condylar Inclination נמדד בגלישה קדימה (פרוטרוזיה) במישור פרה-סגיטלי.",
      "explanation": "לפי שחזורי המבחנים (2016-2019): Condylar Angulation מקורו בתנועת Bennett ונמדד בצד הלא-עובד במישור הוריזונטלי, בעוד Condylar Inclination הוא שיפוע הדרכת הקונדיל בפרוטרוזיה במישור פרה-סגיטלי [43, 101, 226]."
    },
    {
      "id": 242,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קשה",
      "question": "בעת ביצוע רישום בין-לסתי במטופל שעבר כריתה חלקית של המנדיבולה (Hemimandibulectomy / Discontinuity defect), באיזה ארטיקולטור מומלץ להשתמש על פי Boucher ושחזורי המבחנים?",
      "options": [
        "א. אין אפשרות להשתמש בארטיקולטור כלל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. ארטיקולטור Fully adjustable בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. ארטיקולטור ציר פשוט (Simple hinge) בלבד.",
        "ד. ארטיקולטור חצי-מתכוונן (Semi-adjustable articulator)."
      ],
      "correct": "ד. ארטיקולטור חצי-מתכוונן (Semi-adjustable articulator).",
      "explanation": "לפי Boucher (עמ' 486) ושחזורי המבחנים, בכריתה חלקית (Discontinuity defect) משתמשים בארטיקולטור סמי-אדג'סטבל עם קשת פנים ורישום פרוטרוזיבי [5]."
    },
    {
      "id": 243,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קשה",
      "question": "כיצד תשתנה הציור והאנטומיה של חריצי השרבוב (Grooves) בשחזור טוחנת אם ה-Condylar guidance בארטיקולטור נקבעה לתלולה יותר מאשר במטופל?",
      "options": [
        "א. התלוליות בשחזור יתוכננו גבוהות/תלולות מדי, ובעת הושבת השחזור בפה תיווצר הפרעה אוקלוזלית (Interference) בגלישה אקסצנטרית.",
        "ב. לא תהיה כל השפעה על האנטומיה האוקלוזלית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. התלוליות יתוכננו שטוחות מדי ולא ייצרו מגע בכלל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. השחזור ישנה את צבעו מ-Hue ל-Chroma. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. התלוליות בשחזור יתוכננו גבוהות/תלולות מדי, ובעת הושבת השחזור בפה תיווצר הפרעה אוקלוזלית (Interference) בגלישה אקסצנטרית.",
      "explanation": "כאשר ה-Condylar guidance בארטיקולטור תלולה מהקיים בפה, הטכנאי יבנה תלוליות גבוהות מדי, שבפיות המטופל (שם ההדרכה רדודה יותר) יגרמו להפרעה אוקלוזלית [28, 104]."
    },
    {
      "id": 244,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קשה",
      "question": "מה נכון לגבי יציבות ממדית של חומרי הטבעה מסוג Addition Silicone (PVS) לעומת Condensation Silicone בעת השהיית היציקה?",
      "options": [
        "א. שניהם מתכווצים באותו קצב בדיוק תוך 5 דקות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Addition silicone אינו משחרר תוצרי לוואי (כגון אלכוהול) ולכן שומר על יציבות ממדית מעולה גם לאחר ימים, בעוד Condensation silicone משחרר אתנול ומתכווץ במהירות.",
        "ג. Addition silicone מתפרק במים ולכן דורש אטימה בוואקום. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Condensation silicone יציב יותר עקב שחרור מימן. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Addition silicone אינו משחרר תוצרי לוואי (כגון אלכוהול) ולכן שומר על יציבות ממדית מעולה גם לאחר ימים, בעוד Condensation silicone משחרר אתנול ומתכווץ במהירות.",
      "explanation": "סיליקון בהוספה (PVS) אינו מייצר תוצרי לוואי מנדיפים ולכן שומר על יציבות ממדית מעולה, בניגוד לסיליקון בדחיסה שממשיך להתכווץ עקב התאדות אתנול [14, 28]."
    },
    {
      "id": 245,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קשה",
      "question": "מהי המשמעות של Gothic Arch Tracing (Gysi tracing) בקביעת יחס בין-לסתי במטופלים מחוסרי שיניים?",
      "options": [
        "א. שיטה לקביעת ה-VDO בעזרת הגיות פונטיות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מכשיר למדידת עובי הרכס האלבאולרי בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. רישום פנטוגרפי אופקי של תנועות המנדיבולה, שבו קודקוד החץ (Apex) מייצג את עמדת Centric Relation (CR).",
        "ד. צילום רנטגן מיוחד של ה-TMJ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. רישום פנטוגרפי אופקי של תנועות המנדיבולה, שבו קודקוד החץ (Apex) מייצג את עמדת Centric Relation (CR).",
      "explanation": "Gothic arch tracing הוא רישום אופקי תוך-אוראלי או חוץ-אוראלי, שבו קודקוד החץ מציין במדויק את עמדת ה-CR [9, 28]."
    },
    {
      "id": 246,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "קשה",
      "question": "מה יתרחש ברישום הסגר אם המטופל יבצע סגירה כפויה בלחץ שרירים מוגבר בעת נטילת רישום סגר בשעווה קשה?",
      "options": [
        "א. לא תהיה שום השפעה מכיוון שהגבס קשיח. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הארטיקולטור יחליף אוטומטית מ-Arcon ל-Non-Arcon. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. השחזור יצא נמוך מדי בסגר ב-2 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. דחיסה של הדיסק/הרקמה הרכה במפרק או עיוות השעווה, שיובילו להרכבת מודלים שתגרום להגבהה סגרית (High occlusion) בפה."
      ],
      "correct": "ד. דחיסה של הדיסק/הרקמה הרכה במפרק או עיוות השעווה, שיובילו להרכבת מודלים שתגרום להגבהה סגרית (High occlusion) בפה.",
      "explanation": "לחץ סגירה מופרז דוחס את הרקמות הרכות או מעוות את השעווה, מה שמניב רישום דק מדי שבהרכבת המודלים יתורגם להגבהת הסגר בשחזור הסופי [16, 28]."
    },
    {
      "id": 261,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "מומחה",
      "question": "על פי מול (Mohl) ושילינבורג, ממה מורכבת תנועת ה-Immediate Side Shift (ISS) בצד הלא-עובד (Non-working side condyle)?",
      "options": [
        "א. העתקה מדיאלית מיידית של הקונדיל הלא-עובד עוד לפני תחילת הרוטציה וההחלקה קדימה, המתרחשת ב-86% מהאוכלוסיה בטווח ממוצע של 0.75-1.0 מ\"מ.",
        "ב. פריקה מלאה של המפרק הנגדי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. תנועה אנכית טהורה של הקונדיל העובד כלפי מטה. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר.",
        "ד. סיבוב אחורי של הדיסק הארטיקולרי בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "א. העתקה מדיאלית מיידית של הקונדיל הלא-עובד עוד לפני תחילת הרוטציה וההחלקה קדימה, המתרחשת ב-86% מהאוכלוסיה בטווח ממוצע של 0.75-1.0 מ\"מ.",
      "explanation": "לפי מול ושילינבורג, ISS מתארת תנועת צד מדיאלית מיידית של הקונדיל בצד הלא-עובד בתחילת התנועה הצידית, עם ממוצע של כ-0.75-1.0 מ\"מ [49, 86, 381]."
    },
    {
      "id": 262,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "מומחה",
      "question": "מי מבין הבאים נחשב לאחד מאבות האסכולה הגנתולוגית המוקדמת (Early Gnathologist) שפיתח את קשת הפנים הקינמטית והפנטוגרף הראשון?",
      "options": [
        "א. Charles Stuart.",
        "ב. B.B. McCollum.",
        "ג. Niles Guichet.",
        "ד. Harvey Stallard."
      ],
      "correct": "ב. B.B. McCollum.",
      "explanation": "לפי שחזורי המבחנים (2016-2019), B.B. McCollum מייסד ה-Gnathological Society ב-1926 נחשב כ-Early Gnathologist [61, 152, 384]."
    },
    {
      "id": 263,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "מומחה",
      "question": "בפילוסופיית ה-Functionally Generated Path (FGP) של Meyer ו-Pankey-Mann-Schuyler, מהו תפקידו המרכזי של הארטיקולטור?",
      "options": [
        "א. הארטיקולטור קובע את זווית Bennett באופן בלעדי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הארטיקולטור חייב להיות Fully adjustable עם פנטוגרף אלקטרוני. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הארטיקולטור משמש כציר פשוט (Simple hinge) בלבד, מכיוון שכל מסלולי הדינמיקה האוקלוזלית נרשמים ישירות בפי המטופל על גבי לוחית שעווה/מערכת פונקציונלית.",
        "ד. הארטיקולטור מונע שימוש בגבס. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. הארטיקולטור משמש כציר פשוט (Simple hinge) בלבד, מכיוון שכל מסלולי הדינמיקה האוקלוזלית נרשמים ישירות בפי המטופל על גבי לוחית שעווה/מערכת פונקציונלית.",
      "explanation": "לפי שילינבורג (עמ' 355) ושחזורי המבחנים, בשיטת FGP הדינמיקה הסגרית מגולפת בתוך הפה על שעווה, ולכן הארטיקולטור משמש כציר פשוט בלבד (Simple hinge) לשורש [9, 28]."
    },
    {
      "id": 264,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "מומחה",
      "question": "מהי המשמעות הביומכנית המלאה של שימוש ב-Leaf Gauge או Lucia Jig בעת לקיחת רישום Centric Relation (CR)?",
      "options": [
        "א. נעילה של הדיסק הארטיקולרי במדור התחתון בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. מתיחה כפויה של שריר ה-Lateral pterygoid התחתון. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הגדלת המימד האנכי ב-15 מ\"מ באופן קבוע. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר.",
        "ד. ביטול זיכרון השרירים (Deconditioning of masticatory muscles) והשגת הפרדה אחורית (Anterior deprogrammer) המאפשרת לקונדילים להתמקם בעמדה הכי אנטרו-סופריורית בפוסה."
      ],
      "correct": "ד. ביטול זיכרון השרירים (Deconditioning of masticatory muscles) והשגת הפרדה אחורית (Anterior deprogrammer) המאפשרת לקונדילים להתמקם בעמדה הכי אנטרו-סופריורית בפוסה.",
      "explanation": "מכשירים קדמיים (De-programmers) מנתקים את המגעים האחוריים, מנטרלים את הרפלקס השרירי המותנה, ומאפשרים למיוטטיביות השרירית למקם את הקונדילים ב-CR [9, 28]."
    },
    {
      "id": 265,
      "topic": "2. ארטיקולטורים ורישום בין-לסתי",
      "difficulty": "מומחה",
      "question": "איזו השפעה תהיה לכיול שגוי של זווית ה-Condylar inclination (נמוכה ב-15 מעלות מהמציאות) על תכנון גובה התלוליות בשחזור FPD קבוע?",
      "options": [
        "א. התלוליות בשחזור יתוכננו שטוחות/נמוכות מדי, מה שיבטיח היעדר הפרעות בגלישה אך עלול להפחית את יעילות הלעיסה.",
        "ב. לא תהיה שום השפעה ביומכנית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. התלוליות יתוכננו תלולות מדי וייצרו תופעת מגע מטריד בבליעה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. השחזור יישבר מיידית בעת הדבקה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. התלוליות בשחזור יתוכננו שטוחות/נמוכות מדי, מה שיבטיח היעדר הפרעות בגלישה אך עלול להפחית את יעילות הלעיסה.",
      "explanation": "Condylar inclination נמוכה בארטיקולטור מחייבת תלוליות שטוחות יותר כדי למנוע התנגשות, כך שבפועל השחזור יהיה 'בטוח' מהפרעה אך שטוח יותר [28, 104]."
    }
  ],
  "3": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "איזה גורם ביומכני בהכנת שן לכתר קובע באופן הישיר ביותר את ה-Retention של השיקום?",
      "options": [
        "א. עובי קו הסיום השמפר (Chamfer width). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. זווית ההתכנסות הטאפר (Taper / Total Occlusal Convergence) וגובה קירות ההכנה.",
        "ג. שימוש בצמנט שרפי בלבד ללא תלות בצורת ההכנה. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. שירוש השן בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. זווית ההתכנסות הטאפר (Taper / Total Occlusal Convergence) וגובה קירות ההכנה.",
      "explanation": "לפי שילינבורג (פרק 7) ורוזנסטיל (פרק 7), האחיזה (Retention) מונעת עקירת הכתר צדית/אנכית. היא תלויה ישירות בזווית ההתכנסות (זווית טאפר אופטימלית של 6 מעלות / 2-5 מעלות לכל קיר) וגובה הקירות ההכנתיים."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהי המשמעות הביומכנית של Ferrule Effect בשן שעברה טיפול שורש ומשוקמת עם מבנה וכתר?",
      "options": [
        "א. יצירת מפתח סיבוב מרובע בתוך מוך השן. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הדבקת הכתר בצמנט אבקת אבץ פוספט בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. טבעת מתכת/זירקוניה העוטפת לפחות 1.5-2 מ\"מ של דנטין בריא אקסיאלי מעל קו הסיום למניעת שבר שורש.",
        "ד. חיזוק המבנה היצוק ע\"י הרחבת התעלה עד לשיא האפקס. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. טבעת מתכת/זירקוניה העוטפת לפחות 1.5-2 מ\"מ של דנטין בריא אקסיאלי מעל קו הסיום למניעת שבר שורש.",
      "explanation": "לפי שילינבורג ורוזנסטיל, אפקט הפרול (Ferrule Effect) דורש לפחות 1.5-2.0 מ\"מ של דנטין בריא אנכי מעל ה-Finish line עם עובי דופן של לפחות 1 מ\"מ. זהו הגורם הקריטי ביותר למניעת שברי שורש אנכיים בשיניים מטופלות שורש."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "בעת השחזת שן לכתר מתכת-חרסינה (PFM) בטוחנה עליונה, מהו עומק ההורדה הדרוש בדופן הבוקאלית/פציאלית?",
      "options": [
        "א. 2.5 מ\"מ לפחות כדי למנוע חדירת אור. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אין שום צורך בהורדה אקסיאלית מעבר לזגוגית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. 0.5 מ\"מ בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. 1.2 עד 1.5 מ\"מ ליצירת מקום למתכת (0.3-0.5 מ\"מ), אופקר (0.2 מ\"מ) וחרסינה דנטינית (0.7-1.0 מ\"מ)."
      ],
      "correct": "ד. 1.2 עד 1.5 מ\"מ ליצירת מקום למתכת (0.3-0.5 מ\"מ), אופקר (0.2 מ\"מ) וחרסינה דנטינית (0.7-1.0 מ\"מ).",
      "explanation": "לפי שילינבורג (פרק 8), השחזה פציאלית לכתר PFM דורשת הורדה של 1.2-1.5 מ\"מ (או כתף/שמפר עמוק) כדי לאפשר עובי מספק למתכת, לשכבת האופקר ולחרסינה האסתטית מבלי ליצר Overcontouring."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "איזה קו סיום (Finish line) הוא המומלץ ביותר כיום לכתרי זירקוניה מלאה (Monolithic Zirconia) ומתכת מלאה, ומדוע?",
      "options": [
        "א. Chamfer (שמפר עגול) בעומק של 0.5-0.8 מ\"מ, המפחית ריכוז מאמצים ומעניק גבול ברור לטכנאי.",
        "ב. Bevel חריף של 45 מעלות על גבי השורש. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Heavy Shoulder עם זווית 90 מעלות פנימית חדה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Feather edge בלתי מוגדר לכל היקף השן. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. Chamfer (שמפר עגול) בעומק של 0.5-0.8 מ\"מ, המפחית ריכוז מאמצים ומעניק גבול ברור לטכנאי.",
      "explanation": "לפי שילינבורג ורוזנסטיל, קו סיום Chamfer בעומק 0.5-0.8 מ\"מ מתאים לזירקוניה מונולית ולכתרי מתכת מלאים מכיוון שהוא מונע זווית פנימית חדה (אזור ריכוז מאמצים) ומאפשר הסתגלות שוליים קלינית מדויקת."
    },
    {
      "id": 301,
      "topic": "3. עקרונות השחזת שיניים וגאומטריה שיקומית",
      "difficulty": "קל",
      "question": "מהי זווית ההתכנסות הכללית המומלצת (Total Occlusal Convergence - TOC) בספרות הלימוד (שילינבורג / רוזנסטיל) בעת השחזת שן לכתר קבוע?",
      "options": [
        "א. 2 עד 3 מעלות בלבד, כדי למנוע נזק למוך השן. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. 6 מעלות באופן אידיאלי (3 מעלות לכל דופן), כאשר קלינית נפוצה זווית של 10 עד 20 מעלות.",
        "ג. 0 מעלות בדיוק (דפנות מקבילות לחלוטין) ליצירת אחיזה מכנית מקסימלית.",
        "ד. 30 מעלות כדי לאפשר החדרת הכתר ללא חיכוך. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "ב. 6 מעלות באופן אידיאלי (3 מעלות לכל דופן), כאשר קלינית נפוצה זווית של 10 עד 20 מעלות.",
      "explanation": "לפי שילינבורג (פרק 9) ורוזנסטיל (פרק 13), הזווית התיאורטית האידיאלית להשחזה (TOC) היא 6 מעלות (3 מעלות לכל דופן מול שירטוט הציר). במחקרים קליניים נמצא כי הזווית הממוצעת המבוצעת ע\"י רופאים נעה בין 10 ל-20 מעלות."
    },
    {
      "id": 302,
      "topic": "3. עקרונות השחזת שיניים וגאומטריה שיקומית",
      "difficulty": "בינוני",
      "question": "מהו עומק ההפחתה האוקלוזלית (Occlusal Clearance) הנדרש בעת השחזת כתר מתכת-חרסינה (PFM) בתלולית תומכת (Functional Cusp) לעומת תלולית מנחה (Non-functional Cusp)?",
      "options": [
        "א. 3.0 מ\"מ בשתיהן כדי להבטיח עובי חרסינה אסתטי.",
        "ב. 1.5 מ\"מ בתלולית התומכת ו-2.0 מ\"מ בתלולית המנחה עקב כיוון כוחות הסגירה.",
        "ג. 2.0 מ\"מ בתלולית התומכת (סופגת עומס) ו-1.5 מ\"מ בתלולית המנחה.",
        "ד. 1.0 מ\"מ בתלולית תומכת ו-0.5 מ\"מ בתלולית מנחה."
      ],
      "correct": "ג. 2.0 מ\"מ בתלולית התומכת (סופגת עומס) ו-1.5 מ\"מ בתלולית המנחה.",
      "explanation": "לפי שילינבורג (פרק 9), השחזת תלולית תומכת (Functional cusp: בוקאלית בתחתונה, פלטינלית בעליונה) דורשת הפחתה של 2.0 מ\"מ בעוד תלולית מנחה דורשת 1.5 מ\"מ, כדי לספק עובי מספיק לסגסוגת ולחרסינה תחת עומסי הסגירה."
    },
    {
      "id": 303,
      "topic": "3. עקרונות השחזת שיניים וגאומטריה שיקומית",
      "difficulty": "קשה",
      "question": "איזה קו סיום (Margin design) מומלץ בספרות עבור כתר זירקוניה מונוליתית או כתר מתכת מלאה?",
      "options": [
        "א. Bevel של 45 מעלות על גבי אמייל בלבד ללא מעורבות דנטין. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. Feather edge בלתי מוגדר החודר לתוך ה-SULCUS הפריודונטלי.",
        "ג. Shoulder של 2.0 מ\"מ עם זווית ישרה 90 מעלות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Chamfer היקפי מוגדר ברוחב של 0.5 עד 0.8 מ\"מ, המשמר חומר שן ומפחית מאמצי גזירה."
      ],
      "correct": "ד. Chamfer היקפי מוגדר ברוחב של 0.5 עד 0.8 מ\"מ, המשמר חומר שן ומפחית מאמצי גזירה.",
      "explanation": "לפי שילינבורג ורוזנסטיל, קו סיום מסוג Chamfer ברוחב 0.5-0.8 מ\"מ מומלץ לכתרי מתכת וזירקוניה מונוליתית מכיוון שהוא מציע גבול ברור, משמר חומר שן ומפחית ריכוז מאמצים."
    },
    {
      "id": 304,
      "topic": "3. עקרונות השחזת שיניים וגאומטריה שיקומית",
      "difficulty": "מומחה",
      "question": "מהו האפקט הביומכני של ה-Ferrule Effect בשן שעברה טיפול שורש ומשוקמת עם מבנה וכתר?",
      "options": [
        "א. יצירת טבעת דנטין אופקית ואנכית של לפחות 1.5-2.0 מ\"מ גובה ו-1.0 מ\"מ עובי מעל קו הסיום, המפחיתה סיכון לשבר שורש עקב כוחות סגירה.",
        "ב. הידוק היתד (Post) אל תעלה השורש באמצעות צמנט רזין שקוף. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הגדלת עובי החרסינה בחלק האינסיזלי למניעת שברים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מניעת חדירת חיידקים אל תעלות השורש ע\"י אטימה שולית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. יצירת טבעת דנטין אופקית ואנכית של לפחות 1.5-2.0 מ\"מ גובה ו-1.0 מ\"מ עובי מעל קו הסיום, המפחיתה סיכון לשבר שורש עקב כוחות סגירה.",
      "explanation": "לפי רוזנסטיל (פרק 12) ושילינבורג, ה-Ferrule Effect מחייב חומר שן בריא בגובה 1.5-2.0 מ\"מ מעל השוליים. הוא חיוני לעמידות השן בפני כוחות כפיפה ומונע שברי שורש אנכיים."
    }
  ],
  "4": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "איזה חומר מטבע נחשב להידרופילי ביותר באופן טבעי (ללא צורך בהוספת סורפקטנטים)?",
      "options": [
        "א. Condensation Silicone.",
        "ב. Polyether (פוליאתר).",
        "ג. Polysulfide. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Addition Silicone (VPS / PVS)."
      ],
      "correct": "ב. Polyether (פוליאתר).",
      "explanation": "לפי פיליפס (פרק 8) ושילינבורג, פוליאתר (Polyether) הוא חומר מטבע הידרופילי מובהק מטבעו עקב קבוצות האתר בשרשרת הפולימר, בעוד סיליקונים דורשים תוספת סורפקטנט כדי להפחית זווית מגע."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהו תוצר הלוואי שנפלט במהלך תגובת המצליב (Polymerization) בחומר מטבע מסוג Condensation Silicone?",
      "options": [
        "א. אין שום תוצר לוואי (תגובת סיפוח נטו).",
        "ב. גז הידרוגן (Hydrogen gas).",
        "ג. אתנול (Ethyl alcohol).",
        "ד. מים (Water). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. אתנול (Ethyl alcohol).",
      "explanation": "לפי פיליפס, סיליקון בדחיסה (Condensation silicone) פולט אתנול כתוצר לוואי. התאיידות האתנול גורמת להתכווצות מימדית משמעותית לאורך זמן, ולכן יש ליצוק את הגבס מיד."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "מדוע מומלץ להמתין לפחות 30-60 דקות לפני יציקת גבס במטבע מסוג Addition Silicone (PVS) שאינו מכיל Hydrogen Scavenger?",
      "options": [
        "א. כדי לאפשר לחומר להתכווץ ב-5% בחזרה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. כדי שהגבס לא יתחמם יתר על המידה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. כדי לאפשר לחומר להפוך מהידרופילי להידרופובי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. כדי למנוע היווצרות בועות שטח בגבס עקב פליטת גז הידרוגן חופשי בתגובת הצליבה."
      ],
      "correct": "ד. כדי למנוע היווצרות בועות שטח בגבס עקב פליטת גז הידרוגן חופשי בתגובת הצליבה.",
      "explanation": "לפי פיליפס, תגובת המשנה ב-Addition silicone שאינו מכיל קולט הידרוגן משחררת גז הידרוגן. יציקה מיידית גורמת לבועות קטנות וחללים בשטח פנים מודל הגבס."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "כיצד משפיעה ספיגת מים (Water imbibition) על מטבע Polyether אם הוא מאוחסן בסביבה רטובה ממושכת?",
      "options": [
        "א. החומר סופג מים, תופח ועובר שינוי מימדי המעוות את המודל.",
        "ב. החומר מתכווץ עקב איבוד מים לאוויר הפתוח.",
        "ג. החומר נמס לחלוטין והופך לג'ל נוזלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. אין שום השפעה מכיוון שהוא חומר הידרופובי לחלוטין."
      ],
      "correct": "א. החומר סופג מים, תופח ועובר שינוי מימדי המעוות את המודל.",
      "explanation": "לפי פיליפס ושילינבורג, בשל היותו הידרופילי, Polyether סופג מים בסביבה לחה (Imbibition) ועלול להתפשט ולעוות את המטבע, ולכן יש לאחסנו בסביבה יבשה."
    },
    {
      "id": 401,
      "topic": "4. חומרי מטבע וטכניקות הטבעה בשיקום קבוע",
      "difficulty": "קל",
      "question": "איזה חומר מטבע נחשב להידרופילי מבין חומרי המטבע האלסטומריים?",
      "options": [
        "א. Addition Silicone (PVS) ללא תוספת סורפקטנט. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ב. Polyether (פוליאתר), המתאפיין בהידרופיליות גבוהה וקשיחות רבה לאחר התקשות.",
        "ג. Polysulfide (פוליסולפיד) עשיר בריח גופרית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Condensation Silicone (סיליקון בדחיסה). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Polyether (פוליאתר), המתאפיין בהידרופיליות גבוהה וקשיחות רבה לאחר התקשות.",
      "explanation": "לפי פיליפס (פרקים 5-8), פוליאתר (Impregum) הוא חומר הידרופילי טבעי, מה שמאפשר הרטבה טובה של רקמות הפה והגבס, אך מחייב זהירות מספיגת מים (Imbibition) בעת אחסון."
    },
    {
      "id": 402,
      "topic": "4. חומרי מטבע וטכניקות הטבעה בשיקום קבוע",
      "difficulty": "בינוני",
      "question": "מהו תוצר הלוואי שנפלט בעת התקשות Condensation Silicone ומה השפעתו על יציבות המידות?",
      "options": [
        "א. גז מימן (Hydrogen gas), המחייב המתנה של שעה לפני יציקת גבס.",
        "ב. מים, הגורמים להתרחבות המטבע תוך 24 שעות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. אתנול (Ethanol), המתאדה במהירות וגורם להתכווצות המטבע, ולכן יש ליצוק גבס מיד.",
        "ד. חומצה זרחתית, המגרה את הרירית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. אתנול (Ethanol), המתאדה במהירות וגורם להתכווצות המטבע, ולכן יש ליצוק גבס מיד.",
      "explanation": "לפי פיליפס (פרק 8), סיליקון בדחיסה פולט אתנול כתוצר לוואי של תגובת המצליב (Cross-linking). התאדות האתנול גורמת להתכווצות מימדית משמעותית ולכן מחייבת יציקה מיידית."
    },
    {
      "id": 403,
      "topic": "4. חומרי מטבע וטכניקות הטבעה בשיקום קבוע",
      "difficulty": "קשה",
      "question": "מהי הסיבה שבגללה מומלץ להמתין כ-30 עד 60 דקות לפני יציקת גבס במטבע מסוג Addition Silicone (PVS) ללא פלטינום/פלאדיום סורבנט?",
      "options": [
        "א. מניעת דביקות הגבס לחומר המטבע. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. התקררות הכף מטמפרטורת הפה לטמפרטורת החדר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. אפשור ספיגת מים מבוקרת מתוך האוויר להגברת הקשיחות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. סיום תגובת האלסטומר ויצירת גבס ללא בועות אוויר עקב פליטת גז מימן (Secondary reaction)."
      ],
      "correct": "ד. סיום תגובת האלסטומר ויצירת גבס ללא בועות אוויר עקב פליטת גז מימן (Secondary reaction).",
      "explanation": "לפי פיליפס (פרק 8), PVS עשוי להפריש גז מימן כתגובת לוואי של קבוצות Silane הידריד. אם יוצקים גבס מייד, גז המימן יוצר בועות קטנות (Pits) על פני מודל הגבס."
    },
    {
      "id": 404,
      "topic": "4. חומרי מטבע וטכניקות הטבעה בשיקום קבוע",
      "difficulty": "מומחה",
      "question": "איזה חומר סילוק חניכיים (Retraction solution) מבוסס אדרנלין כרוך בסכנה למערכת הלבו-ווקולרית (Epinephrine Racemic Syndrome)?",
      "options": [
        "א. 8% Racemic Epinephrine, העלול לגרום לטכיקרדיה, עליית לחץ דם וחרדה במטופלים רגישים.",
        "ב. Zinc Chloride 1%. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Aluminum Chloride (אלומיניום כלוריד 10%-25%). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Ferric Sulfate (פרייק סולפט 13%-20%). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. 8% Racemic Epinephrine, העלול לגרום לטכיקרדיה, עליית לחץ דם וחרדה במטופלים רגישים.",
      "explanation": "לפי פיליפס ושילינבורג, חוט ספיגה המושרה ב-8% Racemic Epinephrine עלול להיספג סיסטמית ברירית פצועה ולגרום לתופעות לוואי קשות (Epinephrine syndrome) כגון עליית דופק ולחץ דם, ולכן קיימת התווית נגד במטופלי קרדיו."
    }
  ],
  "5": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "איזה צמנט דנטלי מציג קישור כימי ישיר (Chemical adhesion) לזגוגית ולדנטין ע\"י קישור קבוצות קרבוקסיל ליוני סידן?",
      "options": [
        "א. EBA Cement. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Glass Ionomer Cement (GIC) / Polycarboxylate Cement.",
        "ג. Zinc Phosphate Cement. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Zinc Oxide Eugenol (ZOE). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Glass Ionomer Cement (GIC) / Polycarboxylate Cement.",
      "explanation": "לפי פיליפס (פרק 14-15), צמנט פוליקרבוקסילאט ו-Glass Ionomer מכילים חומצה פוליאקרילית הנקשרת כימית ליוני הסידן בהידרוקסיאפטיט של השן."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "איזה רכיב בצמנט Zinc Oxide Eugenol (ZOE) אחראי לעיכוב הפולימריזציה של צמנטים שרפיים וקומפוזיטים?",
      "options": [
        "א. Zinc acetate.",
        "ב. Acetic acid.",
        "ג. Eugenol (אוגנול).",
        "ד. Zinc oxide. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. Eugenol (אוגנול).",
      "explanation": "לפי פיליפס, האוגנול הוא Free radical scavenger המעכב פולימריזציה של שרפים קומפוזיטיים וצמנטים שרפיים, ולכן אסור לשימוש לפני הדבקה שרפית."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "מהי התכונה המכנית הבולטת של זירקוניה מיוצבת איטריום (Y-TZP) המונעת התפשטות סדקים (Transformation Toughening)?",
      "options": [
        "א. התכווצות נפחית של 10% בראש הסדק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אלסטיות גבוהה הדומה לרירית החך. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מעבר פאזה מ-Cubic ל-Amorphous המייצר כוחות מתיחה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מעבר פאזה מ-Tetragonal ל-Monoclinic המלווה בהתרחבות נפחית של כ-3-5% הנגרמת ממאמץ בשיא הסדק."
      ],
      "correct": "ד. מעבר פאזה מ-Tetragonal ל-Monoclinic המלווה בהתרחבות נפחית של כ-3-5% הנגרמת ממאמץ בשיא הסדק.",
      "explanation": "לפי פיליפס ורוזנסטיל, מנגנון ה-Transformation toughening ב-Y-TZP מתבסס על כך שמאמץ בשיא הסדק גורם לשינוי פאזה מטטרגונלית למונוקלינית עם גידול נפחי בשיעור 3-5%, מה שיוצר כוחות לחיצה הסוגרים את הסדק."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "כיצד מתבצעת הכנת השטח הפנימי של שרף סרמי חומצי (זכוכיתי/ליתיום דיסיליקט) לפני הדבקה שרפית (Adhesive luting)?",
      "options": [
        "א. צריבה בחומצה הידרופלואורית (Hydrofluoric acid 5-9%) ולאחריה יישום סילאן (Silane coupling agent).",
        "ב. צריבה בחומצה זרחתית (Phosphoric acid 37%) בלבד ללא סילאן. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. שטיפה במים מזוקקים ללא שום טיפול כימי. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. התזת סילוקסן בלחץ גבוה בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "א. צריבה בחומצה הידרופלואורית (Hydrofluoric acid 5-9%) ולאחריה יישום סילאן (Silane coupling agent).",
      "explanation": "לפי פיליפס ורוזנסטיל, חרסינות זכוכיתיות (Lithium disilicate / Feldspathic) דורשות צריבה ב-HF ליצירת מיקרו-רפורוזיות, ולאחריה Silane לקישור כימי בין המטריצה השרפית לזכוכית."
    },
    {
      "id": 501,
      "topic": "5. צמנטים דנטליים, מתכות וסרמיקות",
      "difficulty": "קל",
      "question": "איזה צמנט מבין הבאים משחרר פלואוריד באופן מתמשך לרקמות השן?",
      "options": [
        "א. Composite Resin Cement (צמנט רזין). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Glass Ionomer Cement (GIC / RMGI), המשחרר יוני פלואוריד ומקשר כימית לזגוגית ולדנטין.",
        "ג. Zinc Phosphate Cement (צמנט אבץ-פוספט). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Zinc Oxide Eugenol (ZOE). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Glass Ionomer Cement (GIC / RMGI), המשחרר יוני פלואוריד ומקשר כימית לזגוגית ולדנטין.",
      "explanation": "לפי פיליפס (פרק 16), גלאס איונומר משחרר פלואוריד המסיק מניעת עששת משנית ובעל קישור כימי (Chelation) ליוני סידן באמייל ובדנטין."
    },
    {
      "id": 502,
      "topic": "5. צמנטים דנטליים, מתכות וסרמיקות",
      "difficulty": "בינוני",
      "question": "מהו מנגנון הקישור הראשי של צמנט Zinc Phosphate לשן ולשיקום?",
      "options": [
        "א. אינטגרציה אופטית ואטימה הידראולית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. קישור קובלנטי חזק לקולגן בדנטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. אחיזה מכנית בלבד (Mechanical interlocking) לתוך אי-סדירויות המשטח המשוחז והשיקום.",
        "ד. קישור אידיאלי המבוסס על חומצה מתקרילית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. אחיזה מכנית בלבד (Mechanical interlocking) לתוך אי-סדירויות המשטח המשוחז והשיקום.",
      "explanation": "לפי פיליפס (פרק 16), אבץ פוספט אינו יוצר קישור כימי לשן או למתכת, אלא מסתמך באופן מוחלט על Mechanical interlocking (מיקרו-אחיזה מכנית)."
    },
    {
      "id": 503,
      "topic": "5. צמנטים דנטליים, מתכות וסרמיקות",
      "difficulty": "קשה",
      "question": "מהו מנגנון ה-Transformation Toughening המעניק לזירקוניה (Y-TZP) את חוזקה המכני הגבוה למניעת התפשטות סדקים?",
      "options": [
        "א. גידול של קריסטלים מסוג Leucite בתוך המטריצה הזכוכיתית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. המסה חלקית של פאזת הזכוכית ע\"י חומצה הידרופלואורית (HF). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מעבר פאזה מקריסטל מונוקליני לפאזה טטרגונלית תוך התכווצות נפח. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מעבר פאזה מיוזם מאמץ מפאזה טטרגונלית (T-phase) לפאזה מונוקלינית (M-phase) באזור קצה הסדק, המלווה בהתרחבות נפחית של 3-5% אשר 'פוקקת' ונועלת את הסדק."
      ],
      "correct": "ד. מעבר פאזה מיוזם מאמץ מפאזה טטרגונלית (T-phase) לפאזה מונוקלינית (M-phase) באזור קצה הסדק, המלווה בהתרחבות נפחית של 3-5% אשר 'פוקקת' ונועלת את הסדק.",
      "explanation": "לפי פיליפס (פרק 21) ורוזנסטיל (פרק 23), Transformation toughening בזירקוניה Y-TZP מתרחש כאשר מאמץ בקצה סדק גורם למעבר מפאזה טטרגונלית למונוקלינית. התרחבות הנפח (3-5%) יוצרת מאמצי לחיצה הנועלים את הסדק מלהתפשט."
    },
    {
      "id": 504,
      "topic": "5. צמנטים דנטליים, מתכות וסרמיקות",
      "difficulty": "מומחה",
      "question": "איזו סגסוגת דנטלית מוגדרת כ-High Noble Alloy על פי ה-ADA (American Dental Association)?",
      "options": [
        "א. סגסוגת המכילה תכולת מתכות אצילות (Noble metals: Au, Pt, Pd) של 60% לפחות מתוכם לפחות 40% זהב (Au).",
        "ב. סגסוגת מבוססת ניקל-כרום (Ni-Cr) ללא זהב. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ג. טיטניום דרגה 4 טהור ללא מתכות נוספות. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. סגסוגת המכילה לפחות 25% מתכות אצילות בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "א. סגסוגת המכילה תכולת מתכות אצילות (Noble metals: Au, Pt, Pd) של 60% לפחות מתוכם לפחות 40% זהב (Au).",
      "explanation": "סיווג ה-ADA מגדיר High Noble כסגסוגת בעלת תכולת מתכות אצילות של 60% ומעלה, כשמתוכם לפחות 40% חייב להיות זהב (Au)."
    }
  ],
  "6": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהו התפקיד המרכזי של מחבר ראשי (Major Connector) בתותבת חלקית נשלפת (RPD)?",
      "options": [
        "א. ספיגת כל כוחות האחיזה הבלעדיים בשיניים הקדמיות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. איחוד וקישור כל רכיבי התותבת מצד אחד לצד השני ויצירת קשיחות (Rigidity) ופיזור עומסים.",
        "ג. ייצור מגעים נקודתיים בלבד ברקמה הרכה. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. הגדלת המימד האנכי של הסגר (VDO). כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר."
      ],
      "correct": "ב. איחוד וקישור כל רכיבי התותבת מצד אחד לצד השני ויצירת קשיחות (Rigidity) ופיזור עומסים.",
      "explanation": "לפי מקרקן (פרק 3-5) וזארב, המחבר הראשי קושר את רכיבי התותבת מצד אחד לצד השני. תכונתו הקריטית ביותר היא קשיחות (Rigidity) כדי לפזר כוחות באופן שווה על הרכסים והשיניים."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהו המרחק המינימלי הנדרש בין הגבול העליון של מחבר ראשי מנדיבולרי מסוג Lingual Bar לבין שולי החניכיים החופשיים (Free gingival margin)?",
      "options": [
        "א. אין שום דרישת מרחק – מותר להניחו ישירות על החניכיים החופשיות.",
        "ב. 8 עד 10 מ\"מ.",
        "ג. 3 עד 4 מ\"מ.",
        "ד. 1 מ\"מ בלבד."
      ],
      "correct": "ג. 3 עד 4 מ\"מ.",
      "explanation": "לפי מקרקן, הגבול העליון של Lingual Bar חייב להיות במרחק של לפחות 3-4 מ\"מ משולי החניכיים כדי למנוע גירוי, איסכמיה ופגיעה פריודונטלית. עובי הבר עצמו הוא 4-5 מ\"מ (סה\"כ דרוש עומק רצפת פה של 7-8 מ\"מ)."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "במצב של עומק רצפת פה מוגבל (פחות מ-7 מ\"מ ממרחק החניכיים לרצפת הפה), איזה מחבר ראשי מנדיבולרי מותווה לשימוש?",
      "options": [
        "א. Labial Bar בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. Continuous Bar בלבד ללא תמיכה.",
        "ג. Lingual Bar סטנדרטי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Lingual Plate (פלטה לינגואלית)."
      ],
      "correct": "ד. Lingual Plate (פלטה לינגואלית).",
      "explanation": "לפי מקרקן, כאשר עומק רצפת הפה קטן מ-7-8 מ\"מ או כאשר יש טורוס לינגואלי/ספיגה קשה, Lingual Bar מנוע ומתווה שימוש ב-Lingual Plate המכסה את המשטחים הלינגואליים של השיניים הקדמיות."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "איזה מחבר ראשי מקסילרי נחשב לקשיח ביותר ולבעל פיזור העומסים הפיזיולוגי הטוב ביותר בשיקומים נרחבים (Class I / Class II)?",
      "options": [
        "א. Complete Palatal Plate / Anteroposterior Palatal Strap.",
        "ב. U-Shaped Palatal Connector (Horse-shoe).",
        "ג. Single Palatal Bar דק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Anterior Palatal Strap בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "א. Complete Palatal Plate / Anteroposterior Palatal Strap.",
      "explanation": "לפי מקרקן, פלטה חכית שלמה (Complete Palatal Plate) או AP Palatal Strap מעניקות את הקשיחות המרבית ופיזור עומסים אופטימלי, בעוד U-shaped (Horseshoe) נחשב למחבר הכי פחות קשיח ונוטה להתכופפות תחת עומס."
    },
    {
      "id": 601,
      "topic": "6. תותבות חלקיות נשלפות (RPD): ביומכניקה ומחברים",
      "difficulty": "קל",
      "question": "מה מנחה את סיווג קנדי (Kennedy Classification) לתותבות חלקיות נשלפות (RPD)?",
      "options": [
        "א. סוג הסגסוגת ממנה מיוצר המחבר המז'ורי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. המיקום והאופי של האזורים מחוסרי השיניים (Edentulous areas), כאשר האזור האחורי ביותר מכתיב את הסיווג הראשי.",
        "ג. עומק החך הקשה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מספר הכתרים הקיימים בפה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. המיקום והאופי של האזורים מחוסרי השיניים (Edentulous areas), כאשר האזור האחורי ביותר מכתיב את הסיווג הראשי.",
      "explanation": "לפי מקרקן (פרק 3), סיווג קנדי מבוסס על האזורים מחוסרי השיניים, כאשר לפי חוקי Applegate האזור מחוסר השיניים האחורי ביותר הוא שמحدد את הסיווג הראשי (Class I-IV)."
    },
    {
      "id": 602,
      "topic": "6. תותבות חלקיות נשלפות (RPD): ביומכניקה ומחברים",
      "difficulty": "בינוני",
      "question": "מהו המרווח האנכי המינימלי הנדרש בין השוליים הג Gingival Margin של השיניים התחתונות לבין הרצפה בעת תכנון Lingual Bar כמחבר מז'ורי מנדיבולרי?",
      "options": [
        "א. 1 מ\"מ בלבד אם מדובר במחבר מסוג Lingual Plate. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. 15 מ\"מ לפחות כדי למנוע הצטברות אבנית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. 8 מ\"מ בסך הכל (3 מ\"מ מרווח מהחניכיים החופשיות + 5 מ\"מ רוחב הבר עצמו).",
        "ד. 3 מ\"מ בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. 8 מ\"מ בסך הכל (3 מ\"מ מרווח מהחניכיים החופשיות + 5 מ\"מ רוחב הבר עצמו).",
      "explanation": "לפי מקרקן (פרק 5), Lingual bar דורש לפחות 8 מ\"מ של מרווח אנכי: 3 מ\"מ שולי בטיחות משולי החניכיים ו-5 מ\"מ רוחב עבור הבר עצמו. אם המרווח קטן מ-8 מ\"מ, מתוכנן Lingual Plate."
    },
    {
      "id": 603,
      "topic": "6. תותבות חלקיות נשלפות (RPD): ביומכניקה ומחברים",
      "difficulty": "קשה",
      "question": "מה תפקידו של המאחז הישיר המשני / המאחז הבלתי-ישיר (Indirect Retaining Element) בתותבת נשלפת מסוג Kennedy Class I או II?",
      "options": [
        "א. הגדלת כוח הלעיסה על השיניים הקדמיות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מניעת שחיקה של השיניים המאחזות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. חיבור בין המחבר המז'ורי למחבר המינורי בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. מניעת התרוממות בסיס התותבות מחוסר השיניים (Distal extension base) הרחק מהרכס סביב קו הציר/הפולקרום (Fulcrum line) בעת אכילת מאכלים דביקים."
      ],
      "correct": "ד. מניעת התרוממות בסיס התותבות מחוסר השיניים (Distal extension base) הרחק מהרכס סביב קו הציר/הפולקרום (Fulcrum line) בעת אכילת מאכלים דביקים.",
      "explanation": "לפי מקרקן (פרק 8), המאחז הבלתי ישיר ממוקם בצד הנגדי לקו הפולקרום ומונע רוטציה והתרוממות של הבסיס המשתרע דיסטלית (Distal extension) הרחק מהרקמה."
    },
    {
      "id": 604,
      "topic": "6. תותבות חלקיות נשלפות (RPD): ביומכניקה ומחברים",
      "difficulty": "מומחה",
      "question": "איזה מחבר מז'ורי מקסילרי נחשב לקשיח ביותר (Most Rigid) ומומלץ במקרים של Kennedy Class I עם רכסים ספוגים מאוד או חך קשה שטוח?",
      "options": [
        "א. Full Palatal Plate (פלטה חכית מלאה) או Anterior-Posterior Palatal Strap רחב.",
        "ב. U-Shaped (Horseshoe) Palatal Connector. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Anterior Palatal Bar בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. Single Palatal Strap צר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. Full Palatal Plate (פלטה חכית מלאה) או Anterior-Posterior Palatal Strap רחב.",
      "explanation": "לפי מקרקן (פרק 5), Full Palatal Plate מציעה את הקשיחות הגבוהה ביותר ופיזור עומסים מקסימלי (Mucosal support) לרכסים ספוגים. U-shaped connector הוא הפחות קשיח מביניהם."
    }
  ],
  "7": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהם שלושת הרכיבים המרכיבים את קומפלקס ה-RPI בתותבת חלקית נשלפת?",
      "options": [
        "א. Retentive arm, Post, Inlay. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Rest (מסעד מסיאלי), Proximal plate (פלטה פרוקסימלית דיסטלית), I-bar (זרוע אחיזה היקפית I).",
        "ג. Reciprocal arm, Pinhole, Impression tray. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Ring clasp, Pontic, Implant. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Rest (מסעד מסיאלי), Proximal plate (פלטה פרוקסימלית דיסטלית), I-bar (זרוע אחיזה היקפית I).",
      "explanation": "לפי מקרקן (פרק 6) וזארב, RPI מורכב מ: Rest מסיאלי, Proximal plate במישור ההדרכה הדיסטלי, וזרוע אחיזה מסוג I-bar בשיא הקעירות הבוקאלית."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהי המשמעות הביומכנית של מיקום ה-Rest בצד המסיאלי (Mesial Rest) בקומפלקס RPI בתותבת מסוג Class I / Class II (שלוחה חופשית)?",
      "options": [
        "א. השן המאחזת נמשכת דיסטלית בחוזה וסובלת מטראומה אוקלוזלית קשה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אין שום הבדל בין Rest מסיאלי לדיסטלי בשלוחה חופשית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. בעת עומס סגרי על הבסיס, השן המאחזת מסתובבת מסיאלית (קדימה) אל עבר השן השכנה, וה-I-bar זז אנטרו-אפיקלית ומשתחרר מהשן.",
        "ד. ה-I-bar ננעל עוד יותר על השן ומפעיל כוח עקירה זוויתי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. בעת עומס סגרי על הבסיס, השן המאחזת מסתובבת מסיאלית (קדימה) אל עבר השן השכנה, וה-I-bar זז אנטרו-אפיקלית ומשתחרר מהשן.",
      "explanation": "לפי מקרקן וזארב (Kratochvil / Kratochvil-Arstad / Berg), Rest מסיאלי מעביר את ציר הסיבוב קדימה. בעת שקילת התותבת, ה-I-bar והפלטה הפרוקסימלית נעים אל אזור השחרור (Relief) ואינם מפעילים כוח מנוף הרסני על השן."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "מהו התפקיד המרכזי של מישורי הדרכה (Guiding Planes) שהוכנו על גבי השיניים המאחזות?",
      "options": [
        "א. מניעת הצטברות אבנית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מניעת צורך במסעדים אוקלוזליים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הגדלת האחיזה החיכוך ב-80% ללא צורך בזרועות אחיזה. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. הקשרת מסלול ההכנסה וההוצאה (Path of insertion) של התותבת והבטחת הדדיות (Reciprocity)."
      ],
      "correct": "ד. הקשרת מסלול ההכנסה וההוצאה (Path of insertion) של התותבת והבטחת הדדיות (Reciprocity).",
      "explanation": "לפי מקרקן, מישורי הדרכה מקבילים מכתיבים מסלול הכנסה והוצאה אחיד, משפרים יציבות, מונעים כוחות רוחביים על השיניים ומבטיחים פונקציית Reciprocity תקינה."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "באילו תנאים אנטומיים מנוע (Contraindicated) שימוש בזרוע אחיזה מסוג I-bar בקומפלקס RPI?",
      "options": [
        "א. כאשר קיימת קעירות שרשית/רירית עמוקה (Tissue Undercut) של מעל 2 מ\"מ במרחק 3 מ\"מ משולי החניכיים, או חסר Vestibule.",
        "ב. כאשר יש מגע אוקלוזלי ב-MI. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. כאשר השיקום מבוצע במקסילה בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. כאשר השן המאחזת היא ניב תחתון בריא. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. כאשר קיימת קעירות שרשית/רירית עמוקה (Tissue Undercut) של מעל 2 מ\"מ במרחק 3 מ\"מ משולי החניכיים, או חסר Vestibule.",
      "explanation": "לפי מקרקן, I-bar מנוע כאשר יש Tissue undercut עמוק (אז ה-I-bar יבלוט ויפצע את הלחי/שפה או יילכד מזון) או בעומק וסטיבולרי נמוך מ-4 מ\"מ."
    },
    {
      "id": 701,
      "topic": "7. RPD: מאחזים, מסעדים וקומפלקס RPI",
      "difficulty": "קל",
      "question": "ממה מורכב קומפלקס ה-RPI המיועד לשן מאחזת הסמוכה לאזור מחוסר שיניים דיסטלי (Distal Extension)?",
      "options": [
        "א. Ring clasp, Palatal rest, I-bar. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Rest מסיאלי (Rest mesial), Proximal plate דיסטלי, וזרוע I-bar הממוקמת בערעור תחתון (Undercut) בוקאלי-מסיאלי או במרכז השן.",
        "ג. Reciprocal arm, Proximal plate, I-bar. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Rest דיסטלי, Proximal plate מסיאלי, I-bar בוקאלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Rest מסיאלי (Rest mesial), Proximal plate דיסטלי, וזרוע I-bar הממוקמת בערעור תחתון (Undercut) בוקאלי-מסיאלי או במרכז השן.",
      "explanation": "לפי מקרקן (פרק 6) וזארב, RPI מורכב מ: Rest מסיאלי, Proximal plate דיסטלי, ו-I-bar בוקאלי. תכנון זה משחרר עומסים מהשן המאחזת בעת שקיעת התותבת על הרקמה הרכה."
    },
    {
      "id": 702,
      "topic": "7. RPD: מאחזים, מסעדים וקומפלקס RPI",
      "difficulty": "בינוני",
      "question": "מהן הדרישות האנטומיות להכנת בית מסעד אוקלוזלי (Occlusal Rest Seat) בשן אחורית לפי מקרקן?",
      "options": [
        "א. קילוף אמייל מלא במשטח הבוקאלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. חריץ שטוח של 0.5 מ\"מ על גבי אמייל בלבד ללא שיפוע. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. צורה כפיתית (Spoon-shaped) קעורה, בעומק 1.0-1.5 מ\"מ, ברוחב 1/2 המרחק הבין-תלוליתי, עם זווית קטנה מ-90 מעלות בין המסעד למחבר המינורי.",
        "ד. צורה מלבנית עמוקה של 3 מ\"מ עם זווית חדה של 110 מעלות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. צורה כפיתית (Spoon-shaped) קעורה, בעומק 1.0-1.5 מ\"מ, ברוחב 1/2 המרחק הבין-תלוליתי, עם זווית קטנה מ-90 מעלות בין המסעד למחבר המינורי.",
      "explanation": "לפי מקרקן (פרק 6), בית מסעד אוקלוזלי מיוצר בצורת כפית קעורה בעומק 1-1.5 מ\"מ והזווית בין המחבר המינורי למסעד חייבת להיות קטנה מ-90 מעלות כדי לכוון את הכוחות לאורך הציר האורך של השן."
    },
    {
      "id": 703,
      "topic": "7. RPD: מאחזים, מסעדים וקומפלקס RPI",
      "difficulty": "קשה",
      "question": "מהי התוויית הנגד המרכזית (Contraindication) לשימוש בזרוע אחיזה מסוג I-bar?",
      "options": [
        "א. חוסר שיניים מסוג Class III. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. שן המאחזת היא מולר עליון. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מציאות Undercut מסיאלי בשן המאחזת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. קיום Undercut רמה/עמוקה ברקמה הרכה (Severe tissue undercut) או פרוזדור פה (Vestibule) רדוד מ-3 מ\"מ אפיקלית לשולי החניכיים."
      ],
      "correct": "ד. קיום Undercut רמה/עמוקה ברקמה הרכה (Severe tissue undercut) או פרוזדור פה (Vestibule) רדוד מ-3 מ\"מ אפיקלית לשולי החניכיים.",
      "explanation": "לפי מקרקן (פרק 6), עקוף תאית/תלולית רקמה רכה (Tissue undercut) מתחת לשן המאחזת או וסטיבולום רדוד מונעים מעבר תקין של זרוע ה-I-bar וגורמים לפציעת הרירית. במצב זה מעדיפים RPA (Wrought wire)."
    },
    {
      "id": 704,
      "topic": "7. RPD: מאחזים, מסעדים וקומפלקס RPI",
      "difficulty": "מומחה",
      "question": "מה תפקידו של מישור ההדרכה (Guiding Plane) שמוכן על המשטחים הפרוקסימליים של השיניים המאחזות?",
      "options": [
        "א. יצירת אחיזה חיכוכית, הכוונת נתיב ההכנסה וההוצאה (Path of insertion) של התותבת, והקטנת מרווחים מתים (Dead spaces).",
        "ב. החלפת התפקיד של המאחז הבלתי ישיר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מניעת שחיקה אוקלוזלית של המסיעים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הגדלת התנועתיות האופקית של התותבת. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר."
      ],
      "correct": "א. יצירת אחיזה חיכוכית, הכוונת נתיב ההכנסה וההוצאה (Path of insertion) של התותבת, והקטנת מרווחים מתים (Dead spaces).",
      "explanation": "לפי מקרקן (פרק 11), מישורי הדרכה מקבילים מכוונים את נתיב ההכנסה/הוצאה, מעניקים יציבות אופקית (Reciprocation), ומפחיתים לכידת מזון."
    }
  ],
  "8": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "איזה מבנה אנטומי במנדיבולה נחשב לאזור התמיכה הראשוני (Primary Stress Bearing Area) בתותבת שלמה תחתאנית?",
      "options": [
        "א. Sublingual fossa. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Buccal Shelf (המדף הבוקאלי).",
        "ג. Sharp crest of the residual ridge (שיא הרכס השארי).",
        "ד. Mylohyoid ridge. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Buccal Shelf (המדף הבוקאלי).",
      "explanation": "לפי זארב (פרק 16) ונטר (Boucher), המדף הבוקאלי (Buccal Shelf) מורכב מעצם דחוסה הקבילה לכוחות מאונכים, והוא משמש אזור תמיכה ראשוני במנדיבולה."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהו הגבול האחורי המרכזי של תותבת שלמה עליונה המשמש ליצירת איטום אחורי (Posterior Palatal Seal / PPS)?",
      "options": [
        "א. Incisive papilla. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Rugae palatinae. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Vibrating line (הקו הרוטט) באזור המעבר בין החך הקשה לחך הרך.",
        "ד. Fovea palatina בכיפה המרכזית בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. Vibrating line (הקו הרוטט) באזור המעבר בין החך הקשה לחך הרך.",
      "explanation": "לפי זארב ונטר, ה-Vibrating line הוא קו אנטומי-פונקציונלי המציין את תחילת תנועת החך הרך. עליו ומעט אנטריורית לו ממוקם ה-Posterior Palatal Seal ליצירת איטום היקפי ושמירת אחיזה (Retention)."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "מהו התפקיד הביולוגי והקליני של הגבול האנטומי Retromolar Pad במטופל חסר שיניים מוחלט?",
      "options": [
        "א. עובר ספיגה עצמית מהירה תוך חודש מעקירת השיניים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אסור לכסותו בתותבת מחשש לנזק עצבי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. יש להשחיזו קלינית למניעת מגע בתותבת העליונה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. משמש גבול אחורי קריטי לתותבת התחתונה, מכיל סיבי שרירים ובלוטות, ואינו עובר ספיגת עצם משמעותית."
      ],
      "correct": "ד. משמש גבול אחורי קריטי לתותבת התחתונה, מכיל סיבי שרירים ובלוטות, ואינו עובר ספיגת עצם משמעותית.",
      "explanation": "לפי זארב ונטר, ה-Retromolar pad מכיל את ה-Pterygomandibular raphe, סיבי בוקסינטור וקונסטריקטור עליון. הוא יציב מבחינה עצמית ומהווה נקודת ייחוס קריטית לקביעת מישור הסגר והגבול האחורי."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "איזה שריר מגביל את עומק והיקף הגבול הלינגואלי של תותבת שלמה תחתונה באזור ה-Mylohyoid Ridge בעת בליעה ובלטת לשון?",
      "options": [
        "א. Mylohyoid Muscle.",
        "ב. Superior Pharyngeal Constrictor.",
        "ג. Genioglossus Muscle.",
        "ד. Masseter Muscle."
      ],
      "correct": "א. Mylohyoid Muscle.",
      "explanation": "לפי זארב ונטר, שריר ה-Mylohyoid מהווה את רצפת הפה. בעת התכווצות ובליעה הוא מתרומם ומכתיב את אורך ה-Lingual flange של התותבת באזור הלינגואלי-אמצעי."
    },
    {
      "id": 801,
      "topic": "8. תותבות שלמות: גבולות אנטומיים ואזורי תמיכה",
      "difficulty": "קל",
      "question": "איזה מבנה אנטומי בלסת התחתונה נחשב לאזור התמיכה הראשוני (Primary Stress-Bearing Area) בתותבת שלמה מנדיבולרית?",
      "options": [
        "א. Alveolar ridge crest (שיא הרכס האלבאולרי) בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. Buccal Shelf (מדף בוקאלי), עקב מבנה עצם קורטיקלית דחוסה הניצבת לכוחות הסגירה.",
        "ג. Lingual frenum. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Sublingual gland. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Buccal Shelf (מדף בוקאלי), עקב מבנה עצם קורטיקלית דחוסה הניצבת לכוחות הסגירה.",
      "explanation": "לפי זארב (פרקים 1-6) ובאוצ'ר, ה-Buccal Shelf מורכב מעצם קורטיקלית עבה והוא ניצב לכוחות הלעיסה האנכיים, ולכן מהווה את אזור התמיכה הראשי בתותבת תחתונה."
    },
    {
      "id": 802,
      "topic": "8. תותבות שלמות: גבולות אנטומיים ואזורי תמיכה",
      "difficulty": "בינוני",
      "question": "מהו ה-Vibrating Line (AH-Line) בלסת העליונה ומה חשיבותו הקלינית בתותבות שלמות?",
      "options": [
        "א. שולי החצוצרה השמעתית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הקו המפריד בין הציפוי הגרמי של החך הקשה לבין הרכס האלבאולרי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. קו דמיוני העובר בחך הרך ומפריד בין רקמה ניידת לרקמה בלתי ניידת בעת אמירת 'אה', והוא קובע את הגבול האחורי של התותבת (Posterior Palatal Seal / Post-Dam).",
        "ד. הקו המחבר בין שתי ה-Incisive papillae. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. קו דמיוני העובר בחך הרך ומפריד בין רקמה ניידת לרקמה בלתי ניידת בעת אמירת 'אה', והוא קובע את הגבול האחורי של התותבת (Posterior Palatal Seal / Post-Dam).",
      "explanation": "לפי זארב (פרק 14) ונשר/נטר, ה-Vibrating line ממוקם בחך הרך ומסמן את גבול ה-Post-dam המבטיח אטימה היקפית (Peripheral seal) ואחיזה לתותבת העליונה."
    },
    {
      "id": 803,
      "topic": "8. תותבות שלמות: גבולות אנטומיים ואזורי תמיכה",
      "difficulty": "קשה",
      "question": "איזה אזור אנטומי בלסת העליונה חייב לעבור שיחרור (Relief Area) בתותבת השלמה כדי למנוע כאב, צריבה או הפרעה לאספקת הדם והעצבים?",
      "options": [
        "א. Rugae palatinae. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Tuberosity. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Buccal frenum. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Incisive Papilla (פיפטת החותכות), עקב מעבר העצב והכלי דם ה-Nasopalatine תחתיה."
      ],
      "correct": "ד. Incisive Papilla (פיפטת החותכות), עקב מעבר העצב והכלי דם ה-Nasopalatine תחתיה.",
      "explanation": "לפי זארב, ה-Incisive Papilla מכסה את ה-Nasopalatine foramen. ספיגת רכס קשה מביאה אותה לשיא הרכס, וללא Relief עומס התותבת יגרום לתחושת צריבה וכאב."
    },
    {
      "id": 804,
      "topic": "8. תותבות שלמות: גבולות אנטומיים ואזורי תמיכה",
      "difficulty": "מומחה",
      "question": "מה קובע את הגבול הדיסטו-לינגואלי של התותבת התחתונה (Retromylohyoid Curtain / Neil's Lateral Throat Form)?",
      "options": [
        "א. פעולת שריר ה-Superior Constrictor Pharyngis, שריר ה-Palatoglossus, ושריר ה-Mylohyoid בעת בליעה והנעת הלשון.",
        "ב. מיקום ה-Mental foramen. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. שריר ה-Masseter בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. גודל ה-Retromolar pad בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "א. פעולת שריר ה-Superior Constrictor Pharyngis, שריר ה-Palatoglossus, ושריר ה-Mylohyoid בעת בליעה והנעת הלשון.",
      "explanation": "לפי זארב (פרק 6) ובאוצ'ר, אזור הרטרו-מיילוהיואיד מוגבל ע\"י ה-Palatoglossus קדימה, ה-Superior Constrictor אחורה, וה-Mylohyoid למטה. עיצוב שוליים מדויק (Border molding) חיוני לאיחזת התותבת."
    }
  ],
  "9": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהו המאפיין המרכזי של אוקלוזיה מאוזנת (Bilateral Balanced Occlusion) בתותבות שלמות?",
      "options": [
        "א. מגע בשיניים הטוחנות בלבד ב-Centric Relation. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. מגעים אוקלוזליים סימולטניים ורציפים בכל השיניים הקדמיות והאחוריות בצד העובד והלא-עובד בעת תנועות גלישה.",
        "ג. Disclusion מלא של השיניים האחוריות בתנועה לטרלית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מגעים בניבים בלבד בלטרליזציה. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. מגעים אוקלוזליים סימולטניים ורציפים בכל השיניים הקדמיות והאחוריות בצד העובד והלא-עובד בעת תנועות גלישה.",
      "explanation": "לפי זארב (פרק 18) ונטר, סגר מאוזן דו-צדדי מיועד למנוע הטיות והתנתקות של בסיסי התותבות השלמות בעת תנועות לעיסה וגלישה אקסצנטריות."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "כיצד מוגדר ה-Vertical Dimension of Rest (VDR) ביחס ל-Vertical Dimension of Occlusion (VDO)?",
      "options": [
        "א. VDR ו-VDO זהים לחלוטין ללא שום מרווח. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ב. VDR משתנה ב-10 מ\"מ בעת סגירת המנשך. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. VDR גדול ב-2 עד 4 מ\"מ מ-VDO (כאשר ההפרש הוא ה-Interocclusal Rest Space / Freeway Space).",
        "ד. VDO גדול ב-5 מ\"מ מ-VDR. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. VDR גדול ב-2 עד 4 מ\"מ מ-VDO (כאשר ההפרש הוא ה-Interocclusal Rest Space / Freeway Space).",
      "explanation": "לפי זארב ונטר, עמדת המנוחה (VDR) גבוהה יותר מעמדת הסגר (VDO). המרווח הבין-סגרי במנוחה (Freeway space) נע בדרך כלל בין 2 ל-4 מ\"מ. VDO = VDR - Freeway Space."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "איזה סימפטום קליני אופייני יופיע אצל מטופל עם תותבות שלמות שבו נקבע VDO מופרז (Overestimated VDO / מנשך מורם)?",
      "options": [
        "א. חוסר יכולת ללסוס מזון עקב חופש מופרז. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מראה פנים זקן עם קמטים בזוויות הפה (Angular cheilitis).",
        "ג. נשירת התותבת העליונה בעת חיוך. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. נקישת שיניים בעת דיבור (Clicking teeth), עייפות שרירי לעיסה וספיגת עצם מואצת."
      ],
      "correct": "ד. נקישת שיניים בעת דיבור (Clicking teeth), עייפות שרירי לעיסה וספיגת עצם מואצת.",
      "explanation": "לפי זארב, VDO מופרז מבטל את ה-Freeway space. כתוצאה מכך השיניים נוקשות בדיבור, שרירי הלעיסה סובלים ממאמץ מתמיד והרכסים השאריים נחשפים לעומס לחיצה הֶרסני."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "מהי הגישה האוקלוזלית מסוג Lingualized Occlusion בתותבות שלמות, ולאילו מטופלים היא מומלצת?",
      "options": [
        "א. תלוליות לינגואליות בלבד של השיניים העליונות עומדות במגע עם הפוסה המרכזית של התחתונות; מומלצת לרכסים ספוגים קשה.",
        "ב. תלוליות בוקאליות בלבד עומדות במגע. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. שימוש בשיניים שטוחות לחלוטין (Monoplane 0 degrees) ללא שום הדרכה. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. סגר מסוג Class III עם מוצלבת מלאה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. תלוליות לינגואליות בלבד של השיניים העליונות עומדות במגע עם הפוסה המרכזית של התחתונות; מומלצת לרכסים ספוגים קשה.",
      "explanation": "לפי זארב, Lingualized Occlusion משלבת את היתרונות האסתטיים של שיניים אנטומיות עליונות עם היתרונות הביומכניים של ריכוז כוחות אנכי במרכז הרכס התחתון, והיא מומלצת במקרים של רכסים ספוגים קשה."
    },
    {
      "id": 901,
      "topic": "9. תותבות שלמות: אוקלוזיה, מימד אנכי ויחס בין-לסתי",
      "difficulty": "קל",
      "question": "מהו המונח המגדיר סגר מאוזן (Balanced Occlusion) בתותבות שלמות?",
      "options": [
        "א. מגעים בלעדיים בניבים בעת תנועה לטרלית כמו במשנן טבעי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מגעים אוקלוזליים בשני צדי הקשת (Working and Non-working) וגם באזור הקדמי בו זמנית בכל התנועות האקסצנטריות למניעת התהפכות התותבת.",
        "ג. מגעים קדמיים חזקים בזמן פתיחת פה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. מגעים אחוריים בעמדת MI בלבד ללא מגע אקסצנטרי. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. מגעים אוקלוזליים בשני צדי הקשת (Working and Non-working) וגם באזור הקדמי בו זמנית בכל התנועות האקסצנטריות למניעת התהפכות התותבת.",
      "explanation": "לפי זארב ובאוצ'ר, סגר מאוזן (Balanced occlusion) בתותבות שלמות נועד לשמור על יציבות התותבות ברקמה ע\"י מגעים סימולטניים דו-צדיים בכל תנועה לטרלית ופרוטרוזיבית."
    },
    {
      "id": 902,
      "topic": "9. תותבות שלמות: אוקלוזיה, מימד אנכי ויחס בין-לסתי",
      "difficulty": "בינוני",
      "question": "מהו המרווח החופשי במנוחה (Freeway Space / Interocclusal Rest Space) ומהו ערכו התקין במטופל?",
      "options": [
        "א. המרחק בין החותכות בעת דיבור שרקות, וערכו 10 מ\"מ.",
        "ב. עובי בסיס השעווה ברשם המנשך, וערכו 0.5 מ\"מ.",
        "ג. ההפרש בין VDO ל-VDR, וערכו התקין נע בין 2 ל-4 מ\"מ.",
        "ד. המרחק בין הקונדילים לפוסה בעת פתיחה מקסימלית."
      ],
      "correct": "ג. ההפרש בין VDO ל-VDR, וערכו התקין נע בין 2 ל-4 מ\"מ.",
      "explanation": "Freeway Space מתקבל מהנוסחה: VDR - VDO = 2-4 mm. שמירה על מרווח זה חיונית למנוחת שרירי הלעיסה ולמניעת ספיגת עצם מואצת."
    },
    {
      "id": 903,
      "topic": "9. תותבות שלמות: אוקלוזיה, מימד אנכי ויחס בין-לסתי",
      "difficulty": "קשה",
      "question": "מהן התופעות הקליניות האופייניות לקביעת מימד אנכי סגורי מופרז (Excessive VDO) בתותבות שלמות?",
      "options": [
        "א. מראה פנים שקוע, דלקת בזוויות הפה (Angular Cheilitis) וירידה ביעילות הלעיסה.",
        "ב. נשיכת לחיים בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. הפרשה מוגברת של רוק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. נקישת שיניים בעת דיבור (Clicking teeth), עייפות שרירים, רגישות ברכס השארי וספיגת עצם מואצת עקב לחץ מתמיד."
      ],
      "correct": "ד. נקישת שיניים בעת דיבור (Clicking teeth), עייפות שרירים, רגישות ברכס השארי וספיגת עצם מואצת עקב לחץ מתמיד.",
      "explanation": "מימד אנכי מופרז (Excessive VDO) מבטל את ה-Freeway space, מה שגורם לשרירים להיות מתוחים, לנקישת שיניים בדיבור וללחץ קבוע המאיץ ספיגת עצם."
    },
    {
      "id": 904,
      "topic": "9. תותבות שלמות: אוקלוזיה, מימד אנכי ויחס בין-לסתי",
      "difficulty": "מומחה",
      "question": "איזה מושג אוקלוזלי בתותבות שלמות מבוסס על תלוליות פלטינליות עליונות בולטות המשתלבות במשטח אוקלוזלי רדוד/שטוח בתחתון (Lingualized Occlusion)?",
      "options": [
        "א. Lingualized Occlusion, המרכזת את כוחות הלעיסה לינגואלית לרכס התחתון ומפחיתה כוחות גזירה אופקיים.",
        "ב. Reverse Articulation. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Organic Occlusion. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Monoplane Occlusion שטוחה לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. Lingualized Occlusion, המרכזת את כוחות הלעיסה לינגואלית לרכס התחתון ומפחיתה כוחות גזירה אופקיים.",
      "explanation": "Lingualized Occlusion משלבת את היתרונות האסתטיים של שיניים אנטומיות בעליון עם ריכוז כוחות אנכיים על שיניים שטוחות/רדודות בתחתון, ומפחיתה כוחות הרסניים על הרכס התחתון."
    }
  ],
  "10": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהו קצב ודפוס ספיגת העצם האלבאולרית (Residual Ridge Resorption) הטיפוסי במקסילה לעומת המנדיבולה לאחר עקירת שיניים?",
      "options": [
        "א. העצם האלבאולרית אינה נספגת כלל לאחר השנה הראשונה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. קצב הספיגה במנדיבולה מהיר פי 4 מאשר במקסילה; במקסילה הספיגה היא צנטריפטלית (כלפי מעלה ופנימה) ובמנדיבולה צנטריפוגלית (כלפי מטה והחוצה).",
        "ג. קצב הספיגה זהה לחלוטין בשתי הלסתות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. המקסילה נספגת החוצה והמנדיבולה פנימה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. קצב הספיגה במנדיבולה מהיר פי 4 מאשר במקסילה; במקסילה הספיגה היא צנטריפטלית (כלפי מעלה ופנימה) ובמנדיבולה צנטריפוגלית (כלפי מטה והחוצה).",
      "explanation": "לפי זארב (Atwood / Tallgren), ספיגת העצם במנדיבולה מהירה פי 4 מאינטנסיביות הספיגה במקסילה. המקסילה הופכת קטנה ומוצרת (Centripetal), בעוד המנדיבולה הופכת רחבה יותר (Centrifugal)."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהי האטיולוגיה המרכזית להתפתחות Denture Stomatitis (דלקת רירית תחת תותבת) בדרגה II/III?",
      "options": [
        "א. חוסר בוויטמין D בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. שימוש מופרז במשחת שיניים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. זיהום פטרייתי מסוג Candida Albicans המשולב עם הרכבה רציפה בלילה והיגיינה לקויה של התותבת.",
        "ד. אלרגיה למתכת השלד בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. זיהום פטרייתי מסוג Candida Albicans המשולב עם הרכבה רציפה בלילה והיגיינה לקויה של התותבת.",
      "explanation": "לפי זארב ונטר (Newton Class I-III), Denture stomatitis נגרמת בעיקר ע\"י שגשוג קנדידה אלביקנס תחת בסיס התותבת, בעיקר במטופלים שאינם מוציאים את התותבת בלילה."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "מה מבין הבאים מתאר נכונה את התופעה הקלינית הנקראת Epulis Fissuratum (Inflammatory Fibrous Hyperplasia)?",
      "options": [
        "א. כיב עמוק הנגרם מחומציות קיבה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. גידול ממאיר של בלוטות הרוק הקטנות בחך. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. פטמה דלקתית במרכז החך הקשה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. שגשוג רקמת חיבור פיברוטית תגובתית בגבול הווסטיבולרי הנגרם מגבול תותבת ארוך או חד מדי."
      ],
      "correct": "ד. שגשוג רקמת חיבור פיברוטית תגובתית בגבול הווסטיבולרי הנגרם מגבול תותבת ארוך או חד מדי.",
      "explanation": "לפי זארב ונטר, Epulis Fissuratum היא תגובה הילפרפלסטית פיברוטית של הרירית הנגרמת מגירוי מכני כרוני של גבול תותבת עודף/חד בפתחת הווסטיבולום."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "מהו סיווג Atwood למבנה הרכס השארי (Order I to Order VI)?",
      "options": [
        "א. סיווג המתאר את שלבי ספיגת הרכס השארי משין קיימת (Order I), רכס גבוה ורחב (Order III) ועד רכס קעור/ספוג לחלוטין (Order VI).",
        "ב. סיווג איכות עצם שתלים D1-D4. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. סיווג סרמיקות דנטליות לפי אחוז הקישור השרפי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. סיווג ניידות שיניים פריודונטליות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. סיווג המתאר את שלבי ספיגת הרכס השארי משין קיימת (Order I), רכס גבוה ורחב (Order III) ועד רכס קעור/ספוג לחלוטין (Order VI).",
      "explanation": "סיווג Atwood (Order I to VI) הוא הסיווג הקלאסי בספרות (זארב/בוצ'ר) המתאר את מורפולוגיית הרכס השארי מציון Order I (לפני עקירה) ועד Order VI (ספיגה שטוחה/קעורה עמוקה)."
    },
    {
      "id": 1001,
      "topic": "10. ספיגת עצם אלבאולרית ופתולוגיות רירית",
      "difficulty": "קל",
      "question": "כיצד משפיע דפוס ספיגת העצם האלבאולרית (Atwood / Tallgren) על היחס בין הלסתות במטופל מחוסר שיניים ממושך?",
      "options": [
        "א. הלסת העליונה והתחתונה נספגות בקצב זהה לחלוטין ללא שינוי בפרופיל. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ב. הלסת העליונה נספגת כלפי מעלה ופנימה (Centripetal), והתחתונה נספגת כלפי מטה והחוצה (Centrifugal), מה שיוצר פרופיל דמוי Class III skeletal.",
        "ג. הלסת התחתונה נספגת פנימה והעליונה החוצה (Class II). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. אין שום ספיגת עצם בלסת העליונה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. הלסת העליונה נספגת כלפי מעלה ופנימה (Centripetal), והתחתונה נספגת כלפי מטה והחוצה (Centrifugal), מה שיוצר פרופיל דמוי Class III skeletal.",
      "explanation": "לפי אטווד וזארב, המקסילה נספגת צנטריפטלית (למעלה ופנימה) והמנדיבולה צנטריפוגלית (למטה והחוצה), מה שגורם להרחבת המאזן המנדיבולרי ופרופיל דמוי Class III."
    },
    {
      "id": 1002,
      "topic": "10. ספיגת עצם אלבאולרית ופתולוגיות רירית",
      "difficulty": "בינוני",
      "question": "מהו הגורם האטיולוגי המרכזי להתפתחות Denture Stomatitis (דלקת רירית תחת תותבת) מסוג Newton Class I-III?",
      "options": [
        "א. חוסר בוויטמין C בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. שימוש מופרז במשחת שיניים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. זיהום פטרייתי מסוג Candida Albicans המשולב עם הרכבת תותבת בלילה, היגיינה לקויה וטראומה מופרזת מהתותבת.",
        "ד. אלרגיה לאקריל בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. זיהום פטרייתי מסוג Candida Albicans המשולב עם הרכבת תותבת בלילה, היגיינה לקויה וטראומה מופרזת מהתותבת.",
      "explanation": "Denture Stomatitis נגרמת בעיקר ע\"י Candida albicans, ומוחמרת ע\"י הרכבה רציפה בלילה, חוסר היגיינה של התותבת וטראומה מכנית."
    },
    {
      "id": 1003,
      "topic": "10. ספיגת עצם אלבאולרית ופתולוגיות רירית",
      "difficulty": "קשה",
      "question": "מהי הפתולוגיה הרירית הידועה כ-Epulis Fissuratum וכיצד נוהגים לטפל בה?",
      "options": [
        "א. גידול ממאיר בחך הדורש כריתה נרחבת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. דלקת חריפה בעצם האלבאולרית הדורשת אנטיביוטיקה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. כיב טראומטי שחולף תוך 24 שעות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. שגשוג רקמת חיבור תגובתית (Hyperplastic tissue fold) בפרוזדור הפה הנגרם מגירוי כרוני של שולי תותבת עודפים/ארוכים; הטיפול כולל התאמת שוליים, טיפול ברקמה וכריתה כירורגית במידת הצורך."
      ],
      "correct": "ד. שגשוג רקמת חיבור תגובתית (Hyperplastic tissue fold) בפרוזדור הפה הנגרם מגירוי כרוני של שולי תותבת עודפים/ארוכים; הטיפול כולל התאמת שוליים, טיפול ברקמה וכריתה כירורגית במידת הצורך.",
      "explanation": "Epulis Fissuratum מתרחש בווסטיבולום עקב גירוי כרוני משולי תותבת עודפים. הטיפול הראשוני הוא הורדת הגירוי המכני (Relief/Shortening) וכריתה במידת הצורך."
    },
    {
      "id": 1004,
      "topic": "10. ספיגת עצם אלבאולרית ופתולוגיות רירית",
      "difficulty": "מומחה",
      "question": "מהי התופעה הקלינית המכונה Kelly's Combination Syndrome במטופלים עם תותבת שלמה עליונה מול שיניים טבעיות קדמיות תחתונות ו-RPD אחורי?",
      "options": [
        "א. ספיגת עצם מואצת במקסילה הקדמית, היפרטרופיה של ה-Tuberosities, Inflammatory Papillary Hyperplasia בחך, ספיגת עצם אחורית בתחתון ובקעה של השיניים הקדמיות התחתונות.",
        "ב. אובדן מלא של התותבת התחתונה בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. צמיחת שיניים עודפות בתוך הרכס. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הרחבה של השריר המיילוהיואידי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. ספיגת עצם מואצת במקסילה הקדמית, היפרטרופיה של ה-Tuberosities, Inflammatory Papillary Hyperplasia בחך, ספיגת עצם אחורית בתחתון ובקעה של השיניים הקדמיות התחתונות.",
      "explanation": "לפי קלי (Combination Syndrome), עומס יתר של החותכות התחתונות על קדמת המקסילה גורם לספיגת עצם קדמית עליונה, ציפה של הרקמה (Flabby ridge), הגדלת התוברסיטיס ופפילרי היפרפלזיה בחך."
    }
  ],
  "11": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהי הגדרת איכות עצם מסוג D1 לפי סיווג Misch, באיזה אזור אנטומי היא שכיחה ביותר?",
      "options": [
        "א. עצם שעברה ספיגה פתולוגית מלאה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. עצם קורטיקלית דחוסה כמעט לחלוטין; שכיחה בעיקר בקדמת המנדיבולה הספוגה.",
        "ג. עצם טרבקולרית רכה ופתוחה; שכיחה במקסילה האחורית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. עצם ספוגית בלבד ללא מעטפת קורטיקלית. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ב. עצם קורטיקלית דחוסה כמעט לחלוטין; שכיחה בעיקר בקדמת המנדיבולה הספוגה.",
      "explanation": "לפי מיש (מהדורה 2, פרק 5/7), עצם D1 מורכבת כמעט כולה מעצם קורטיקלית דחוסה (Dense cortical bone), והיא שכיחה בקדמת המנדיבולה הספוגה (Anterior mandible)."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "כיצד משפיע קוטר השתל (Implant Diameter) ביחס לאורכו על פיזור כוחות הלחיצה והגזירה בעצם הקורטיקלית העליונה?",
      "options": [
        "א. לקוטר השתל אין שום השפעה ביומכנית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. קוטר רחב מפתח כוחות גזירה הרסניים בצוואר השתל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הגדלת קוטר השתל מפתחת שטחי פנים ומפחיתה עומסי לחיצה בשיא הרכס בצורה משמעותית יותר מאשר הגדלת אורך השתל.",
        "ד. הגדלת אורך השתל מ-10 ל-15 מ\"מ מורידה את העומס בצוואר השתל ב-90%. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. הגדלת קוטר השתל מפתחת שטחי פנים ומפחיתה עומסי לחיצה בשיא הרכס בצורה משמעותית יותר מאשר הגדלת אורך השתל.",
      "explanation": "לפי מיש (פרק 12-13), רוב העומס האוקלוזלי מתרכז ב-3-5 המילימטרים העליונים של העצם הקורטיקלית. הגדלת הקוטר מגדל את שטח הפנים במישור הקריטי ומפחית מאמצים בצורה אפקטיבית בהרבה מהארכת השתל."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "מהו המרחק המינימלי הנדרש בין שני שתלים סמוכים, ובין שתל לשן טבעית סמוכה, לשמירה על הפפילה והעצם הבין-דנטלית?",
      "options": [
        "א. 0.5 מ\"מ בלבד בין שתלים. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. 5 מ\"מ בין שתל לשן טבעית. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. אין שום מגבלת מרחק כל עוד השתלים לא נוגעים זה בזה.",
        "ד. לפחות 3 מ\"מ בין שתל לשתל, ולפחות 1.5 מ\"מ בין שתל לשן טבעית."
      ],
      "correct": "ד. לפחות 3 מ\"מ בין שתל לשתל, ולפחות 1.5 מ\"מ בין שתל לשן טבעית.",
      "explanation": "לפי מיש וזארב (Tarnow et al.), דרוש מרחק של לפחות 3 מ\"מ בין שתל לשתל כדי למנוע חפיפה של ספיגת העצם המקורית (Biological width/microgap), ו-1.5 מ\"מ בין שתל לשן לשמירה על העצם הבין-דנטלית והפפילה."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "מהי ההגדרה של אוסטאואינטגרציה (Osseointegration) לפי Brånemark ברמה המיקרוסקופית והקלינית?",
      "options": [
        "א. מגע ישיר, תפקודי ומבני בין עצם חיה לפני שטח השתל הטעון עומס, ללא התערבות רקמת חיבור פיברוטית.",
        "ב. חיבור ע\"י סיבי ליגמנט פריודונטלי מלאכותי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. יצירת סחוס היאליני סביב הטיטניום. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. התחברות כימית של אבקת הידרוקסיאפטיט בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "א. מגע ישיר, תפקודי ומבני בין עצם חיה לפני שטח השתל הטעון עומס, ללא התערבות רקמת חיבור פיברוטית.",
      "explanation": "אוסטאואינטגרציה (Brånemark / Misch / Zarb) מוגדרת כחיבור מבני ותפקודי ישיר בין עצם חיה ומאורגנת לבין פני שטח השתל הטעון עומס ללא שכבת רקמת חיבור ביניהם."
    },
    {
      "id": 1101,
      "topic": "11. שתלים דנטליים: ביומכניקה, איכות עצם ותכנון",
      "difficulty": "קל",
      "question": "כיצד מוגדרת איכות עצם D1 לפי סיווג Misch (מיש) ואיפה היא שכיחה ביותר אנטומית?",
      "options": [
        "א. עצם קורטיקלית דקה עם ספוגית צפופה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. עצם קורטיקלית דחוסה מאוד (Dense cortical bone), השכיחה בעיקר בחלק הקדמי של המנדיבולה.",
        "ג. עצם שעברה רדיותרפיה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. עצם ספוגית רכה מאוד (Loose trabecular) השכיחה במקסילה האחורית."
      ],
      "correct": "ב. עצם קורטיקלית דחוסה מאוד (Dense cortical bone), השכיחה בעיקר בחלק הקדמי של המנדיבולה.",
      "explanation": "לפי מיש (פרק 8), עצם D1 היא עצם קורטיקלית דחוסה, שכיחה בקדמת המנדיבולה, מעניקה יציבות ראשונית מצוינת אך בעלת אספקת דם מופחתת."
    },
    {
      "id": 1102,
      "topic": "11. שתלים דנטליים: ביומכניקה, איכות עצם ותכנון",
      "difficulty": "בינוני",
      "question": "מהי ההגדרה המקובלת של אוסטאואינטגרציה (Osseointegration) לפי Bränemark?",
      "options": [
        "א. התקשרות כימית של יוני סידן מתוך הרוק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. חיבור של רקמת חיבור סיבית בין השתל לעצם. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. חיבור מבני ותפקודי ישיר בין עצם חיים לבין פני שטח של שתל טיטניום נושא עומס, ללא תיווך רקמה רכה.",
        "ד. ציפוי השתל בסחוס היאליני. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. חיבור מבני ותפקודי ישיר בין עצם חיים לבין פני שטח של שתל טיטניום נושא עומס, ללא תיווך רקמה רכה.",
      "explanation": "אוסטאואינטגרציה לפי ברנמרק מוגדרת כמגע ישיר ברמת מיקרוסקופ האור בין עצם חיה לפני השטח של הטיטניום תחת עומס תפקודי."
    },
    {
      "id": 1103,
      "topic": "11. שתלים דנטליים: ביומכניקה, איכות עצם ותכנון",
      "difficulty": "קשה",
      "question": "מהי ספיגת העצם הקרסטלית הממוצעת הנחשבת לתקינה בשנה הראשונה לאחר העמסת שתל לפי הקריטריונים של Albrektsson & Zarb?",
      "options": [
        "א. אפס ספיגת עצם לחלוטין כל חיי השתל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. 0.5 מ\"מ בכל חודש. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. 3.0 מ\"מ בשנה הראשונה ו-1.0 מ\"מ בכל שנה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. עד 1.5 מ\"מ בשנה הראשונה, ולאחר מכן פחות מ-0.2 מ\"מ לכל שנה נוספת."
      ],
      "correct": "ד. עד 1.5 מ\"מ בשנה הראשונה, ולאחר מכן פחות מ-0.2 מ\"מ לכל שנה נוספת.",
      "explanation": "לפי אלברקטסון וזארב (מיש פרק 5), הצלחת שתל מוגדרת כאשר ספיגת העצם הקרסטלית אינה עולה על 1.5 מ\"מ בשנה הראשונה ולאחר מכן פחות מ-0.2 מ\"מ לשנה."
    },
    {
      "id": 1104,
      "topic": "11. שתלים דנטליים: ביומכניקה, איכות עצם ותכנון",
      "difficulty": "מומחה",
      "question": "כיצד משפיע קוטר השתל (Implant Diameter) לעומת אורך השתל (Implant Length) על פיזור מאמצי הגזירה והלחיצה בצוואר השתל לפי מיש?",
      "options": [
        "א. הגדלת הקוטר ב-1 מ\"מ מגדלת את שטח הפנים בצוואר השתל ומפחיתה עומסים קרסטליים ב-20-30%, בעוד שהארכת השתל משפיעה בעיקר על היציבות הראשונית ופחות על העומס הקרסטלי.",
        "ב. אורך השתל הוא המשתנה היחיד הקובע עמידות בפני כוחות כפיפה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הגדלת האורך משפרת את פיזור המאמצים הקרסטליים פי 4 מהגדלת הקוטר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. אין שום הבדל בין קוטר לאורך. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. הגדלת הקוטר ב-1 מ\"מ מגדלת את שטח הפנים בצוואר השתל ומפחיתה עומסים קרסטליים ב-20-30%, בעוד שהארכת השתל משפיעה בעיקר על היציבות הראשונית ופחות על העומס הקרסטלי.",
      "explanation": "לפי מיש (פרק 7), רוב העומס הביומכני מתרכז ב-3-5 המ\"מ הראשונים של צוואר השתל. לכן, הגדלת הקוטר מורידה מאמצים קרסטליים באופן משמעותי הרבה יותר מהארכת השתל."
    }
  ],
  "12": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "איזו שיטת חיבור שיקום על גבי שתלים (Screw-retained vs. Cement-retained) מעניקה יתרון מובהק של פריקות (Retrievability) וגובה אקסאלי מצומצם?",
      "options": [
        "א. שיקום בהדבקה בלתי הפיכה עם אקריל.",
        "ב. שיקום מוברג (Screw-retained).",
        "ג. שיקום מודבק (Cement-retained).",
        "ד. שיקום שרפי ישיר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. שיקום מוברג (Screw-retained).",
      "explanation": "לפי מיש (פרק 14-15) וזארב, שיקום מוברג מאפשר פירוק קל (Retrievability) לצורך תחזוקה וטיפול, ומתאים למצבים של גובה בין-סגרי מצומצם ללא סכנת שאריות צמנט."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהי הסכנה הפריודונטלית/פרי-אימפלנטרית המרכזית בשימוש בשיקום מודבק (Cement-retained) עם קו סיום תת-חניכי עמוק?",
      "options": [
        "א. שבר של בורג השתל המרכזי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. התרופפות הבורג הפנימי ב-100% מהמקרים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. שאריות צמנט בלתי מפונות בתת-חניכיים המובילות לדלקת קשה, Peri-implantitis וספיגת עצם.",
        "ד. שינוי צבע הכתר לצהוב. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. שאריות צמנט בלתי מפונות בתת-חניכיים המובילות לדלקת קשה, Peri-implantitis וספיגת עצם.",
      "explanation": "לפי מיש וזארב, שאריות צמנט הדבקה תת-חניכיות סביב שתלים הן גורם אטיולוגי מרכזי להתפתחות פרי-אימפלנטיטיס ואיבוד עצם מהיר."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "מהו מנגנון ה-Platform Switching בשיקום על גבי שתלים, ומהי תרומתו לשמירה על שיא עצם הרכס (Crestal Bone)?",
      "options": [
        "א. ביטול הבורג המרכזי ושימוש בפין פלדה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. חיבור מבנה רחב יותר מקוטר השתל להגדלת התמיכה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הרחבת השתל ב-3 מ\"מ מעל החניכיים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. חיבור מבן (Abutment) בעל קוטר צר יותר מקוטר פלטפורמת צוואר השתל; מסיט את ה-Microgap פנימה והרחק משיא העצם הקורטיקלית."
      ],
      "correct": "ד. חיבור מבן (Abutment) בעל קוטר צר יותר מקוטר פלטפורמת צוואר השתל; מסיט את ה-Microgap פנימה והרחק משיא העצם הקורטיקלית.",
      "explanation": "לפי מיש וזארב, Platform Switching מסיט את המפתח המיקרוסקופי (Microgap) והתגובה הדלקתית הנלווית אליו מרכזית פנימה, ובכך משמר את גובה עצם הרכס ה-Crestal bone."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "מהי התופעה הפיזיקלית של Preload בבורג המבנה (Abutment screw), ואיך היא מגינה מנפילת הבורג?",
      "options": [
        "א. מתיחה אלסטית פנימית של הבורג המופעלת בעת הידוק במומנט (Torque) מומלץ, המייצרת כוח לחיצה (Clamping force) בין השרכים.",
        "ב. הדבקה כימית של הברגה בעזרת רוק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. התפשטות תרמית של הברזל בבורג. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. שבירה מבוקרת של ראש הבורג. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. מתיחה אלסטית פנימית של הבורג המופעלת בעת הידוק במומנט (Torque) מומלץ, המייצרת כוח לחיצה (Clamping force) בין השרכים.",
      "explanation": "לפי מיש וזארב, Preload הוא מתח המתיחה האלסטי שנוצר בבורג בעת הידוק במומנט הנכון (כגון 30-35 Ncm). מתח זה מייצר Clamping force המהדק את המבנה לשתל ומונע התרופפות תחת עומסים מחזוריים."
    },
    {
      "id": 1201,
      "topic": "12. שיקום על גבי שתלים: מבנים ואפשרויות שיקומיות",
      "difficulty": "קל",
      "question": "מהו היתרון המרכזי של שחזור המוברג ישירות לשתל (Screw-Retained Crown) לעומת שחזור מודבק (Cement-Retained Crown)?",
      "options": [
        "א. מחיר זול בהרבה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. פריקות ושליפות (Retrievability) קלה לצורך תחזוקה, והיעדר סיכון לדלקת סביב שתל (Peri-implantitis) מפתור עודפי צמנט תת-חניכיים.",
        "ג. אין צורך בצילום רדיוגרפי לבדיקת ההתאמה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. אסתטיקה עדיפה באזור הקדמי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. פריקות ושליפות (Retrievability) קלה לצורך תחזוקה, והיעדר סיכון לדלקת סביב שתל (Peri-implantitis) מפתור עודפי צמנט תת-חניכיים.",
      "explanation": "לפי מיש (פרקים 12-14), שחזור מוברג מונע לחלוטין דלקת פרי-אימפלנטיטיס הנגרמת מששארי צמנט ומאפשר פירוק קל לצורך תחזוקה."
    },
    {
      "id": 1202,
      "topic": "12. שיקום על גבי שתלים: מבנים ואפשרויות שיקומיות",
      "difficulty": "בינוני",
      "question": "מהו ה-Preload בבורג המבנה (Abutment Screw) ומה חשיבות מומנט הסגירה (Torque) המומלץ ע\"י היצרן?",
      "options": [
        "א. עובי מעטפת הטיטניום בצוואר השתל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. מומנט ההחדרה של השתל בעת ההשתלה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מתיחה אלסטית מבוקרת של הבורג המייצרת כוח הידוק (Clamping force) בין המבנה לשתל למניעת שחרור הבורג בעומסי לעיסה.",
        "ד. כוח הדחיסה האנכי של השתל לתוך העצם. כפי שמוגדר במנח האנטומי של בסיס הגולגולת ומישור הסגר."
      ],
      "correct": "ג. מתיחה אלסטית מבוקרת של הבורג המייצרת כוח הידוק (Clamping force) בין המבנה לשתל למניעת שחרור הבורג בעומסי לעיסה.",
      "explanation": "הידוק הבורג במומנט הנכון (למשל 30-35 Ncm) יוצר Preload (מתיחה אלסטית) השומרת על ה-Clamping force ומונעת שחרור גראוויטציוני/דינמי של הבורג."
    },
    {
      "id": 1203,
      "topic": "12. שיקום על גבי שתלים: מבנים ואפשרויות שיקומיות",
      "difficulty": "קשה",
      "question": "איזה סיווג פרוסטודונטי לפי מיש (Misch Prosthetic Classification FP-1 to FP-3) מתאר כתר המשחזר את כותרת השן בלבד ללא שחזור של חניכיים אבודות?",
      "options": [
        "א. FP-2, המשחזר כותרת וחלק מהשורש. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. FP-3, המשחזר כותרת ורקמה ורודה (Gingival acrylic/porcelain).",
        "ג. RP-4, תותבת נשלפת נתמכת שתלים בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. FP-1 (Fixed Prosthesis 1), המשחזר את כותרת השן בלבד ונראה כטבעי לחלוטין."
      ],
      "correct": "ד. FP-1 (Fixed Prosthesis 1), המשחזר את כותרת השן בלבד ונראה כטבעי לחלוטין.",
      "explanation": "לפי מיש, FP-1 משחזר רק את כותרת השן הטבעית. FP-2 משחזר כותרת וחלק מהשורש, ואילו FP-3 משחזר גם את הרקמה הורודה."
    },
    {
      "id": 1204,
      "topic": "12. שיקום על גבי שתלים: מבנים ואפשרויות שיקומיות",
      "difficulty": "מומחה",
      "question": "מהי הסכנה המרכזית בשימוש בצמנט רזין קשיח להדבקת כתרים על גבי שתלים כאשר גבול ההדבקה ממוקם 2 מ\"מ תת-חניכית?",
      "options": [
        "א. קושי רב או חוסר יכולת לסלק עודפי צמנט נוקשים מהמרחב התת-חניכי, המוביל להתפתחות מואצת של Peri-implantitis ואובדן עצם קרסטלית.",
        "ב. שבירה מיידית של השתל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. המסה מהירה של הצמנט ע\"י הרוק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. שינוי צבע החרסינה לצהוב. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. קושי רב או חוסר יכולת לסלק עודפי צמנט נוקשים מהמרחב התת-חניכי, המוביל להתפתחות מואצת של Peri-implantitis ואובדן עצם קרסטלית.",
      "explanation": "שאריות צמנט תת-חניכיות סביב שתלים הן מהגורמים השכיחים ביותר לפרי-אימפלנטיטיס. צמנט רזין תת-חניכי עמוק קשה מאוד לזיהוי ולסילוק."
    }
  ],
  "13": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהו הרוחב הביולוגי (Biological Width / Supracrestal Tissue Attachment) הממוצע סביב שן טבעית בריאה?",
      "options": [
        "א. 0.5 מ\"מ בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. כ-2.04 מ\"מ (מורכב מ-0.97 מ\"מ Junctional Epithelium ו-1.07 מ\"מ Connective Tissue attachment).",
        "ג. 10 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. 5.0 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. כ-2.04 מ\"מ (מורכב מ-0.97 מ\"מ Junctional Epithelium ו-1.07 מ\"מ Connective Tissue attachment).",
      "explanation": "לפי לינדה (פרק 16) ושילינבורג (Gargiulo / Vacek), הרוחב הביולוגי סביב שן ממוצע ב-2.04 מ\"מ. חדירת שולי שחזור לתוך תחום זה גורמת לדלקת כרונית, נסיגת חניכיים או ספיגת עצם."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהו ההבדל המרכזי בין טראומה אוקלוזלית ראשונית (Primary Occlusal Trauma) לטראומה אוקלוזלית שניונית (Secondary Occlusal Trauma)?",
      "options": [
        "א. אין שום הבדל קליני ביניהן. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הראשונית גורמת לאיבוד עצם אופקי בלבד והשניונית לאיבוד אנכי. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. הראשונית מתרחשת עקב כוחות סגר מופרזים על שן עם תמיכה פריודונטלית בריאה ורגילה; השניונית מתרחשת עקב כוחות סגר רגילים/מופרזים על שן עם תמיכה פריודונטלית מופחתת/חולה.",
        "ד. הראשונית מופיעה רק בשיניים חלביות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. הראשונית מתרחשת עקב כוחות סגר מופרזים על שן עם תמיכה פריודונטלית בריאה ורגילה; השניונית מתרחשת עקב כוחות סגר רגילים/מופרזים על שן עם תמיכה פריודונטלית מופחתת/חולה.",
      "explanation": "לפי לינדה (פרק 17) ומיש, טראומה ראשונית היא כוח מופרז על מנגנון תמיכה שמור ובריא. טראומה שניונית היא הפעלת כוחות (אפילו פיזיולוגיים) על מנגנון אחיזה מופחת שסבל ממחלת חניכיים."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "איזה ניתוח פריודונטלי מותווה לצורך יצירת מקום לרוחב ביולוגי ולפרול (Ferrule) בשן עם הרס כותרתי תת-חניכי?",
      "options": [
        "א. Subgingival Curettage. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Free Gingival Graft בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. Gingivectomy בלבד ללא נגיעה בעצם. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. Crown Lengthening Procedure (הארכת כותרת כירורגית הכוללת הסרת עצם)."
      ],
      "correct": "ד. Crown Lengthening Procedure (הארכת כותרת כירורגית הכוללת הסרת עצם).",
      "explanation": "לפי לינדה ושילינבורג, הארכת כותרת כירורגית (Crown Lengthening) דורשת אוסטאוטומיה/אוסטאופלסטיה כדי להרחיק את שיא העצם לפחות 3 מ\"מ משולי ההכנה המתוכננים לשמירה על הרוחב הביולוגי והפרול."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "מהם המדדים הקליניים והרדיוגרפיים האופייניים לטראומה אוקלוזלית על שן טבעית?",
      "options": [
        "א. ניידות שן גוברת, התרחבות מרווח הליגמנט הפריודונטלי (PDL space widening), עיבוי Lamina dura, ורגישות לניקוש.",
        "ב. אובדן חיוניות השן תוך 24 שעות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. היווצרות כיס פריודונטלי דלקתי אמיתי בודד ללא חיידקים. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. נסיוג חניכיים ללא שום שינוי ברדיוגרפיה. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "א. ניידות שן גוברת, התרחבות מרווח הליגמנט הפריודונטלי (PDL space widening), עיבוי Lamina dura, ורגישות לניקוש.",
      "explanation": "לפי לינדה ומיש, טראומה אוקלוזלית מתבטאת קלינית בניידות ורגישות, ורדיוגרפית בהתרחבות ה-PDL, עיבוי ה-Lamina Dura וספיגת עצם אופקית/זוויתית (אך אינה יוצרת כיס פריודונטלי ללא נוכחות פלאק חיידקי)."
    },
    {
      "id": 1301,
      "topic": "13. יחסי גומלין פריודונטיה-שיקום וניידות שיניים",
      "difficulty": "קל",
      "question": "מהו הרוחב הביולוגי (Biological Width) הממוצע סביב שן טבעית לפי Gargiulo / Lindhe?",
      "options": [
        "א. 0.5 מ\"מ בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. כ-2.04 מ\"מ בסך הכל (כ-1.07 מ\"מ Junctional Epithelium + כ-0.97 מ\"מ Connective Tissue Attachment).",
        "ג. 0.1 מ\"מ של אמייל. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. 5.0 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. כ-2.04 מ\"מ בסך הכל (כ-1.07 מ\"מ Junctional Epithelium + כ-0.97 מ\"מ Connective Tissue Attachment).",
      "explanation": "לפי גרג'יולו ולינדה (פרקים 16-17), הרוחב הביולוגי מורכב מאחיזת האפיתל והאחיזה המחברת ומסתכם ב-2.04 מ\"מ בממוצע. פגיעה בו ע\"י שולי כתר עמוקים גורמת לדלקת ואובדן עצם."
    },
    {
      "id": 1302,
      "topic": "13. יחסי גומלין פריודונטיה-שיקום וניידות שיניים",
      "difficulty": "בינוני",
      "question": "מה מוגדר כטראומה אוקלוזלית ראשונית (Primary Occlusal Trauma)?",
      "options": [
        "א. עששת עמוקה בצוואר השן. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. ניידות שן הנגרמת עקב שבר שורש בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. כוחות אוקלוזליים מופרזים/חריגים המופעלים על שן בעלת מנגנון תמיכה פריודונטלי תקין ובריא.",
        "ד. כוחות אוקלוזליים תקינים המופעלים על מנגנון תמיכה פריודונטלי מופחת עקב מחלת חניכיים."
      ],
      "correct": "ג. כוחות אוקלוזליים מופרזים/חריגים המופעלים על שן בעלת מנגנון תמיכה פריודונטלי תקין ובריא.",
      "explanation": "לפי לינדה, טראומה אוקלוזלית ראשונית מתרחשת כאשר כוחות מופרזים פועלים על תמיכה פריודונטלית בריאה. טראומה משנית מתרחשת כאשר כוחות תקינים/חריגים פועלים על תמיכה מופחתת."
    },
    {
      "id": 1303,
      "topic": "13. יחסי גומלין פריודונטיה-שיקום וניידות שיניים",
      "difficulty": "קשה",
      "question": "מהו המרווח המינימלי הנדרש בין קו הסיום של השחזה (Margin) לבין שיא העצם האלבאולרית בעת ביצוע הארכת כותרת (Crown Lengthening)?",
      "options": [
        "א. 6.0 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. 1.0 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. 0.5 מ\"מ בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. לפחות 3.0 מ\"מ (2.0 מ\"מ לשמירת הרוחב הביולוגי + 1.0 מ\"מ עבור ה-Ferrule והשוליים)."
      ],
      "correct": "ד. לפחות 3.0 מ\"מ (2.0 מ\"מ לשמירת הרוחב הביולוגי + 1.0 מ\"מ עבור ה-Ferrule והשוליים).",
      "explanation": "כדי למנוע פגיעה ברוחב הביולוגי ולהבטיח Ferrule תקין, הניתוח להארכת כותרת חייב לייצר מרחק של לפחות 3 מ\"מ משיא העצם עד לקו הסיום המתוכנן."
    },
    {
      "id": 1304,
      "topic": "13. יחסי גומלין פריודונטיה-שיקום וניידות שיניים",
      "difficulty": "מומחה",
      "question": "מתי מתווית סדיקת שיניים (Splinting) כאמצעי טיפולי לפי הספרות הפריודונטלית-שיקומית?",
      "options": [
        "א. כאשר ניידות השיניים היא פרוגרסיבית וגורמת לאי נוחות למטופל או מפריעה לתפקוד הלעיסה, לאחר שליטה בדלקת החניכיים ואיזון אוקלוזלי.",
        "ב. בכל שן עם ניידות דרגה 1 בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. במקום טיפול חניכיים לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. רק לפני עקירת השיניים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. כאשר ניידות השיניים היא פרוגרסיבית וגורמת לאי נוחות למטופל או מפריעה לתפקוד הלעיסה, לאחר שליטה בדלקת החניכיים ואיזון אוקלוזלי.",
      "explanation": "סדיקה אינה מרפאת מחלת חניכיים, אלא מתווית רק כאשר יש ניידות מתקדמת המפריעה לנוחות/תפקוד המטופל, לאחר שבוצעה שליטה בדלקת ואיזון סגר."
    }
  ],
  "14": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהו היתרון הפיזיולוגי והאנטומי המרכזי של תותבת על על גבי שורשים (Tooth-supported Overdenture) בהשוואה לתותבת שלמה נתמכת רירית בלבד?",
      "options": [
        "א. מניעת היווצרות אבנית בשיניים העליונות. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. שימור עצם הרכס האלבאולרי סביב השורשים ושמירה על הפרופריוצפציה (Proprioception) של ה-PDL.",
        "ג. אפשרות להקטין את עובי השיניים ב-80%. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. ביטול מוחלט של הצורך בניקוי התותבת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. שימור עצם הרכס האלבאולרי סביב השורשים ושמירה על הפרופריוצפציה (Proprioception) של ה-PDL.",
      "explanation": "לפי זארב (פרק 20) ומיש, השארת שורשים תחת תותבת על משמרת את עצם הרכס מספיגה מואצת ומשמרת את קולטני ה-PDL התחושתיים."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "כמה שתלים דנטליים נדרשים כמינימום המומלץ לתמיכה ואחיזה של תותבת על תחתונה (Mandibular Implant Overdenture) לפי קונצנזוס מקגיל (McGill Consensus)?",
      "options": [
        "א. שמונה שתלים ארוכים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. שישה שתלים קשיחים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. שני שתלים באזור הקדמי (Interforaminal region).",
        "ד. שתל אחד בלבד במרכז המנדיבולה. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים."
      ],
      "correct": "ג. שני שתלים באזור הקדמי (Interforaminal region).",
      "explanation": "לפי קונצנזוס מקגיל (McGill Consensus Statement / Zarb / Misch), תותבת על תחתונה נתמכת 2 שתלים באזור האינטרפורמינלי היא סטנדרט הטיפול הטיפולי המינימלי למטופל חסר שיניים במנדיבולה."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "איזה סוג מחבר (Attachment) מעניק תנועתיות רוטציונית (Resilient Attachment) ומפחית עומסים ממוקדים מ-2 שתלים בודדים במנדיבולה?",
      "options": [
        "א. מבנה יצוק בודד מוברג ללא שום חופש. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ב. צמנט זמני רך. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Dolder Bar קשיח לחלוטין ללא שחרור. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ד. Stud Attachments (כגון Locator / Ball Attachment)."
      ],
      "correct": "ד. Stud Attachments (כגון Locator / Ball Attachment).",
      "explanation": "לפי זארב ומיש, מחברי Stud בודדים (Locator / Ball) הם Resilient attachments המאפשרים תנועה מסוימת של בסיס התותבת לכיוון הרירית ומפחיתים כוחות מנוף על השתלים."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "מהן הדרישות האנטומיות והביומכניות לתכנון מחבר מסוג Dolder Bar / Hader Bar בתותבת על נתמכת שתלים?",
      "options": [
        "א. מרווח אנכי (Clearance) של לפחות 10-12 מ\"מ משיא העצם למישור הסגר, ומקבילות של הבר למישור הסיבוב.",
        "ב. הדבקת הבר בצמנט אבץ פוספט. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מרווח אנכי של 2 מ\"מ בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. הרכבת הבר ישירות על החניכיים ללא שום מרווח ניקוי. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation."
      ],
      "correct": "א. מרווח אנכי (Clearance) של לפחות 10-12 מ\"מ משיא העצם למישור הסגר, ומקבילות של הבר למישור הסיבוב.",
      "explanation": "לפי זארב ומיש, מחבר Bar מורכב דורש מרווח אנכי משמעותי (לפחות 10-12 מ\"מ) ליצירת מקום לשתלים, למבנים, לבר, לרוכבים (Riders), לבסיס האקרילי ולשיניים."
    },
    {
      "id": 1401,
      "topic": "14. תותבות על (Overdentures) ואטצ'מנטים",
      "difficulty": "קל",
      "question": "מהו היתרון הפיזיולוגי והביומכני המרכזי בשמירת שורשים טבעיים מתחת לתותבת על (Root-Supported Overdenture)?",
      "options": [
        "א. ביטול הצורך בניקוי התותבת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. שימור העצם האלבאולרית סביב השורשים (מניעת ספיגה עד פי 8), ושמירה על הפרופריוצפציה (Proprioception) מהליגמנט הפריודונטלי.",
        "ג. מניעת עששת לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. הגדלת המרווח הבין-אוקלוזלי. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. שימור העצם האלבאולרית סביב השורשים (מניעת ספיגה עד פי 8), ושמירה על הפרופריוצפציה (Proprioception) מהליגמנט הפריודונטלי.",
      "explanation": "לפי זארב ומול, שמירת שורשים מתחת לתותבת משמרת את העצם האלבאולרית באופן דרמטי ושומרת על התחושה הפרופריוצפטית של ה-PDL."
    },
    {
      "id": 1402,
      "topic": "14. תותבות על (Overdentures) ואטצ'מנטים",
      "difficulty": "בינוני",
      "question": "מהו המרווח הבין-אוקלוזלי האנכי המינימלי הנדרש (Interocclusal Space) משיא הרכס עד מישור הסגר לצורך תכנון תותבת על נתמכת שתלים עם Locator Attachments?",
      "options": [
        "א. 3-4 מ\"מ בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. 20 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. כ-8.5 עד 10 מ\"מ לפחות (עבור המבנה, האטצ'מנט, האקריל והשיניים), כדי למנוע שבר תותבת.",
        "ד. 1 מ\"מ. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. כ-8.5 עד 10 מ\"מ לפחות (עבור המבנה, האטצ'מנט, האקריל והשיניים), כדי למנוע שבר תותבת.",
      "explanation": "לפי מיש (פרק 15), תותבת על עם Locator מחייבת מרווח אנכי של 8.5-10 מ\"מ. חוסר במרווח אנכי יוביל לתותבת דקה ונשברת או בלט אסתטי."
    },
    {
      "id": 1403,
      "topic": "14. תותבות על (Overdentures) ואטצ'מנטים",
      "difficulty": "קשה",
      "question": "מהו ההבדל הביומכני בין תותבת על מסוג RP-4 לבין RP-5 בסיווג של מיש?",
      "options": [
        "א. RP-4 מיועדת רק למקסילה ו-RP-5 רק למנדיבולה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אין שום הבדל ביומכני בנשיאת העומס. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. RP-4 היא תותבת מודבקת ו-RP-5 היא מוברגת. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. RP-4 היא תותבת נשלפת הנתמכת לחלוטין ע\"י שתלים/בר (Implant-supported), בעוד RP-5 נתמכת ע\"י שתלים וגם ע\"י הרכס והרקמה הרכה (Implant-retained, tissue-supported)."
      ],
      "correct": "ד. RP-4 היא תותבת נשלפת הנתמכת לחלוטין ע\"י שתלים/בר (Implant-supported), בעוד RP-5 נתמכת ע\"י שתלים וגם ע\"י הרכס והרקמה הרכה (Implant-retained, tissue-supported).",
      "explanation": "לפי מיש (פרק 15), RP-4 נתמכת קשיחה ע\"י השתלים בלבד (אין שקיעה על הרקמה), בעוד RP-5 מקבלת תמיכה משולבת מהשתלים ומבסיס הרקמה הרכה."
    },
    {
      "id": 1404,
      "topic": "14. תותבות על (Overdentures) ואטצ'מנטים",
      "difficulty": "מומחה",
      "question": "איזה סוג בר (Bar attachment) מאפשר תנועת רוטציה חופשית במישור הסגיטלי סביב ציר יחיד (Hinge motion) בלבד?",
      "options": [
        "א. Dolder Bar בצורת אגס/עגולה (Egg-shaped / Runner bar) עם רווח מעל הבר בתוך התפס (Rider).",
        "ב. בר מולחם קשיח ללא חופש תנועה. עקב היעדר מגעים אוקלוזליים הרמוניים בעמדת Maximum Intercuspation.",
        "ג. Dolder Bar בצורת U קשיחה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Ackermann bar מרובע. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. Dolder Bar בצורת אגס/עגולה (Egg-shaped / Runner bar) עם רווח מעל הבר בתוך התפס (Rider).",
      "explanation": "Dolder bar עגול/דמוי אגס מאפשר תנועת הציר (Hinge Action) וספיגת עומסים מהרקמה הרכה, בעוד Dolder bar מרובע בצורת U הוא קשיח (Rigid)."
    }
  ],
  "15": [
    {
      "id": 1,
      "difficulty": "קל",
      "question": "מהם שלושת הממדים של הצבע לפי מודל Munsell המשמשים לבחירת גוון שיניים (Shade selection)?",
      "options": [
        "א. Opacity, Translucency, Fluorescence. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Hue (הגוון הבסיסי), Value (בהירות/כהות), Chroma (רוויית הצבע).",
        "ג. Red, Green, Blue. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Brightness, Contrast, Saturation. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Hue (הגוון הבסיסי), Value (בהירות/כהות), Chroma (רוויית הצבע).",
      "explanation": "לפי רוזנסטיל (פרק 21) ופיליפס, מודל Munsell מגדיר: Hue = משפחת הצבע (A,B,C,D), Chroma = עוצמת/רוויית הצבע, ו-Value = כמות האור/בהירות (המדד הקריטי ביותר באסתטיקה)."
    },
    {
      "id": 2,
      "difficulty": "בינוני",
      "question": "מהי התופעה האופטית הנקראת Metamerism, וכיצד מונעים טעות בעת בחירת צבע במרפאה?",
      "options": [
        "א. שבירת אור דרך זגוגית השן בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ב. שינוי צבע הכתר עקב שתיית קפה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מצב שבו שני עצמים נראים בעלי צבע זהה תחת מקור אור אחד אך שונים תחת מקור אור אחר; מונעים ע\"י התאמת צבע בטמפרטורת אור יום תקנית (5000K / D50).",
        "ד. ספיגת קרני אולטרה-סגול ופליטת אור כחול. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. מצב שבו שני עצמים נראים בעלי צבע זהה תחת מקור אור אחד אך שונים תחת מקור אור אחר; מונעים ע\"י התאמת צבע בטמפרטורת אור יום תקנית (5000K / D50).",
      "explanation": "לפי רוזנסטיל ופיליפס, מטאמריזם הוא שינוי בתפיסת גוון תחת מקורות תאורה שונים. התאמת גוון מקצועית מבוצעת תחת תאורה נטרלית תקנית (5000 Kelvin)."
    },
    {
      "id": 3,
      "difficulty": "קשה",
      "question": "בעבודה עם סורק תוך-אוראלי דיגיטלי (Intraoral Scanner), מהי המשמעות של Stitching וכיצד מונעים עיוות מרחבי בסריקת קשת שלמה?",
      "options": [
        "א. המרת הקובץ מ-STL ל-PDF. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. תפירת החניכיים באמצעות חוט משי לפני הסריקה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. הדפסת מודל תלת-ממד בלייזר ירוק. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. חיבור אלגוריתמי של אלפי תמונות תלת-ממדיות תופפות; מונעים עיוות ע\"י מסלול סריקה מובנה רציף (Scanning path) ללא קפיצות."
      ],
      "correct": "ד. חיבור אלגוריתמי של אלפי תמונות תלת-ממדיות תופפות; מונעים עיוות ע\"י מסלול סריקה מובנה רציף (Scanning path) ללא קפיצות.",
      "explanation": "לפי רוזנסטיל ופיליפס (CAD/CAM), סורקים תוך-אוראליים מחברים רצף תמונות התרשמות (Stitching). סטייה ממסלול הסריקה הרציף צוברת שגיאה מרחבית מצטברת במעבר מקשת אחת לשנייה."
    },
    {
      "id": 4,
      "difficulty": "מומחה",
      "question": "מהו קובץ הפורמט הסטנדרטי הפתוח לייצוג פני שטח תלת-ממדיים במערכות CAD/CAM דנטליות?",
      "options": [
        "א. STL (Standard Tessellation Language / Stereolithography).",
        "ב. DICOM (המשמש להדמיה רדיולוגית CT בלבד). תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ג. JPEG. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. DOCX. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. STL (Standard Tessellation Language / Stereolithography).",
      "explanation": "לפי רוזנסטיל וספרות CAD/CAM, קובץ STL הוא הפורמט הבינלאומי הפתוח המייצג את הגאומטריה התלת-ממדית באמצעות רשת משולשים (Mesh), בעוד DICOM מיועד להדמיה נפחית CT."
    },
    {
      "id": 1501,
      "topic": "15. אסתטיקה, בחירת צבע ורפואת שיניים דיגיטלית",
      "difficulty": "קל",
      "question": "איזומיימנד צבע (Color Dimension) לפי סולם Munsell נחשב לקריטי ביותר להתאמה אסתטית של שחזור דנטלי?",
      "options": [
        "א. Hue (גיוון הצבע / A, B, C, D). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. Value (בהירות / Lightness-Darkness), שהוא הממד הקריטי ביותר מכיוון שהעין האנושית רגישה ביותר לשינויי בהירות.",
        "ג. Translucency בלבד. תוך הסתמכות מוחלטת על הדרכת הרכס האלבאולרי השארי במשטחים הבוקאליים.",
        "ד. Chroma (רוויית הצבע). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ב. Value (בהירות / Lightness-Darkness), שהוא הממד הקריטי ביותר מכיוון שהעין האנושית רגישה ביותר לשינויי בהירות.",
      "explanation": "לפי רוזנסטיל (פרק 20) ופיליפס, Value (בהירות) הוא הממד החשוב ביותר בבחירת צבע. טעות ב-Value בולטת מיד לעין האנושית יותר מטעות ב-Hue או Chroma."
    },
    {
      "id": 1502,
      "topic": "15. אסתטיקה, בחירת צבע ורפואת שיניים דיגיטלית",
      "difficulty": "בינוני",
      "question": "מהי תופעת המטמריזם (Metamerism) בבחירת צבע דנטלי וכיצד מונעים אותה?",
      "options": [
        "א. שינוי צבע השן לאחר טיפול שורש. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. הידרדרות הברק של החרסינה עם השנים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. מצב שבו שני עצמים נראים זהים בצבעם תחת מקור אור אחד (למשל תאורת מרפאה), אך שונים תחת מקור אור אחר (למשל אור יום natural light); למניעה יש לבדוק צבע במקורות תאורה שונים.",
        "ד. התאמת צבע המבוצעת כשהשן יבשה לחלוטין. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "ג. מצב שבו שני עצמים נראים זהים בצבעם תחת מקור אור אחד (למשל תאורת מרפאה), אך שונים תחת מקור אור אחר (למשל אור יום natural light); למניעה יש לבדוק צבע במקורות תאורה שונים.",
      "explanation": "לפי רוזנסטיל (פרק 20), מטמריזם מתרחש עקב ספקטרום החזרה שונה של שני חומרים. לכן חובה לבחון צבע בשני מקורות אור שונים (אור יום ותאורת מרפאה 5500K)."
    },
    {
      "id": 1503,
      "topic": "15. אסתטיקה, בחירת צבע ורפואת שיניים דיגיטלית",
      "difficulty": "קשה",
      "question": "מהי המשמעות של Trueness ו-Precision בהערכת דיוק של סורק תוך-אוראלי (Intraoral Scanner)?",
      "options": [
        "א. Trueness מיועדת רק לשתלים ו-Precision רק לכתרים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ב. אין שום הבדל בין המושגים. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. Trueness מתאר את רמת הרזולוציה של המסך, ו-Precision את מהירות הסריקה. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. Trueness מתארת כמה התמונה הדיגיטלית קרובה למציאות האנטומית האמיתית, ו-Precision מתארת את היכולת של הסורק להפיק תוצאה זהה בסריקות חוזרות ונשנות."
      ],
      "correct": "ד. Trueness מתארת כמה התמונה הדיגיטלית קרובה למציאות האנטומית האמיתית, ו-Precision מתארת את היכולת של הסורק להפיק תוצאה זהה בסריקות חוזרות ונשנות.",
      "explanation": "לפי תקן ISO 5725, Trueness מודדת קירבה לאמת האבסולוטית, ו-Precision מודדת חזרתיות (Repeatability) בין סריקות שונות של אותו אובייקט."
    },
    {
      "id": 1504,
      "topic": "15. אסתטיקה, בחירת צבע ורפואת שיניים דיגיטלית",
      "difficulty": "מומחה",
      "question": "כיצד משפיעה התייבשות השן (Tooth Dehydration) במהלך טיפול דנטלי על בחירת הצבע?",
      "options": [
        "א. השן מאבדת מים, הופכת לבהירה יותר (Value גבוה יותר) ולאופקית יותר, ולכן יש לבצע לבחירת צבע בתחילת הפגישה כשהשיניים רטובות.",
        "ב. אין שום השפעה על צבע השן. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ג. השן הופכת לכהה יותר (Value נמוך יותר). כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית.",
        "ד. השן הופכת לשקופה יותר. כפי שמתואר בניתוח הביומכני של מערכת הלעיסה בספרות העיונית."
      ],
      "correct": "א. השן מאבדת מים, הופכת לבהירה יותר (Value גבוה יותר) ולאופקית יותר, ולכן יש לבצע לבחירת צבע בתחילת הפגישה כשהשיניים רטובות.",
      "explanation": "לפי רוזנסטיל (פרק 20), התייבשות שן מעלה את ה-Value (נראית לבנה/אטום יותר) עקב שינוי מקדם השבירה של האוויר באמייל. לכן בוחרים צבע בתחילת הפגישה כשהשן רטובה ברוק."
    }
  ]
}

# Simple Data persistence file
DATA_FILE = "user_data.json"

def load_data():
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
        except:
            pass
    return {"users": {}}

def save_data(data):
    with open(DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

# Initialize Session State
if "user_data" not in st.session_state:
    st.session_state.user_data = load_data()

# Header
st.markdown("""
<div class="main-header">
    <h1>🦷 מדרשת שיקום הפה - אתר הלמידה והתרגול</h1>
    <p>הכנה ממוקדת למבחן הכניסה בהתמחות בשיקום הפה | מבוסס ספרות ושחזורים</p>
</div>
""", unsafe_allow_html=True)

# Sidebar
st.sidebar.header("👤 פרופיל משתמש")
current_user = st.sidebar.selectbox(
    "בחר/י שם מתמחה:",
    ["ד\"ר דורון", "ד\"ר עפרה", "ד\"ר יארה"]
)

# Ensure user entry exists
if current_user not in st.session_state.user_data["users"]:
    st.session_state.user_data["users"][current_user] = {
        "answers": {},
        "ratings": {},
        "feedback": {},
        "bookmarks": {}
    }

user_profile = st.session_state.user_data["users"][current_user]

if "bookmarks" not in user_profile:
    user_profile["bookmarks"] = {}

st.sidebar.markdown("---")
st.sidebar.header("📚 מנגנון תרגול וסיכומים")

main_mode = st.sidebar.radio(
    "בחר מצב עבודה:",
    ["🎯 תרגול שאלות", "📌 מחברת סיכומים לחיזוק (נושאים מסומנים)"]
)

if main_mode == "🎯 תרגול שאלות":
    selected_topic = st.sidebar.selectbox(
        "בחר נושא לימוד (מתוך 15):",
        TOPICS
    )

    selected_difficulty = st.sidebar.radio(
        "רמת קושי:",
        ["הכל", "קל", "בינוני", "קשה", "מומחה"]
    )

    topic_num_str = selected_topic.split(".")[0]
    topic_questions = ALL_QUESTIONS.get(topic_num_str, [])

    if selected_difficulty == "הכל":
        filtered_q = topic_questions
    else:
        filtered_q = [q for q in topic_questions if q.get("difficulty") == selected_difficulty]

    st.subheader(f"📌 {selected_topic}")
    
    if not filtered_q:
        st.warning("⚠️ לא נמצאו שאלות ברמת קושי זו עבור הנושא שנבחר.")
    else:
        st.info(f"מציג **{len(filtered_q)}** שאלות ברמת קושי: **{selected_difficulty}** עבור **{current_user}**")

        score = 0
        total_answered = 0

        for idx, q in enumerate(filtered_q, 1):
            q_id = str(q.get("id", idx))

            badge_class = {
                "קל": "badge-easy",
                "בינוני": "badge-medium",
                "קשה": "badge-hard",
                "מומחה": "badge-expert"
            }.get(q.get("difficulty", "קל"), "badge-easy")

            st.markdown(f"""
            <div class="question-card">
                <span class="difficulty-badge {badge_class}">דרגה: {q.get("difficulty", "קל")}</span>
                <div class="question-title">שאלה {idx}: {q["question"]}</div>
            </div>
            """, unsafe_allow_html=True)

            saved_answer = user_profile["answers"].get(q_id, None)

            selected_opt = st.radio(
                f"בחר/י תשובה לשאלה {idx}:",
                q["options"],
                index=q["options"].index(saved_answer) if saved_answer in q["options"] else None,
                key=f"q_{q_id}_{topic_num_str}_{current_user}"
            )

            if selected_opt:
                user_profile["answers"][q_id] = selected_opt
                total_answered += 1

                if selected_opt == q["correct"]:
                    st.success("✅ תשובה נכונה!")
                    score += 1
                else:
                    st.error(f"❌ תשובה שגויה. התשובה הנכונה היא: **{q['correct']}**")

                st.markdown(f"""
                <div class="explanation-box">
                    <strong>💡 הסבר מפורט מהספרות והשחזורים:</strong><br>
                    {q['explanation']}
                </div>
                """, unsafe_allow_html=True)

                # Bookmark / Summary button
                is_bookmarked = q_id in user_profile["bookmarks"]
                btn_label = "📌 הסר ממחברת הסיכומים" if is_bookmarked else "📌 סמן נושא זה לסיכום וחיזוק"
                
                if st.button(btn_label, key=f"bm_btn_{q_id}_{current_user}"):
                    if is_bookmarked:
                        del user_profile["bookmarks"][q_id]
                        st.toast("הנושא הוסר ממחברת הסיכומים שלך!")
                    else:
                        user_profile["bookmarks"][q_id] = {
                            "topic": q.get("topic", selected_topic),
                            "question": q["question"],
                            "correct": q["correct"],
                            "explanation": q["explanation"]
                        }
                        st.toast("הנושא התווסף בהצלחה למחברת הסיכומים האישית שלך!")
                    save_data(st.session_state.user_data)
                    st.rerun()

                st.markdown("---")
                col1, col2 = st.columns([1, 2])
                with col1:
                    saved_rating = user_profile["ratings"].get(q_id, 5)
                    rating = st.slider(
                        "דירוג איכות השאלה והמסיחים (1-5):",
                        1, 5, value=saved_rating, key=f"rate_{q_id}_{topic_num_str}_{current_user}"
                    )
                    user_profile["ratings"][q_id] = rating

                with col2:
                    saved_fb = user_profile["feedback"].get(q_id, "")
                    fb_text = st.text_input(
                        "הערה/משוב אישי לשאלה זו (אופציונלי):",
                        value=saved_fb, key=f"fb_{q_id}_{topic_num_str}_{current_user}"
                    )
                    user_profile["feedback"][q_id] = fb_text

            st.markdown("<br>", unsafe_allow_html=True)

        save_data(st.session_state.user_data)

        if total_answered > 0:
            st.sidebar.markdown("---")
            st.sidebar.subheader("📊 סיכום התקדמות אישית")
            st.sidebar.progress(total_answered / len(filtered_q))
            st.sidebar.write(f"ענית על **{total_answered}** מתוך **{len(filtered_q)}** שאלות")
            st.sidebar.write(f"ציון נוכחי: **{int((score/total_answered)*100)}%** ({score}/{total_answered})")

else:
    # Bookmarks Notebook Mode
    st.header(f"📌 מחברת סיכומים אישית לחיזוק - {current_user}")
    st.write("כאן מרוכזות כל הנקודות התיאורטיות שסימנת במהלך התרגול לצורך חזרה ושינון:")

    bookmarks = user_profile.get("bookmarks", {})
    if not bookmarks:
        st.info("טרם סימנת נושאים לחיזוק. במהלך התרגול, לחץ על הכפתור '📌 סמן נושא זה לסיכום וחיזוק' מתחת לשאלות שתרצה לשנן, והן יופיעו כאן באופן מאורגן!")
    else:
        grouped_bm = {}
        for q_id, bm_data in bookmarks.items():
            top = bm_data.get("topic", "כללי")
            if top not in grouped_bm:
                grouped_bm[top] = []
            grouped_bm[top].append((q_id, bm_data))

        for top, bm_list in grouped_bm.items():
            st.markdown(f"### 📚 {top}")
            for q_id, bm_data in bm_list:
                st.markdown(f"""
                <div class="bookmark-box">
                    <strong>❓ שאלה:</strong> {bm_data['question']}<br>
                    <strong>✅ תשובה נכונה:</strong> {bm_data['correct']}<br><br>
                    <strong>💡 תמצית הסיכום והדגשים הלימודיים:</strong><br>
                    {bm_data['explanation']}
                </div>
                """, unsafe_allow_html=True)
                
                if st.button(f"🗑️ הסר ממחברת הסיכומים", key=f"del_bm_{q_id}"):
                    del user_profile["bookmarks"][q_id]
                    save_data(st.session_state.user_data)
                    st.toast("הנושא הוסר ממחברת הסיכומים!")
                    st.rerun()
                st.markdown("<br>", unsafe_allow_html=True)
